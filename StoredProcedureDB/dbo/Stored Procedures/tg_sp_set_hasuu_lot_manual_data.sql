...
-- =============================================
CREATE PROCEDURE [dbo].[tg_sp_set_hasuu_lot_manual_data]
...
BEGIN
	...
	, [lot_no])
		...
		,HOST_NAME()
		,APP_NAME()
		,'EXEC [tg_sp_set_hasuu_lot_manual_data] Get Store @empno = ''' + CAST(@EmpNo_int as varchar(7)) + ''',@lotno = ''' + @newlotno + ''',@QtyPass = ''' + CAST(@total_pcs as varchar(7)) + ''''
		,@newlotno
	...
	BEGIN TRY
		...
		BEGIN TRY
				...
				, [lot_no])
					...
					,HOST_NAME()
					,APP_NAME()
					,'EXEC [tg_sp_set_hasuu_lot_manual_data] Set data to APCSPro @empno = ''' + CAST(@EmpNo_int as varchar(7)) + ''',@lotno = ''' + @newlotno + ''',@QtyPass = ''' + CAST(@total_pcs as varchar(7)) + ''''
					,@newlotno
	...
	BEGIN CATCH
		...
		, [lot_no])
			...
			,HOST_NAME()
			,APP_NAME()
			,'EXEC [tg_sp_set_hasuu_lot_manual_data] Set data store Error @empno = ''' + CAST(@EmpNo_int as varchar(7)) + ''',@lotno = ''' + @newlotno + ''',@QtyPass = ''' + CAST(@total_pcs as varchar(7)) + ''''
			,@newlotno
