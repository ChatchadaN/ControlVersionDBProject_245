...
-- =============================================
CREATE PROCEDURE [dbo].[tg_sp_cancel_mix_lot_002]
...
BEGIN
	...
	SELECT GETDATE()
		...
		,HOST_NAME()
		,APP_NAME()
		,'EXEC [dbo].[tg_sp_cancel_mix_lot_002] @lot_standard = ''' + @lot_standard + ''''
		,@lot_standard
   ...
   SELECT @OPName =
	...
	BEGIN
		...
		set operated_by = @EmpnoId --@EmpNo_int --new
			,created_by = @EmpnoId --@EmpNo_int --new
			,updated_by = @EmpnoId --@EmpNo_int --new
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN CATCH
				...
				, HOST_NAME()
				, APP_NAME()
				, 'EXEC [dbo].[tg_sp_cancel_mix_lot_002 Update Hasuu Qty In Table Label Error !! ] @lot_standard = ''' + @lot_standard 
				, @lot_standard
			END CATCH
		END
