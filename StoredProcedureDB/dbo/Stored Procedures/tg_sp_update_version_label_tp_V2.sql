...
-- =============================================
CREATE PROCEDURE [dbo].[tg_sp_update_version_label_tp_V2]  
...
BEGIN
	...
	--Check QC_Instruction or Tomson_3
	DECLARE @Qc_InStruction_allocat char(5) = ''
	DECLARE @Qc_InStruction_label_rec char(5) = ''
	DECLARE @Lotno_allocat char(10) = ''
	DECLARE @FactoriesName nvarchar(10) = ''
	...
	SELECT @FactoriesName = factories.name
	...
	where emp_num = TRIM(@empno_str)

	IF @FactoriesName = 'REPI'
	BEGIN
		SELECT @OPName =
		CASE
		...
		FROM [APCSProDB].[man].[users]
		WHERE [users].[emp_num] = TRIM(@empno_str)
	END
	ELSE
	BEGIN
		SELECT @OPName =
	...
	--Date Modify : 20022/05/18 Time : 11.04
	--Check Tomson_3 in table : label_issue_record
	select @Qc_InStruction_label_rec = TRIM(tomson_3) from APCSProDB.trans.label_issue_records where lot_no = @lotno and type_of_label = 1
    --Check Tomson_3 in table : Allocation 
	select @Lotno_allocat = LotNo,@Qc_InStruction_allocat = TRIM(Tomson3) from APCSProDB.method.allocat where LotNo = @lotno
	
	--check lotno in allocation is not null or is not blank
	IF @Lotno_allocat != ''
	BEGIN
		--Compare Data Tomson 3 
		IF @Qc_InStruction_label_rec != @Qc_InStruction_allocat
		BEGIN
			--if tomson_rec != tomson_allocat is update tomson_3 in table : label_issur_record
			update APCSProDB.trans.label_issue_records
			set tomson_3 = @Qc_InStruction_allocat
			   ,update_at = GETDATE()
			where lot_no = @lotno
			...
			SELECT GETDATE()
				...
				,HOST_NAME()
				,APP_NAME()
				,'EXEC [dbo].[tg_sp_update_version_label_tp_V2 Update QC_Instruction] @lotno = ''' + @lotno + ''',@empno = ''' + CAST(@emp_no as varchar(7)) + ''''
				,@lotno
		END
	END
	

    -- Support Label Type Hasuu and Reel
	IF @status = 1
	BEGIN
		IF @reel_number = ' '
		BEGIN
				...
				,op_no = @emp_no  --new --Update 30/01/2025 14:35 By George
	...
	BEGIN
		IF @reel_number = ' '
		BEGIN
				--UPDATE QRCODE DETAIL REEL AND HASUU LABEL , Data Update : 2022/04/27 time : 12.49
				update APCSProDB.trans.label_issue_records
				SET qrcode_detail = SUBSTRING(qrcode_detail,1,35) + CAST(IIF(VERSION > 9,9,VERSION) AS CHAR(1)) + FORMAT(CAST(no_reel AS INT),'00')
				where lot_no = @lotno 
				and type_of_label = @Type_label
	...
	ELSE
		BEGIN
				--UPDATE QRCODE DETAIL REEL , Data Update : 2022/04/27 time : 12.49
				update APCSProDB.trans.label_issue_records
				SET qrcode_detail = SUBSTRING(qrcode_detail,1,35) + CAST(IIF(VERSION > 9,9,VERSION) AS CHAR(1)) + FORMAT(CAST(no_reel AS INT),'00')
				where lot_no = @lotno 
				and type_of_label = @Type_label
