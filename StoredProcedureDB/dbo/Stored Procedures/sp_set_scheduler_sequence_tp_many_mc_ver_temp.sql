...
-- =============================================
CREATE PROCEDURE [dbo].[sp_set_scheduler_sequence_tp_many_mc_ver_temp]
...
delete from [DBx].[dbo].[scheduler_tp_qa_wip_01] where pkg_id in (SELECT DISTINCT package_group_id FROM APCSProDB.method.packages WHERE name in ((SELECT * from STRING_SPLIT( @PackageName , ',' ))));

BEGIN TRANSACTION;
BEGIN TRY

--Delete old table insert new table HERE !!!
DELETE FROM DBxDW.dbo.[scheduler_temp_seq_tp_01];
...
-----------------Get individual WIP include QA and TP Job-----------------------------------------------------------/
	...
	([mc_name] ,lot_no ,device_name ,pkg_name ,job_name ,kpcs ,qty_production ,state ,standare_time ,job_id ,update_at ,rack_address ,rack_name ,pkg_id)	
...
----- SET TP Calculate TO sum all same deivce into lot ----------***************
 ...
 EXEC	[StoredProcedureDB].[dbo].[sp_set_scheduler_tp_qa_calculate_ver_temp] @Pk_name = @PackageName;
...
EXEC	[StoredProcedureDB].[dbo].[sp_get_scheduler_tp_qa_calculate_ver_temp] @value = 1;
...
---------------------------------------------------------------------
	...
	from(select pk.name,device.id AS device_id, device.name as Devicename,SUM( lots.qty_in)  as Kpcs
		...
		where lot_record.record_class = 2  and job_id in (222,231,236,289,397,401,409,428)  
			and PK.name in (SELECT * from STRING_SPLIT ( @PackageName , ',' )) --กำหนดแพ็คเก็ทกรุ๊ป
...
seq_no bigint, package_name varchar(30),lot_end datetime,lot_start datetime
		...
		FROM #getLotInMC_TEST as lots where lot_no COLLATE SQL_Latin1_General_CP1_CI_AS NOT IN (select lot_no from DBxDW.dbo.[scheduler_temp_seq_tp_01] ) 

		INSERT INTO DBxDW.dbo.[scheduler_temp_seq_tp_01]
		SELECT distinct lots.lot_no , lots.JobName ,lots.FTDevice ,'' as rack_address,'' as rack_name,lots.MCName as MachineName,1 as Seq_no,lots.pkg ,lots.ENDTIME,lots.STARTTIME 
		FROM #getLotInMC_TEST as lots where lot_no COLLATE SQL_Latin1_General_CP1_CI_AS NOT IN (select lot_no from DBxDW.dbo.[scheduler_temp_seq_tp_01] ) 
...
--------------FIND WIP TP ONLY FOR GROUPING DEVICE AND MACHINE TOGETHER----------------------------------------------------------------------
		...
		ISNULL(rack_addresses.address,'Setup') as rack_address, --Rack.address
		rack_controls.name as rack_name --Rack.name
		...
		2 as Seq_no,Package.name as package_name,NULL as lot_end,NULL as lot_start,deivce.name as deviceName,Machine_table.id as mcid INTO #MachineLockSeq
			...
			where lots.act_job_id  in (222,236,289,231,397,401,409,428) and ((lots.process_state in (0,100) and lots.location_id is not null) or (lots.process_state in (1,101)))
	...
	, Is_GDIC int,wip_state tinyint,quality_state tinyint,FT_lots int
		...
		(
			...
			(
				...
				FROM [DBx].[dbo].[scheduler_tp_qa_wip_01] as temp
				...
				WHERE job_id in (222,231,236,289,397,401,409,428) and pkg_name in (SELECT * from STRING_SPLIT ( @PackageName , ',' ))
