...
-- =============================================
CREATE PROCEDURE [dbo].[tg_sp_get_data_reel_all_Test]
...
BEGIN
	...
	SELECT GETDATE()
		...
		,HOST_NAME()
		,APP_NAME()
		,'EXEC [dbo].[tg_sp_get_data_reel_all_Test] @lot_no = ''' + @lotno + ''' @Reel_no = ''' + CAST(@reel_no as varchar(20)) + ''' @Status = ''' + CAST(@status_int as varchar(2)) + ''' @empno = ''' + @emp_no + ''''
		,@lotno
	...
	BEGIN
		...
		BEGIN
			BEGIN TRY
				...
				select @qty_hasuu_now = case when qty = 0 then @qty_hasuu_rollback else (qty - @qty_hasuu_rollback) end
				from APCSProDB.trans.label_issue_records 
				...
				--Get Data qty Hasuu
				--select @qty_hasuu = qty from APCSProDB.trans.label_issue_records where lot_no = @lotno and type_of_label = 2  --แก้ตรงการคำนวณ hasuu ตรงนี้ด้วย
				...
				BEGIN
					...
					WHERE lot_no = @lotno
				END
				ELSE IF @in_stock_now = 2  --New Condition 2022/06/22
				BEGIN
					--Update qty_out Tran lot
					UPDATE APCSProDB.trans.lots
					SET qty_out = @qty_shipment
				...
				SELECT GETDATE()
					...
					,HOST_NAME()
					,APP_NAME()
					,'EXEC [dbo].[tg_sp_get_data_reel_all_Test Set Data Reel RollBack] @lot_no = ''' + @lotno + ''' @Reel_no = ''' + CAST(@reel_no as varchar(20)) + ''' @Status = ''' + CAST(@status_int as varchar(2)) + ''' @empno = ''' + @emp_no + ''''
					,@lotno
