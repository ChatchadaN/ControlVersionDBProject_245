...
-- =============================================
CREATE PROCEDURE [dbo].[tg_sp_set_qty_fristlot_update_ver2_test] 
...
BEGIN
	...
	DECLARE @Barcode_buttom_hasuu_for_pcrequest char(18) = ''
	...
	SELECT GETDATE()
		...
		,HOST_NAME()
		,APP_NAME()
		,'EXEC [dbo].[tg_sp_set_qty_fristlot_update_ver2_test after lot end update qty on label]  @lot_no = ''' + @lot_no + ''',@qty = ''' + CONVERT (varchar (10), @qty) + ''',@reel_standard = ''' + CONVERT (varchar (10), @reel_standard) + ''''
		,@lot_no
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						print '@reel_max_before > @reel_max_after'
						--update APCSProDB.trans.label_issue_records 
						--set type_of_label = 0
						--,update_at = GETDATE()
			...
			BEGIN
				...
				BEGIN
				    print '@qty_out_new_value > @qty_out_get_tranlot'
					--update APCSProDB.trans.label_issue_records 
					--set type_of_label = 3
					--,update_at = GETDATE()
	...
	BEGIN
		...
		BEGIN
			...
			select @qty = @qty_out_get_tranlot
		...
		select @Qrcode_Hasuu_new as Qrcode_Hasuu_new,@Barcode_buttom_hasuu as Barcode_buttom_hasuu
		,@Barcode_buttom_hasuu_new as Barcode_buttom_hasuu_new,@Qrcode_Hasuu_Shipment as Qrcode_Hasuu_Shipment
		,@qty as qty,@qty_hasuu as qty_hasuu,@qty_out_get_tranlot as qty_out,@reel_standard as reel_standard

		BEGIN TRY
			IF @pcInstruction = 13 --pc request new version
			BEGIN
				print '@pcInstruction = 13'
				...
				BEGIN
					...
					select
					 @qty_out_get_tranlot
					, @Barcode_buttom_hasuu_new
					, @Qrcode_Hasuu_Shipment
					, GETDATE()
					,'hasuu shipment'
