...
-- =============================================
CREATE PROCEDURE [dbo].[sp_set_scheduler_temp] 
...
BEGIN TRY
	...
	SELECT DISTINCT [FTSetupReport].[MCNo]
		...
		INSERT INTO DBxDW.dbo.scheduler_temp_ft_history 		
		 SELECT  GETDATE() AS record_at
		  ...
		  ,[AdaptorA]
		  ,[AdaptorB]
	  FROM [DBxDW].[dbo].[scheduler_temp_ft]

	  DELETE DBxDW.dbo.[scheduler_temp] FROM DBxDW.dbo.[scheduler_temp] where rack_name is null and rack_address is null and lot_start is null
	  COMMIT;
END TRY
BEGIN CATCH
