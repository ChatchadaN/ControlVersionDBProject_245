...
-- =============================================
CREATE PROCEDURE [dbo].[tg_sp_get_lot_stock_V2]
		...
		BEGIN
			...
			CROSS APPLY (
				...
				WHERE (
						...
						(
							(CASE
								WHEN SUBSTRING(lots.lot_no, 5, 1) = 'A' AND SUBSTRING(serial_no, 5, 1) IN ('A','B') THEN 1
								WHEN SUBSTRING(lots.lot_no, 5, 1) = 'B' AND SUBSTRING(serial_no, 5, 1) IN ('A','F') THEN 1
								WHEN SUBSTRING(lots.lot_no, 5, 1) = 'F' AND SUBSTRING(serial_no, 5, 1) IN ('A','B','F') THEN 1
								ELSE 0
							END) = 1
