...
-- =============================================
CREATE PROCEDURE [dbo].[tg_sp_new_dlot_recall_is_if]
...
BEGIN
	...
	left join APCSProDB.method.allocat_temp as allocat on lot.lot_no = allocat.LotNo  --Add Query 2023/02/03 Time : 10.48
	...
	select  @op_no_len_value =  case when LEN(CAST(@EmpNo_Char as char(5))) = 4 then '0' + CAST(@EmpNo_Char as char(5))
		...
		, [lot_no])
			...
			,HOST_NAME()
			,APP_NAME()
			,'EXEC [tg_sp_new_dlot_recall_is_if] Set data to IS @empno = ''' + @empno + ''',@lotno = ''' + @newlotno + ''',hasuu_lot = ''' + @hasuu_lotno + '''' + ''',total_pcs = ''' + cast(@total_pcs as varchar) + ''',carrier_no_set = ''' + @carrier_no_set + ''''
			,@newlotno
