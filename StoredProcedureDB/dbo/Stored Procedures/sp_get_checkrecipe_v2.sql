...
-- =============================================
CREATE PROCEDURE [dbo].[sp_get_checkrecipe_v2] 
...
BEGIN
	SET NOCOUNT ON;
		...
		SELECT  ft_name,recipe,job_id,ProgramName AS ProgramOIS, TBOIS.Package1,device_slip_id
		,TBAPCSPRO.device_id,TBOIS.DeviceOIS,TBOIS.Flow AS FlowOIS,version_num,device,assy_name
		...
		LEFT JOIN APCSProDB.trans.job_commons with(NOLOCK) on job_commons.job_id = device_flows.job_id AND device_flows.job_id <> 222
