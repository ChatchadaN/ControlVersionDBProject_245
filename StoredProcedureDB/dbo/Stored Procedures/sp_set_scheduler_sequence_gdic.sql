...
-- =============================================
CREATE PROCEDURE [dbo].[sp_set_scheduler_sequence_gdic] 
...
BEGIN Try
	...
	IF EXISTS(
		...
		LEFT JOIN [DBx].[dbo].[rcs_current_locations] as [current_locations] ON [Rack].[id] = [current_locations].[location_id]
		...
		AND (ISNULL(special_flows.quality_state,lots.quality_state) != 0 
			...
			OR ISNULL([current_locations].[status] ,0) != 1 )
	...
	SELECT DISTINCT  
		...
		, ISNULL(Rack.address,'Setup') as rack_address
		, ISNULL(Rack.name,(select distinct rack.name from DBx.dbo.rcs_records 
			inner join ( select lot_no,MAX(update_at_in) as Latest_date from DBx.dbo.rcs_records group by  lot_no)as RCS on RCS.lot_no = lots.lot_no WHERE  rcs_records.lot_no= lots.lot_no ))
		  as RackName
	...
	where ((lots.process_state in (0,100) and lots.location_id is not null) or (lots.process_state in (1,101)))
        ...
        WHERE Flow = CASE 
					...
					WHEN Job.name = 'AUTO(1) HV' THEN 'AUTO1 HV'
					WHEN Job.name LIKE '%ASISAMPLE%' 
						THEN TRIM(REPLACE(REPLACE(REPLACE(Job.name,'(',''),')',''),'ASISAMPLE',''))
					WHEN Job.name LIKE '%BIN27' 
						THEN TRIM(REPLACE(REPLACE(REPLACE(Job.name,'(',''),')',''),'BIN27',''))
	...
	INSERT INTO [DBxDW].[dbo].[scheduler_temp] select * from #LockNextLots_gdic
	...
	INSERT INTO [DBxDW].[dbo].[scheduler_temp] select distinct * from #Result
