...
-- =============================================
CREATE PROCEDURE [dbo].[tg_sp_set_auto_continue_lot_V3_manual_data_by_aomsin]
...
BEGIN
   ...
   ''SELECT COUNT(HASUU_LotNo) as [count] FROM [DBLSISHT].[dbo].[MIX_HIST] with (NOLOCK) WHERE HASUU_LotNo = ''''' + @lotno_standard + ''''''')';
	...
	IF @Lot_Type_Standard = 'F' and @Lot_Type_Hasuu = 'D'
	BEGIN
		SELECT 'FALSE' AS Status ,'MixLot ERROR !!' AS Error_Message_ENG,N'Lot Hasuu Before' + @hasuu_lot + 'และ' + @lotno_standard + 'ไม่สามารถทำการ mix กันได้'  AS Error_Message_THA ,N' กรุณาติดต่อ System' AS Handling
		RETURN
	END
	...
	ELSE 
	BEGIN
		select @MNo_Hasuu_Value = @MNo_Hasuu
	END
	...
	BEGIN
		...
		BEGIN
			...
			IF @Package = @package_loths AND @ROHM_Model_Name = @device_loths AND @Rank = @Rank_hasuu AND @Tomson_Mark_3 = @Tomson_Mark_3_lot_hasuu
				...
				--close update qty_pass Date : 20210930
				...
				BEGIN
						...
						,@stock_class = '01'  --Update 2022/03/10
				END
				...
				,@emp_no_int = @EmpNo_int --update 2021/12/07 Time : 11.54
				...
				BEGIN CATCH 
					...
					SELECT GETDATE()
						...
						, 'StoredProcedureDB'
						, 'TGSYSTEM'
						, 'EXEC [dbo].[tg_sp_set_auto_continue_lot_V3_manual_data_by_aomsin Create Record Class TG Error] @lotno_standard = ''' + @lotno_standard 
						, @lotno_standard
				END CATCH
				...
				SELECT GETDATE()
					...
					,HOST_NAME()
					,APP_NAME()
					,'EXEC [dbo].[tg_sp_set_auto_continue_lot_V3_manual_data_by_aomsin] @lotno_standard = ''' + @lotno_standard + ''', @hasuu_lot = ''' + @hasuu_lot + ''', @hasuu_qty = ''' + CONVERT (varchar (10), @hasuu_qty) + ''', @lotno_standard_qty = '''+ CONVERT (varchar (10), @lotno_standard_qty) + ''', @empno = ''' + @empno + ''',@MNo_Hasuu = ''' + @MNo_Hasuu + ''',@package_loths = ''' + @package_loths + ''',@device_loths = ''' + @device_loths + ''', @qty_lot_hasuu_update = ''' + CONVERT (varchar (5), @State_Qty_hasuu) +  ''', @McName = ''' + @machine_name + ''', @MCId = ''' + CONVERT (varchar (10), @machine_id) + ''''
					,@lotno_standard
				END
			ELSE 
			BEGIN
	...
	ElSE IF @hasuu_qty = 0
		BEGIN
			...
			IF @count_lotid_fristlot = 0
				BEGIN
					...
					SELECT GETDATE()
						...
						,HOST_NAME()
						,APP_NAME()
						,'EXEC [dbo].[tg_sp_set_auto_continue_lot_V3_manual_data_by_aomsin] @lotno_standard = ''' + @lotno_standard + ''', @hasuu_lot = ''' + @hasuu_lot + ''', @hasuu_qty = ''' + CONVERT (varchar (10), @hasuu_qty) + ''', @lotno_standard_qty = '''+ CONVERT (varchar (10), @lotno_standard_qty) + ''', @empno = ''' + @empno + ''',@MNo_Hasuu = ''' + @MNo_Hasuu + ''',@package_loths = ''' + @package_loths + ''',@device_loths = ''' + @device_loths +  ''', @McName = ''' + @machine_name + ''', @MCId = ''' + CONVERT (varchar (10), @machine_id) +  ''''
						,@lotno_standard
					...
					BEGIN
						...
						SELECT 'TRUE' AS Status ,'Frist Lot Success !!' AS Error_Message_ENG,N'บันทึกข้อมูลเรียบร้อย !!' AS Error_Message_THA
						RETURN
					END
			...
			ELSE
				BEGIN
					SELECT 'TRUE' AS Status ,'Function Frist Lot Warning!!' AS Error_Message_ENG,N'มีการทำ FristLot ไปแล้ว !!' AS Error_Message_THA ,N' กรุณาติดต่อ System' AS Handling
					RETURN
				END
