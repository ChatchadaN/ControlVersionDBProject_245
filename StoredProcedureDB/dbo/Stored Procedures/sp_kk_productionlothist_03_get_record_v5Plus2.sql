...
CREATE PROCEDURE [dbo].[sp_kk_productionlothist_03_get_record_v5Plus2] @lot_id int = NULL
AS
BEGIN
	...
	FROM (
		...
		FROM (
			SELECT t0.id AS id
				...
				,CASE 
					WHEN t0.record_class = 2
						THEN t0.qty_last_pass
					ELSE NULL
					END AS qty_last_pass
				,CASE 
					WHEN t0.record_class = 2
						THEN t0.qty_last_fail
					ELSE NULL
					END AS qty_last_fail
				,CASE 
					WHEN t0.record_class = 2
						THEN t0.qty_pass_step_sum
					ELSE NULL
					END AS qty_pass_step_sum
				,CASE 
					WHEN t0.record_class = 2
						THEN t0.qty_fail_step_sum
					ELSE NULL
					END AS qty_fail_step_sum
				,CASE 
					WHEN t0.record_class = 2
						THEN t0.qty_p_nashi
					ELSE NULL
				...
				,CASE 
					WHEN t0.record_class = 2
						THEN t0.qty_cut_frame
					ELSE NULL
					END AS qty_cut_frame
				,CASE 
					WHEN t0.record_class = 2
						THEN t0.qty_frame_in
					ELSE NULL
					END AS qty_frame_in
				,CASE 
					WHEN t0.record_class = 2
						THEN t0.qty_frame_pass
					ELSE NULL
					END AS qty_frame_pass
				,CASE 
					WHEN t0.record_class = 2
						THEN t0.qty_frame_fail
					ELSE NULL
					END AS qty_frame_fail
				,t0.recipe AS recipe
				,t0.delay2 AS delay2
				,CASE 
					WHEN t0.record_class = 1
	...
	FROM (
		...
		FROM (
			SELECT t1.id AS id
				...
				,max(t1.qty_in) OVER (PARTITION BY t1.lot_id) AS qty_in
				,max(t1.qty_pass) OVER (PARTITION BY t1.lot_id) AS qty_pass
			...
			FROM (
				SELECT c_lp.id AS id
					...
					,CASE 
						WHEN c_lp.record_class = 2
							THEN c_lp.qty_last_pass
						ELSE NULL
						END AS qty_last_pass
					,CASE 
						WHEN c_lp.record_class = 2
							THEN c_lp.qty_last_fail
						ELSE NULL
						END AS qty_last_fail
					,CASE 
						WHEN c_lp.record_class = 2
							THEN c_lp.qty_pass_step_sum
						ELSE NULL
						END AS qty_pass_step_sum
					,CASE 
						WHEN c_lp.record_class = 2
							THEN c_lp.qty_fail_step_sum
						ELSE NULL
						END AS qty_fail_step_sum
					,CASE 
						WHEN c_lp.record_class = 2
							THEN c_lp.qty_p_nashi
						ELSE NULL
						END AS qty_p_nashi
					,CASE 
						WHEN c_lp.record_class = 2
							THEN c_lp.qty_front_ng
						ELSE NULL
						END AS qty_front_ng
					,CASE 
						WHEN c_lp.record_class = 2
							THEN c_lp.qty_marker
						ELSE NULL
						END AS qty_marker
					,CASE 
						WHEN c_lp.record_class = 2
							THEN c_lp.qty_combined
						ELSE NULL
						END AS qty_combined
					,CASE 
						WHEN c_lp.record_class = 2
					...
					,CASE 
						WHEN c_lp.record_class = 2
							THEN c_lp.qty_cut_frame
						ELSE NULL
						END AS qty_cut_frame
					,CASE 
						WHEN c_lp.record_class = 2
							THEN c_lp.qty_frame_in
						ELSE NULL
						END AS qty_frame_in
					,CASE 
						WHEN c_lp.record_class = 2
							THEN c_lp.qty_frame_pass
						ELSE NULL
						END AS qty_frame_pass
					,CASE 
						WHEN c_lp.record_class = 2
							THEN c_lp.qty_frame_fail
						ELSE NULL
						END AS qty_frame_fail
	...
	FROM (
		SELECT convert(NVARCHAR, ROW_NUMBER() OVER (
			...
			,rtrim(dv.name) as device_name
			,rtrim(pk.name) as package_name
			,t0.lot_id AS lot_id
			,rtrim(dl.lot_no) AS lot_no
			...
			,t0.qty_frame_fail
			----工程歩留まり[%]
			,convert(DECIMAL(9, 1), t0.qty_last_pass) / nullif((t0.qty_last_pass + t0.qty_last_fail), 0) * 100 AS yield
			----全体歩留まり[%]
		...
		left outer join APCSProDb.method.device_names as dv with (NOLOCK) on dv.id = dl.device_id
		left outer join APCSProdwh.dwh.dim_packages as pk with (NOLOCK) on pk.id = dl.package_id
