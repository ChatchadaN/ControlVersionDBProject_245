...
-- =============================================
Create PROCEDURE [dbo].[tg_sp_auto_disable_reel_and_update_qty_on_label] 
...
BEGIN
	...
	SELECT GETDATE()
		...
		,HOST_NAME()
		,APP_NAME()
		,'EXEC [dbo].[tg_sp_set_qty_fristlot_update_ver2 after lot end update qty on label]  @lot_no = ''' + @lot_no + ''',@qty = ''' + CONVERT (varchar (10), @qty) + ''',@reel_standard = ''' + CONVERT (varchar (10), @reel_standard) + ''''
		,@lot_no
