...
-- =============================================
CREATE PROCEDURE [dbo].[tg_sp_get_data_cancel_mix_lot]
...
BEGIN
	...
	DECLARE @wip_state tinyint = null  --add parameter 2022/12/05 time : 13.53
	...
	SELECT HASUU_LotNo,LotNo,Type_Name,ROHM_Model_Name,TPRank,Packing_Standerd_QTY,QTY FROM [APCSProDWH].[dbo].[MIX_HIST_IF] WHERE HASUU_LotNo = @lotno;
	...
	IF @lotno != ''
	BEGIN
		IF @wip_state != 200  --Check wip state is cancel not show data -->date modify : 2022/12/05 time : 13.59
		BEGIN
			IF @process_state = 2
			BEGIN
			...
			END
			ELSE IF @process_state = 101
			BEGIN
				select @status_lot = N'101' --Abnormal Setup
			END
			ELSE IF @process_state = 102
			BEGIN
				select @status_lot = N'102' --Abnormal Start
			END
			ELSE
			BEGIN
			...
			BEGIN
				...
				select ROW_NUMBER() OVER(ORDER BY sur.pcs) AS row_id  
					...
					, ISNULL(lot_master.pc_instruction_code,0) AS pc_instruction_code
			...
			BEGIN
				...
				select ROW_NUMBER() OVER(ORDER BY mix_hist.QTY) AS row_id  
					,lot_master.lot_no as lot_master
					, isnull(mix_hist.LotNo,'') as lot_member
