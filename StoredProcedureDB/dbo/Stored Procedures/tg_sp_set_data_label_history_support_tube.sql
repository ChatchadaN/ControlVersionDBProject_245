-- =============================================
-- Author:		<Author,,Vanatjaya S.>
-- Create date: <2022/06/20>
-- Update Data: 
-- Description:	<Label History>
-- =============================================
CREATE PROCEDURE [dbo].[tg_sp_set_data_label_history_support_tube]
	-- Add the parameters for the stored procedure here
	 @lot_no_value varchar(10) = ' '
	,@process_name varchar(10) = ' '
	,@emp_no_val char(6) = '' 
	,@is_std_tube_adjust int = 0
...
BEGIN
	...
	--Add parameter new_pcs_per_pack modify : 2022/06/20
	DECLARE @new_pcs_per_pack int = 0

	--<< modify : 2022/06/20
	select @new_pcs_per_pack = (case when @is_std_tube_adjust = 0 then device_names.pcs_per_pack else @is_std_tube_adjust end)
	...
	select @qty_lot_standard_before = (qty_out - qty_combined) from APCSProDB.trans.lots where lot_no = @lot_no_value

	select @qty_hasuu_before = qty_combined from APCSProDB.trans.lots where lot_no = @lot_no_value

	---New query 26/04/2022 08.40
	--select @Lotno = lot.lot_no
	...
	--						else (qty_out + qty_hasuu)/@is_std_tube_adjust end
	--	, @reel_num_shipment = (lot.qty_out/@is_std_tube_adjust)
	...
	--	, @Standard = case when @is_std_tube_adjust = 0 then dv.pcs_per_pack 
	--						else @is_std_tube_adjust end
	--	, @Total = case when @is_std_tube_adjust = 0 then ([qty_out] + [qty_hasuu]) - ([qty_out] + [qty_hasuu])%(dv.pcs_per_pack)  
	--					else ([qty_out] + [qty_hasuu]) - ([qty_out] + [qty_hasuu])%(@is_std_tube_adjust) end
	...
	select @Lotno = lot.lot_no
		, @QTY = case when SUBSTRING(lot.lot_no,5,1) = 'D' then lot.qty_pass else qty_out + qty_hasuu end
		, @qty_shipment = lot.qty_out
		, @reel_num = case when [surpluses].[original_lot_id] is not null or [surpluses].[original_lot_id] != 0 then 0 else (qty_out + qty_hasuu)/@new_pcs_per_pack end
		, @reel_num_shipment = lot.qty_out/@new_pcs_per_pack
		, @Hasuu_qty = case when [surpluses].[original_lot_id] is not null or [surpluses].[original_lot_id] != 0 then lot.qty_pass else (qty_out + qty_hasuu)%(@new_pcs_per_pack) end
		, @Package = [pk].[name]
		, @Shortname = [pk].[short_name]
		, @Device = [dv].[name]
		, @AssyName = [dv].[assy_name]
		...
		, @Rank = case when dv.rank is null then '' else dv.rank end
		, @TPRank = case when dv.tp_rank is null then '' else dv.tp_rank end
		, @Standard = @new_pcs_per_pack
		, @Total = ([qty_out] + [qty_hasuu]) - ([qty_out] + [qty_hasuu])%(@new_pcs_per_pack)  
		, @Tomson_box = [tomson].[tomson_box]
		, @BoxType = [tomson].[tomson_box]
		, @OPNo = [lot_cb].[updated_by]
	-->> modify : 2022/06/20
	...
	left join APCSProDB.method.allocat as allo_cat on lot.lot_no = allo_cat.LotNo
	...
	(
		...
		(
			...
			inner join [APCSProDB].[method].[material_set_list] on [material_sets].[id] = [material_set_list].[id]
				and [material_sets].[process_id] = 317
	...
	--Add Condition Wait Task Run Data Allocat Create : 2021/09/28 Time : 15.41 PM
	SELECT @get_time_value =  CONVERT(VARCHAR(5), GETDATE(), 108)
	IF (@get_time_value = '05:00' or @get_time_value = '09:00')
	BEGIN
		WAITFOR DELAY '00:00:08'
		select @Tomson_Mark_3 = Tomson3 from [APCSProDB].[method].[allocat] where LotNo = @Lotno
		--select 'WAITFOR DELAY'
	END
	ELSE
	BEGIN
		select @Tomson_Mark_3 = Tomson3 from [APCSProDB].[method].[allocat] where LotNo = @Lotno
		--select 'NO WAITFOR DELAY'
	END
	...
	DECLARE @Tomson3_D_lot char(4) = ''
	select top(1) @Tomson3_D_lot = den_pyo.TOMSON_INDICATION from APCSProDB.trans.lot_combine as lb
	inner join APCSProDB.trans.lots as lots on  lb.member_lot_id = lots.id
	inner join APCSDB.dbo.LCQW_UNION_WORK_DENPYO_PRINT as den_pyo on den_pyo.LOT_NO_2 = lots.lot_no
	where lot_id = @Lot_id
	...
	select 
	 @chk_tp_rank = ISNULL(dn.tp_rank,'')
	,@chk_universal_tp_rank = ISNULL(universal_tp_rank,'')
	...
	IF @chk_tp_rank = '' and @chk_universal_tp_rank = ''
	BEGIN
		...
		where ms.process_id = 317 and details = 'TRAY'
		...
		select @qty_set_tray = case when @count_qty_set_tray = '' then 0 else (@Standard/@count_qty_set_tray) end
		...
		--Create Check Mno Label Tray Date : 2021/11/1
		--select @check_mno_hasuu_for_tray as check_float_tray
		--select @convert_mno_hasuu_tray as mno_tray
	...
	SELECT GETDATE()
		...
		,HOST_NAME()
		,APP_NAME()
		,'EXEC [dbo].[tg_sp_set_data_label_history_support_tube] @Lotno = ''' + @Lotno + ''', @Process = ''' + @process_name + ''', @Qc_Instruction = ''' + @Tomson_Mark_3 + ''', @emp_no_at_ogi = ''' + @emp_no_val + ''''
		,@Lotno
