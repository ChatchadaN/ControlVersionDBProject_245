...
-- =============================================
CREATE PROCEDURE [dbo].[tg_sp_set_hasuu_lot_new_004]
...
BEGIN
	...
	--Modify query add condition check slip version is_released = 1 #2025.May.14 Time : 10.03 By Aomsin
	SELECT 
		 @StockClass = '01' --fix
	...
	--WHERE serial_no IN (SELECT TOP 1 TRIM(value) FROM STRING_SPLIT(@lot_hasuu, ','))
	WHERE serial_no IN (
			SELECT TOP 1 lots.lot_no
			FROM APCSProDB.trans.lots 
			INNER JOIN APCSProDB.method.device_names as dn on lots.act_device_name_id = dn.id
			LEFT JOIN [APCSProDB].[method].[device_versions] as dv on dv.device_name_id = dn.id and (dv.device_type = 0 or dv.device_type = 6)
			LEFT JOIN [APCSProDB].[method].[device_slips] as ds on ds.device_id = dv.device_id and ds.is_released in(1,2)
			WHERE lots.lot_no IN (SELECT TRIM(value) FROM STRING_SPLIT(@lot_hasuu, ','))
				AND ds.is_released = 1
		)
		AND (sur.updated_at  >= (getdate() - 1095) or sur.is_ability = 1)
		AND sur.pcs != '0'
	...
	DECLARE @lotno_1 varchar(10) = ''
	DECLARE @lotno_2 varchar(10) = ''
	DECLARE @lotno_3 varchar(10) = ''

	SELECT @lotno_1 = ISNULL([Lot1], '')
		,@lotno_2 = ISNULL([Lot2], '')
		,@lotno_3 = ISNULL([Lot3], '')
	FROM (
		SELECT TRIM(value) AS [data]
			, 'Lot' + CAST(ROW_NUMBER() OVER (ORDER BY (SELECT 1)) AS VARCHAR(2)) AS [columns]
		FROM STRING_SPLIT(@lot_hasuu, ',') AS [lotno]
	) AS [t1]
	PIVOT (
		MAX([data])
		FOR [columns] IN ([Lot1], [Lot2], [Lot3])
	) AS [pivotTable];
	...
	SELECT GETDATE()
		...
		, HOST_NAME()
		, APP_NAME()
		, 'EXEC [tg_sp_set_hasuu_lot_new_004] Get Store @empno = ''' + CAST(@EmpNo_int as varchar(7)) 
			+ ''',@lotno = ''' + @newlotno 
	...
	BEGIN TRY ---- START TRY
		...
		INSERT INTO [APCSProDWH].[dbo].[LSI_SHIP_IF] (
			  ...
			  ,[Timestamp_Time]
				...
				VALUES (
					...
					,@Label_Class
					,@newlotno
					,@lotno_1 -- hasuu_lotno ตัวที่ 1
					,@lotno_2 -- hasuu_lotno ตัวที่ 2 ถ้ามี
					,@lotno_3 -- hasuu_lotno ตัวที่ 3 ถ้ามี
					,'MX' -- Mno Standard
		...
		INSERT INTO [APCSProDWH].[dbo].[WH_UKEBA_IF] (
		   ...
		   VALUES
		   (
				   '10' --RECORD_CLASS  update : #2024/10/17 time: 11.43 by Aomsin
				  ,@ROHM_Model_Name
				  ,@newlotno
		...
		BEGIN TRY
			--Insert Record Hasuu Lot in Tabel : surpluses_record date modify : 2023/03/27  time : 14.32
			...
			SELECT  
				 ...
				 ,@emp_id AS operated_by  --new Modify : 2024/12/26
				 ,2 as record_class
				 ,id as surpluse_id
				 ,lot_id
				 ,pcs
		...
		BEGIN CATCH
			...
			SELECT GETDATE()
				...
				, HOST_NAME()
				, APP_NAME()
				, 'EXEC [tg_sp_set_hasuu_lot] Insert Surpluses Record Error for Lot Member @empno = ''' + CAST(@EmpNo_int as varchar(7)) 
					+ ''',@lotno = ''' + @newlotno 
		...
		--UPDATE InStock = 0 Hasuu Use IS_IF
		UPDATE APCSProDWH.dbo.H_STOCK_IF
		SET DMY_OUT_Flag = 1
		where LotNo IN (SELECT TRIM(value) FROM STRING_SPLIT(@lot_hasuu, ','));

		BEGIN TRY ---- START TRY 2
			EXEC [StoredProcedureDB].[dbo].[tg_sp_set_d_lot_in_tranlot] @lotno = @newlotno
				, @device_name = @device
				, @assy_name = @ASSY_Model_Name
				, @qty = @Total_All
				, @production_category_val = 20 
				, @carrier_no_val = @carrierNo   
			...
			SELECT 'TRUE' AS Status 
				...
				, N'บันทึกข้อมูล d_lot_in_tranlot สำเร็จ !!' AS Error_Message_THA 
				, N' กรุณาติดต่อ System' AS Handling
			RETURN;
		END TRY  ---- END TRY 2
		BEGIN CATCH ---- START CATCH 2
			---- ROLLBACK
			--IF @@TRANCOUNT > 0  
			--	ROLLBACK TRANSACTION;

			--Create log Date : 2024/06/27 Time : 13.15 by Aomsin
			---- insert log
			INSERT INTO [StoredProcedureDB].[dbo].[exec_sp_history]
				( [record_at]
				, [record_class]
			...
			SELECT GETDATE()
				...
				, HOST_NAME()
				, APP_NAME()
				, 'EXEC [tg_sp_set_hasuu_lot] can not create lot in tranlot @empno = ''' + CAST(@EmpNo_int as varchar(7)) 
					+ ''',@lotno = ''' + @newlotno 
					+ ''',@QtyPass = ''' + CAST(@total_pcs as varchar(7)) 
					+ ''',@QtyPass_Form_Sum_On_Store = ''' + CAST(@Total_All as varchar(7))
					+ ''',@Hasuu_lot_list = ''' + @hasuu_lot_list 
			...
			SELECT 'FALSE' AS Status 
				...
				, N'บันทึกข้อมูล d_lot_in_tranlot ผิดพลาด !!' AS Error_Message_THA 
				, N' กรุณาติดต่อ System' AS Handling
			RETURN;
		END CATCH ---- END CATCH 2
	
		---- return
		SELECT 'TRUE' AS Status 
			, 'Insert Success !!' AS Error_Message_ENG
			, N'บันทึกข้อมูล d_lot สำเร็จ !!' AS Error_Message_THA 
			, N' กรุณาติดต่อ System' AS Handling
		RETURN;
	END TRY ---- END TRY
	BEGIN CATCH ---- START CATCH
		...
		SELECT 'FALSE' AS Status 
			, 'Insert error !!' AS Error_Message_ENG
			, N'บันทึกข้อมูล d_lot ผิดพลาด !!' AS Error_Message_THA 
			, N' กรุณาติดต่อ System' AS Handling
		RETURN;
	END CATCH ---- END CATCH
END
