...
-- =============================================
CREATE PROCEDURE [dbo].[tg_sp_set_qty_fristlot_update_ver2] 
...
BEGIN
	...
	SELECT GETDATE()
		...
		,HOST_NAME()
		,APP_NAME()
		,'EXEC [dbo].[tg_sp_set_qty_fristlot_update_ver2 after lot end update qty on label]  @lot_no = ''' + @lot_no + ''',@qty = ''' + CONVERT (varchar (10), @qty) + ''',@is_process = ''' +  @is_process + ''',@reel_no = ''' +  @reel_no + ''',@qty_sum_ng = ''' + CONVERT (varchar (10), @qty_sum_ng) + ''''
		,@lot_no
