...
-- =============================================
CREATE PROCEDURE [dbo].[sp_set_scheduler_tp_qa_calculate]
...
BEGIN
	...
	BEGIN
		delete from DBx.dbo.scheduler_tp_qa_calculate 
		insert into DBx.dbo.scheduler_tp_qa_calculate 
		([pkgname]
		,[devicename]
		,[tp_rank]
		,[jobname]
	...
	, WIP.job_id as [jobid]
	, (select COUNT(lot_no) from [DBx].[dbo].[scheduler_tp_qa_wip] 
		where [DBx].[dbo].[scheduler_tp_qa_wip].device_name = WIP.device_name 
		and [DBx].[dbo].[scheduler_tp_qa_wip].job_name = WIP.job_name ) as [allLots]

	, (select COUNT(lot_no) from [DBx].[dbo].[scheduler_tp_qa_wip] 
		where [DBx].[dbo].[scheduler_tp_qa_wip].device_name = WIP.device_name 
		and [DBx].[dbo].[scheduler_tp_qa_wip].job_name = WIP.job_name 
		and [DBx].[dbo].[scheduler_tp_qa_wip].qa_in is not null ) as [sumlots]

	, (select COUNT(lot_no) from [DBx].[dbo].[scheduler_tp_qa_wip] 
		where [DBx].[dbo].[scheduler_tp_qa_wip].device_name = WIP.device_name 
		and [DBx].[dbo].[scheduler_tp_qa_wip].job_name = WIP.job_name 
		and [DBx].[dbo].[scheduler_tp_qa_wip].job_id in (122,316)) as [sumWipQa]

	, (select COUNT(lot_no) from [DBx].[dbo].[scheduler_tp_qa_wip] 
		where [DBx].[dbo].[scheduler_tp_qa_wip].device_name = WIP.device_name 
		and [DBx].[dbo].[scheduler_tp_qa_wip].job_name = WIP.job_name 
		and [DBx].[dbo].[scheduler_tp_qa_wip].state = 3 ) as hold
	, sum(WIP.kpcs) AS [sumkpcs] 
	, sum(WIP.standare_time) as [standardtime]
	, WIP.pkg_id
	FROM [DBx].[dbo].[scheduler_tp_qa_wip] as WIP
...
SELECT [APCSProDB].[mc].[machines].name AS mc_name
		...
		delete from DBx.dbo.scheduler_tp_qa_calculate where DBx.dbo.scheduler_tp_qa_calculate.pkgname in (SELECT * from STRING_SPLIT ( @Pk_name , ',' ))  
		...
		, (select COUNT(lot_no) from [DBx].[dbo].[scheduler_tp_qa_wip] 
			where [DBx].[dbo].[scheduler_tp_qa_wip].device_name = WIP.device_name 
			and [DBx].[dbo].[scheduler_tp_qa_wip].job_name = WIP.job_name ) as [sumlots] --sum lot on rack
