...
-- =============================================
CREATE PROCEDURE [dbo].[tg_sp_set_data_label_history_V.3]
...
BEGIN
	...
	DECLARE @EmpnoId int = CAST(@OPNo As int)
	SELECT @GetEmpno = emp_num FROM [APCSProDB].[man].[users] WHERE [id] = @EmpnoId
	------------------------------ End Get EmpnoId #Modify : 2024/12/26 --------------------------------

	--add check condition empno in process value date modify : 2022/02/25 time : 11.15
	SELECT @op_no_len = CASE WHEN @process_name = 'OGI' then @emp_no_val else @GetEmpno end  --new
	...
	BEGIN
	  	...
	  	SELECT @op_no_len_value = FORMAT(CAST(@op_no_len AS INT), '000000')
	...
	SELECT GETDATE()
		...
		,HOST_NAME()
		,APP_NAME()
		,'EXEC [dbo].[tg_sp_set_data_label_history_V.4_new_version_20240206] @Lotno = ''' + @Lotno + ''', @Process = ''' + @process_name + ''', @Qc_Instruction = ''' + @Tomson_Mark_3 + ''', @emp_no_at_ogi = ''' + @emp_no_val + ''', @qty_shipment = ''' + CONVERT(VARCHAR(6),@qty_shipment) + ''''
		,@Lotno
