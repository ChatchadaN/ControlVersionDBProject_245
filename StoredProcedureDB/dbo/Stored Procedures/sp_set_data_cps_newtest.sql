...
-- =============================================
CREATE PROCEDURE [dbo].[sp_set_data_cps_newtest]
...
BEGIN
	...
	INSERT INTO [StoredProcedureDB].[dbo].[exec_sp_history]
		...
		SELECT GETDATE()
			...
			, 'EXEC [dbo].[sp_set_data_cps] @lotno = ''' + @lotno + ''',@is_PcRequest = ''' + CAST(@pc_instr_value as char(5)) + ''',@is_Incoming = ''' + CAST(@is_incoming as char(5)) + '''' 
			, @lotno
	...
	BEGIN TRY
		...
		BEGIN
			...
			select 
				...
				left join APCSDB.dbo.LCQW_UNION_WORK_DENPYO_PRINT as den_pyo on lot.lot_no = den_pyo.LOT_NO_4
				--left join StoredProcedureDB.dbo.IS_CPS_LICENSE as cps_lice on pk.short_name = cps_lice.TYPEX4
				where label_rec.lot_no = @lotno
				--and label_rec.type_of_label = 21  --close 2022/15 time : 15.54
				and label_rec.type_of_label in (21,5) --new condition 2022/15 time : 15.54
			...
			BEGIN
			  ...
			  ,[TR_FLG]
				...
				select 
				 @lotno + RIGHT('000'+ CONVERT(VARCHAR,ROW_NUMBER() OVER(ORDER BY label_rec.type_of_label DESC)),3)
				,'001' 
				,CONVERT(varchar,GETDATE(),112)  
				,'10' 
				,'' 
				...
				left join APCSDB.dbo.LCQW_UNION_WORK_DENPYO_PRINT as den_pyo on lot.lot_no = den_pyo.LOT_NO_4
				where label_rec.lot_no = @lotno
				and label_rec.type_of_label in (21,5)
				order by label_rec.type_of_label desc
			END
			ELSE
			BEGIN
				INSERT INTO APCSProDB.trans.mli02_lsi(
