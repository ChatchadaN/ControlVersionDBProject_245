...
-- =============================================
CREATE PROCEDURE [dbo].[tg_sp_disable_reel_ng_test_auto_shipment]
...
BEGIN
	...
	SELECT GETDATE()
		...
		,HOST_NAME()
		,APP_NAME()
		,'EXEC [dbo].[tg_sp_disable_reel_ng_test_auto_shipment] @lot_no = ''' + @lotno_standard + ''' @Reel_no = ''' + CAST(@reel_no as varchar(20)) + ''' @qty_input = ''' + CAST(@qty_input as varchar(7)) + ''' @qty_good = ''' + CAST(@qty_good as varchar(7)) + ''' @empno = ''' + @emp_no + ''' @count_reel_ng = ''' + CAST(@count_reel_ng as varchar(7)) + ''''
		,@lotno_standard
	...
	BEGIN
		BEGIN TRY
			...
			SET qty_out = @qty_shipment
				...
				,wip_state = case when @Count_reel_good_all = 0 then 70 else wip_state end
			WHERE lot_no = @lotno_standard
