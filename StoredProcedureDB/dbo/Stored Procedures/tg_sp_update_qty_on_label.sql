...
-- =============================================
CREATE PROCEDURE [dbo].[tg_sp_update_qty_on_label] 
	-- Add the parameters for the stored procedure here
	 @lot_no varchar(10) = ''
	,@qty int = 0  --จำนวนงานทั้งหมด
	,@emp_no varchar(6) = ''
	--@reel_standard int = 0
AS
BEGIN
	...
	BEGIN
		...
		set 
			...
			,qty_out = ((@reel_standard) * ((@qty)/(@reel_standard))) 
		where lot_no = @lot_no

		select @qty_hasuu = (@qty % @reel_standard)
	...
	SELECT GETDATE()
		...
		,HOST_NAME()
		,APP_NAME()
		,'EXEC [dbo].[tg_sp_update_qty_on_label]  @lot_no = ''' + @lot_no + ''',@qtyAll = ''' + CONVERT (varchar (10), @qty) + ''',@Qty_Hasuu_Update = ''' + CONVERT (varchar (10), @qty_hasuu) + ''',@OpNo = ''' + @emp_no ''''
		,@lot_no
