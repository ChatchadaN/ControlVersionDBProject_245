...
-- =============================================
CREATE PROCEDURE [dbo].[tg_sp_disable_reel_ng_ver_001]
...
BEGIN
	...
	SELECT GETDATE()
		...
		,HOST_NAME()
		,APP_NAME()
		,'EXEC [dbo].[tg_sp_disable_reel_ng_ver_001] @lot_no = ''' + @lotno_standard + ''' @Reel_no = ''' + CAST(@reel_no as varchar(20)) + ''' @qty_input = ''' + CAST(@qty_input as varchar(7)) + ''' @qty_good = ''' + CAST(@qty_good as varchar(7)) + ''' @empno = ''' + @emp_no + ''' @count_reel_ng = ''' + CAST(@count_reel_ng as varchar(7)) + ''''
		,@lotno_standard
	...
	select @Barcode_buttom_hasuu = case when LEN(CAST(@qty_hasuu_in_label as varchar(6))) = 5 then '0' + CAST(@qty_hasuu_in_label as varchar(6)) + ' ' + Cast(SUBSTRING(@lotno_standard, 1, 4) + ' ' + SUBSTRING(@lotno_standard, 5, 6) as char(11))
		...
		else CAST(@qty_hasuu_in_label as varchar(6)) + ' ' + Cast(SUBSTRING(@lotno_standard, 1, 4) + ' ' + SUBSTRING(@lotno_standard, 5, 6) as char(11)) end
		+ '''';
	...
	BEGIN
		BEGIN TRY
			...
			SELECT GETDATE()
				...
				,HOST_NAME()
				,APP_NAME()
				,'EXEC [dbo].[tg_sp_disable_reel_ng_check_carrier] @lot_no = ''' + @lotno_standard + ''' @Reel_no = ''' + CAST(@reel_no as varchar(20)) + ''' @qty_input = ''' + CAST(@qty_input as varchar(7)) + ''' @qty_good = ''' + CAST(@qty_good as varchar(7)) + ''' @empno = ''' + @emp_no + ''' @count_reel_ng = ''' + CAST(@count_reel_ng as varchar(7)) + ''' @carrier_no = ''' + @Carrier + ''''
				,@lotno_standard
