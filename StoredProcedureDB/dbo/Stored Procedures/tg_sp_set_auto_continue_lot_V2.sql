...
-- =============================================
CREATE PROCEDURE [dbo].[tg_sp_set_auto_continue_lot_V2]
...
BEGIN
	...
	--Check if the data is in the table label_issue_records or not
	--SET @Check_lot_label_issue = (select lot_no 
 --   from APCSProDB.trans.label_issue_records where lot_no = @hasuu_lot
	--and type_of_label = 1)

	--Check Condition mix is type lot hasuu d-lot [Add condtion Date : 2021/06/17]
	--IF @Lot_Type_Hasuu = 'D'
	--	BEGIN
	--		IF @Check_lot_label_issue != ''
	--		BEGIN
	--			select @Check_Lot_Hasuu = lot_no
	--			,@Tomson_Mark_3_lot_hasuu = tomson_3
	--			from APCSProDB.trans.label_issue_records where lot_no = @hasuu_lot
	--			and type_of_label = 1

	--			SET @Rank_hasuu = ( select rank from APCSProDB.trans.lots as lot
	...
	select @Check_Lot_Hasuu = LotNo ,@Tomson_Mark_3_lot_hasuu = Tomson3
	,@Rank_hasuu = Rank 
	from [APCSProDB].[method].[allocat] where LotNo = @hasuu_lot
	...
	SELECT GETDATE()
		...
		,HOST_NAME()
		,APP_NAME()
		,'EXEC [dbo].[tg_sp_set_auto_continue_lot_V2] @lotno_standard = ''' + @Lot_No + ''', @hasuu_lot = ''' + @hasuu_lot + ''', @hasuu_qty = ''' + CONVERT (varchar (10), @hasuu_qty) + ''', @lotno_standard_qty = '''+ CONVERT (varchar (10), @lotno_standard_qty) + ''', @empno = ''' + @empno + ''',@MNo_Hasuu = ''' + @MNo_Hasuu + ''',@package_loths = ''' + @package_loths + ''',@device_loths = ''' + @device_loths + ''''
		...
		BEGIN
			...
			IF @count_lotid_fristlot = 0
				BEGIN
					...
					--update qty_pass in tranlot by FristLot
					UPDATE [APCSProDB].[trans].[lots]
					SET 
						  [qty_pass] = @lotno_standard_qty
					WHERE [lot_no] = @lotno_standard
