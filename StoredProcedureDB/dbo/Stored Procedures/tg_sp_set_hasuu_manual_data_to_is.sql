...
-- Description:	<Description,Support Hasuu InVENToRY To IS,>
-- =============================================
CREATE PROCEDURE [dbo].[tg_sp_set_hasuu_manual_data_to_is]
...
BEGIN
	...
	,@Out_Out_Flag = case when sur.pdcd = 'QI001' then 'C' 
					 else 'B' end
	,@empno = sur.created_by
	...
	select @EmpNo_Char = CONVERT(char(5),@empno); --update 2021/03/06
	...
	, [lot_no])
		...
		,HOST_NAME()
		,APP_NAME()
		,'EXEC [tg_sp_set_hasuu_manual_data_to_is] Get Store @empno = ''' + CAST(@empno as varchar(7)) + ''',@lotno = ''' + @newlotno + ''',@QtyPass = ''' + CAST(@total_pcs as varchar(7)) + ''''
		,@newlotno
