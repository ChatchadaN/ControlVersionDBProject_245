...
-- =============================================
CREATE PROCEDURE [dbo].[tg_sp_set_hasuu_lot_new_manual_data_to_Is_IF]
...
BEGIN
	...
	--		else SUBSTRING(dn.ft_name, 1, CHARINDEX('-', dn.ft_name)-1) end  --close 2022/01/22 Time : 22.25
	,@R_Fukuoka_Model_Name = REVERSE(SUBSTRING(REVERSE(dn.name), CHARINDEX('-',  REVERSE(dn.name)) + 1,LEN(dn.name))) 
	--LEFT(
	--	trim(dn.name)
	--	,IIF(charindex('-',trim(dn.name)) > 0 
	--		,len(trim(dn.name)) - 
	--			(len(right(trim(dn.name), len(trim(dn.name))-charindex('-',trim(dn.name)))) - 
	--			charindex('-',right(trim(dn.name), len(trim(dn.name))-charindex('-',trim(dn.name)))) + 1)
	--		,len(trim(dn.name))
	--	)
	--)  
	...
	, [lot_no])
		...
		,HOST_NAME()
		,APP_NAME()
		,'EXEC [tg_sp_set_hasuu_lot_new_manual_data_to_Is_IF] Get Store @empno = ''' + CAST(@EmpNo_int as varchar(7)) + ''',@lotno = ''' + @newlotno + ''',@QtyPass = ''' + CAST(@total_pcs as varchar(7)) + ''''
		,@newlotno
	...
	BEGIN CATCH
		...
		, [lot_no])
			...
			,HOST_NAME()
			,APP_NAME()
			,'EXEC [tg_sp_set_hasuu_lot_new_manual_data_to_Is_IF] Set data store Error @empno = ''' + CAST(@EmpNo_int as varchar(7)) + ''',@lotno = ''' + @newlotno + ''',@QtyPass = ''' + CAST(@total_pcs as varchar(7)) + ''''
			,@newlotno
