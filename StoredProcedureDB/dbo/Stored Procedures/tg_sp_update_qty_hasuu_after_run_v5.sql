...
-- =============================================
CREATE PROCEDURE [dbo].[tg_sp_update_qty_hasuu_after_run_v5] --ตัวจริงต้อง update เป็น V.5
...
BEGIN
	...
	select @InStock_Current = in_stock
		, @pcs_current = pcs 
	from APCSProDB.trans.surpluses where serial_no = @standard_lot
	...
	SELECT GETDATE()
		...
		,HOST_NAME()
		,APP_NAME()
		,'EXEC [dbo].[tg_sp_update_qty_hasuu_after_run_V5 update qty after lot end] @lotno = ''' + @standard_lot 
	...
	BEGIN
		BEGIN TRY  
			...
			BEGIN
				IF(@is_process = 'INSP')
				Begin
					select @qty_insp = (select (COUNT(@reel_no) * @pcs_per_pack) + IIF(@InStock_Current = 2,@pcs_current,0) - @qty_sum_ng FROM STRING_SPLIT(@reel_no, ','))
					select @qty_hasuu_now = @qty_insp
				end
				UPDATE [APCSProDB].[trans].[surpluses]
				SET 
					  [pcs] = IIF(@is_process = 'INSP',@qty_insp,@qty_hasuu_now)--@qty_hasuu_now
					, [in_stock] = IIF(@qty_hasuu_now != 0,2,0)
					, [updated_at] = GETDATE()
				...
				SELECT GETDATE()
					...
					,HOST_NAME()
					,APP_NAME()
					,'EXEC [dbo].[tg_sp_update_qty_hasuu_after_run_V5 update hasuu now in surpluses and qty_hasuu_now == 0] @lotno = ''' + @standard_lot 
						+ ''',@qty_hasuu_before = ''' + CONVERT (varchar (10), @qty_hasuu_before) 
						+ ''',@qty_hasuu_now = ''' + CONVERT (varchar (10), @qty_hasuu_now) 
						+ ''' ,@qty_pass_now = ''' + CONVERT (varchar (10), @qty_pass_now) 
						+ ''' ,@qty_shipment_now = ''' + CONVERT (varchar (10), @qty_shipment_now) 
		...
		BEGIN TRY
			...
			EXEC [dbo].[tg_sp_set_qty_fristlot_update_ver3] 
			 @lot_no = @standard_lot
			,@qty = @qty_sum
			,@is_process = @is_process
			,@reel_no = @reel_no
	...
	BEGIN
		...
		SELECT GETDATE()
			...
			,HOST_NAME()
			,APP_NAME()
			,'EXEC [dbo].[tg_sp_update_qty_hasuu_after_run_V5 update flag is_test_fttp on TP] @lotno = ''' + @standard_lot 
				+ ''' ,@qty_hasuu_before = ''' + CONVERT (varchar (10), @qty_hasuu_before) 
				+ ''' ,@qty_hasuu_now = ''' + CONVERT (varchar (10), @qty_hasuu_now) 
				+ ''' ,@qty_pass_now = ''' + CONVERT (varchar (10), @qty_pass_now) 
				+ ''' ,@qty_shipment_now = ''' + CONVERT (varchar (10), @qty_shipment_now) 
			...
			,@standard_lot
	END
	print 'tg_sp_update_qty_hasuu_after_run_V5'
END
