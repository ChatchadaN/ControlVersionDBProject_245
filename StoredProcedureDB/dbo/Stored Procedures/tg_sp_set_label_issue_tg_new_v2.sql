...
-- =============================================
CREATE PROCEDURE [dbo].[tg_sp_set_label_issue_tg_new_v2]
...
BEGIN
	...
	DECLARE @datestart VARCHAR(50) = CAST(GETDATE() AS DATE) 
	...
	SELECT GETDATE()
		...
		, HOST_NAME()
		, APP_NAME()
		, 'EXEC [dbo].[tg_sp_set_label_issue_tg] @empno = ''' + @empno 
			+ ''',@lotno_standard = ''' + @lotno_standard 
			+ ''',@Total = ''' + CAST(@QtyPass_Standard as varchar(7)) + ''''
		, @lotno_standard

	----# 1
	BEGIN TRANSACTION
	BEGIN TRY
		--insrt into table MIX_HIST to DB-IS
	...
	BEGIN TRY
		EXEC [StoredProcedureDB].[atom].[sp_set_tsugitashi_tg] @master_lot_no = @lotno_standard
			, @hasuu_lot_no = ''
	...
	BEGIN CATCH 
		SELECT 'FALSE' AS [Status] 
			, 'INSERT DATA LOT_COMBINE ERROR !!' AS [Error_Message_ENG]
			, N'บันทึกข้อมูลผิดพลาด !!' AS [Error_Message_THA] 
			, N' กรุณาติดต่อ System' AS [Handling];
		RETURN;
	END CATCH
	...
	BEGIN TRY
		...
		EXEC [StoredProcedureDB].[trans].[sp_set_record_class_lot_process_records] @lot_no = @lotno_standard
			, @opno = @empno
			, @record_class = 46
			, @mcno = @machine_name
	END TRY
