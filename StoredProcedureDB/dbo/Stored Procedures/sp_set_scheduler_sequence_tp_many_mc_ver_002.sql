...
-- =============================================
CREATE PROCEDURE [dbo].[sp_set_scheduler_sequence_tp_many_mc_ver_002]
...
SET @PackageName += 'SSOP-A32,SSOP-B40,TO263-3,TO263-5,TO263-7,HTSSOP-A44,HTSSOP-A44R,HTSSOP-B54,HTSSOP-B54R,TO252-J3,SSOP-A20,SSOP-A24,HSOP-M36,SOP20,SOP22,SOP24,TO263-3F,TO263-5F,SOT223-4,SOT223-4F,TSSOP-C48V,TO263-7L,HTSSOPB20E,TO252-5,HTSSOPC48E,HTSSOP-C64A,TSSOP-B30,';
SET @PackageName += 'TO252S-7+,SSOP-A54_23,SSOP-A54_36,SSOP-A54_42,SSOP-A26_20,SSOP-B24,SSOP-B28,SSOP-B30,SSOP-A32,SSOP-B40,SOP-JW8,HSSOP-C16,HTSSOP-B20,HTSSOP-C48R,HTSSOP-B40,SSOP-C38W,TO252-3,TO252-5,';
...
-----------------Get individual WIP include QA and TP Job-----------------------------------------------------------/
	...
	SELECT [APCSProDB].[mc].[machines].name AS mc_name
		...
		and [APCSProDB].[trans].[lots].act_job_id in (222,231,236,289,197,291,122,316,353,397,401,409) 
		...
		where lots.wip_state = 20 
		and [APCSProDB].[method].[packages].name in (SELECT * from STRING_SPLIT ( @PackageName , ',' ))    
		and lotspecial.job_id in(222,231,236,289,197,291,122,316,353,397,401,409) 
...
select result.name as pkgname, result.Devicename as devicename , input.Kpcs as input , result.Kpcs as [output], result.Kpcs-input.Kpcs as summary
	from(select pk.name, device.name as Devicename,SUM( lots.qty_in)  as Kpcs
		...
		where lot_record.record_class = 2  and job_id in (222,231,236,289,397,401,409)  and PK.name in (SELECT * from STRING_SPLIT ( @PackageName , ',' )) --กำหนดแพ็คเก็ทกรุ๊ป
...
seq_no bigint, package_name varchar(30),lot_end datetime,lot_start datetime
		...
		where lot.act_job_id  in (236,289,231,397,401,222,409) and lot.process_state IN (2,102)  and lot.wip_state= 20 and lot_record.record_class in (1,5) and lot.is_special_flow = 0
		...
		from  [APCSProDB].trans.lot_process_records as lot_record with (NOLOCK)
			...
			inner join [APCSProDB].[method].[device_flows] as deviceflow with (NOLOCK) on deviceflow.device_slip_id = lot.device_slip_id and deviceflow.step_no = lot.step_no
		where lotspecial.job_id  in (222,236,289,231,397,401,409) and special.process_state IN (2,102)  and lot.wip_state = 20 and lot_record.record_class in (1,5) and lot.is_special_flow = 1 
			and pk.name in (SELECT * from STRING_SPLIT ( @PackageName , ',' )) 
		group by  lot.lot_no,lot.process_state,mc.name,lot.id,device.name,device.ft_name
			,deviceflow.process_minutes,lot.is_special_flow,device.official_number,lot.qty_in,job.name,pk.name;
...
--------------FIND WIP TP ONLY FOR GROUPING DEVICE AND MACHINE TOGETHER----------------------------------------------------------------------
		...
		2 as Seq_no,Package.name as package_name,NULL as lot_end,NULL as lot_start,deivce.name as deviceName,Machine_table.id as mcid INTO #MachineLockSeq
			...
			where lots.act_job_id  in (222,236,289,231,397,401,409) and ((lots.process_state in (0,100) and lots.location_id is not null) or (lots.process_state in (1,101)))
	...
	, Is_GDIC int,wip_state tinyint,quality_state tinyint,FT_lots int
		...
		(
			...
			(
				...
				LEFT JOIN #WipBeforeTP as LotFTBeforeTP on LotFTBeforeTP.Device = temp.device_name
				...
				WHERE job_id in (222,231,236,289,397,401,409) and pkg_name in (SELECT * from STRING_SPLIT ( @PackageName , ',' ))
...
----------------------------------------------------------------------------------------------
		...
		Seq_no + ISNULL((select 1 from #MachineLockSeq where #MachineLockSeq.machine_name = B.machine_name AND B.dublicate_machineCanRun = 1),0) as Seq_no
...
seq_no bigint, package_name varchar(30),lot_end datetime,lot_start datetime,priority_num int,applyPriorityGroup int
		...
		) as Result ORDER BY machine_name,Result.seq_no
