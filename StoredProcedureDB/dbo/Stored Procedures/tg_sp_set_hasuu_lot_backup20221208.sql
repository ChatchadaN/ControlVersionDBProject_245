...
-- =============================================
Create PROCEDURE [dbo].[tg_sp_set_hasuu_lot_backup20221208]
...
BEGIN
	...
	--call new store create 2022/12/07 Time : 14.40
	------------------------------------------------------------------------------------------------
	...
	, [lot_no])
		...
		,HOST_NAME()
		,APP_NAME()
		,'EXEC [tg_sp_set_hasuu_lot] Get Store @empno = ''' + CAST(@EmpNo_int as varchar(7)) + ''',@lotno = ''' + @newlotno + ''',@QtyPass = ''' + CAST(@total_pcs as varchar(7)) + ''''
		,@newlotno
	...
	BEGIN TRY
		...
		BEGIN TRY
				...
				, [lot_no])
					...
					,HOST_NAME()
					,APP_NAME()
					,'EXEC [tg_sp_set_hasuu_lot] Set data to APCSPro @empno = ''' + CAST(@EmpNo_int as varchar(7)) + ''',@lotno = ''' + @newlotno + ''',@QtyPass = ''' + CAST(@total_pcs as varchar(7)) + ''''
					,@newlotno
