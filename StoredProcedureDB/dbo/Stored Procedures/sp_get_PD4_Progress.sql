...
-- =============================================
CREATE PROCEDURE [dbo].[sp_get_PD4_Progress]
...
BEGIN
					...
					IF (@JOB = 'FL')
							BEGIN
									...
									WHERE
											...
											OR [Package] IN ('TO263-9','TO263-9Y','TO252-3X','TO252-5X','SOT223-4Y')
											OR [Package] IN ('SIP9','SOP24-HF','TO252S-7','TO263-3X','TO263-5X','TO263-7Y') -- 2025-06-11 (JRC Request)
							END
					...
					ELSE IF (@JOB = 'FT')
							BEGIN
									...
									WHERE
											...
											OR [Package] IN ('TO263-3','TO263-5','TO263-7','TO252-J3','TO252-J5','TO252-3','TO252-5','TO252-3','TO252-5','TO252-3/5','TO252-3/5')
											OR [Package] IN ('SSOP-A32','SSOP-B40')
											OR [Package] IN ('TO220-6M','TO220-7M')
							END
	...
	--INNER JOIN
	--		#DVICE_SETTING AS [DVICE_SETTING]
	--ON
	--		[DVICE_SETTING].[id] = [lots].[act_device_name_id]
	...
	WHERE 
			...
			AND [item_labels1].[label_eng] <> 'Cancel' AND SUBSTRING([lots].[lot_no],1,2) IN ('22','23','24','25','26')
	...
	INSERT INTO #INPUTPLAN_DETAIL_MAX
			...
			--		(
			--				SELECT 
			--						[LotNo],[OWNER_Job],MAX([FRONT_BEFORE_EndTime]) AS [FRONT_BEFORE_EndTime_Max]
			--				FROM 
			--						#INPUTPLAN_DETAIL AS [INPUTPLAN_DETAIL] 
			--				GROUP BY 
			--						[LotNo],[OWNER_Job]
			...
			--		AND [INPUTPLAN_DETAIL_MAX].[FRONT_BEFORE_EndTime_Max] = [INPUTPLAN_DETAIL].[FRONT_BEFORE_EndTime]
			...
			BEGIN
					...
					FROM
							...
							SELECT DISTINCT 
									...
									--ROW_NUMBER() OVER(ORDER BY [LotNo] ASC) AS [No]
									--,SUBSTRING([LotNo],3,2) AS [WeekSlip]
									,CASE 
	...
	ELSE IF (@ReportType = 'CheckData')
			BEGIN
					SELECT * FROM #LOT_IN_JOB AS [LOT_IN_JOB]
			END
