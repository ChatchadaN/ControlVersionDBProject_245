...
-- =============================================
CREATE PROCEDURE [dbo].[sp_get_scheduler_ft_wip]
...
SELECT [APCSProDB].[mc].[machines].name AS MCName
	...
	--		THEN CASE 
	--			WHEN TRIM(REPLACE(REPLACE(REPLACE(jobs.name,'(',''),')',''),'SBLSYL','')) =  'OS+AUTO1' 
	--			THEN 'AUTO1'
	--		ELSE TRIM(REPLACE(REPLACE(REPLACE(jobs.name,'(',''),')',''),'SBLSYL','')) END
	--	WHEN jobs.name = 'OS+AUTO(1)' 
	--		THEN 'AUTO1'
	--	WHEN jobs.name = 'AUTO(1) RE'
	--		THEN 'AUTO1'
