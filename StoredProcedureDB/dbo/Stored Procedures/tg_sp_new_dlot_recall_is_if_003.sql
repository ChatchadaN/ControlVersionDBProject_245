...
CREATE PROCEDURE [dbo].[tg_sp_new_dlot_recall_is_if_003]
	-- Add the parameters for the stored procedure here
	@lotno VARCHAR(10), 
	@qty INT, 
	@empid INT, 
	@abnormal_id INT = NULL,
	@pdrequest CHAR(10) = ''
AS
BEGIN
	...
	SELECT GETDATE()
		...
		, HOST_NAME()
		, APP_NAME()
		, ISNULL('EXEC [atom].[tg_sp_new_dlot_recall_is_if_003] @lotno = ''' + @lotno + '''','EXEC [atom].[tg_sp_new_dlot_recall_is_if_003] @lotno = NULL')
			+ ISNULL(', @qty = ' + CAST(@qty AS VARCHAR(20)),', @qty = NULL')
			+ ISNULL(', @empid = ' + CAST(@empid AS VARCHAR(20)),', @empid = NULL')
			+ ISNULL(', @abnormal_id = ' + CAST(@abnormal_id AS VARCHAR(20)),', @abnormal_id = NULL')
			+ ISNULL(', @pdrequest = ' + CAST(@pdrequest AS VARCHAR(10)),', @pdrequest = NULL')
		, @lotno;

	---- set emp_num
	DECLARE @empnum VARCHAR(6) = (SELECT [emp_num] FROM [APCSProDB].[man].[users] WITH (NOLOCK) WHERE [id] = @empid);
	DECLARE @date DATETIME = GETDATE();
	DECLARE @abnormal_mode VARCHAR(255) = (SELECT (CASE WHEN LEN([name]) > 30 THEN SUBSTRING([name], 0, 30) ELSE [name] END) AS [name] FROM [APCSProDB].[trans].[abnormal_detail] WHERE [id] = @abnormal_id);
	...
	UPDATE @table_mix_hist
	SET [WFLotNo] = (SELECT [WFLotNo] FROM @table_mix_hist WHERE [HASUU_LotNo] != [LotNo])
	...
	SELECT [HASUU_LotNo] AS [LotNo]
	  ...
	  , IIF([data].[QTY] < [data].[Packing_Standerd_QTY],[data].[QTY],(([data].[Packing_Standerd_QTY]) * ([data].[QTY]/[data].[Packing_Standerd_QTY]))) AS [Standerd_QTY]
	  ...
	  , IIF([data].[QTY] < [data].[Packing_Standerd_QTY],[data].[QTY],(([data].[Packing_Standerd_QTY]) * ([data].[QTY]/[data].[Packing_Standerd_QTY]))) AS [Shipment_QTY]
	...
	SELECT [Stock_Class]
	  ...
	  , IIF([Good_Product_QTY] < [Packing_Standerd_QTY],0,ISNULL([Good_Product_QTY]%NULLIF([Packing_Standerd_QTY],0),0)) AS [HASU_Stock_QTY]
      , IIF([Good_Product_QTY] < [Packing_Standerd_QTY],[Good_Product_QTY],[Standerd_QTY]) AS [HASU_WIP_QTY]
	...
	SELECT [data].[LotNo] AS [LOTNO]
		...
		, IIF([data].[QTY] < [data].[Packing_Standerd_QTY],0,(([data].[Packing_Standerd_QTY]) * ([data].[QTY]/NULLIF([data].[Packing_Standerd_QTY],0)))) AS [STDQTY]
		, IIF([data].[QTY] < [data].[Packing_Standerd_QTY],[data].[QTY],([data].[QTY]%(([data].[Packing_Standerd_QTY]) * ([data].[QTY]/NULLIF([data].[Packing_Standerd_QTY],0))))) AS [HASUUQTY]
		, 1 AS [FLAG]
		...
		, IIF([data].[QTY] < [data].[Packing_Standerd_QTY],0,(([data].[Packing_Standerd_QTY]) * ([data].[QTY]/NULLIF([data].[Packing_Standerd_QTY],0)))) AS [FINAL_STD_QTY]
		, IIF([data].[QTY] < [data].[Packing_Standerd_QTY],[data].[QTY],([data].[QTY]%(([data].[Packing_Standerd_QTY]) * ([data].[QTY]/NULLIF([data].[Packing_Standerd_QTY],0))))) AS [FINAL_HASUU_QTY]
