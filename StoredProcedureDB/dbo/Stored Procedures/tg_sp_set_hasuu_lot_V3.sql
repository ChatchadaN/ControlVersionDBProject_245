...
-- Edit Data : 2021/09/03 Time : 15.40 By Aomsin 
-- Description:	<Description,,>
-- =============================================
CREATE PROCEDURE [dbo].[tg_sp_set_hasuu_lot_V3]
...
BEGIN
	...
	--Create log Date : 2022/01/19 Time : 14.16
	...
	, [lot_no])
		...
		,HOST_NAME()
		,APP_NAME()
		,'EXEC [tg_sp_set_hasuu_lot_V3] Get Store @empno = ''' + CAST(@EmpNo_int as varchar(7)) + ''',@lotno = ''' + @newlotno + ''',@QtyPass = ''' + CAST(@total_pcs as varchar(7)) + ''''
		,@newlotno
		 ...
		 BEGIN TRY
				...
				,@emp_no_int = @EmpNo_int --update 2021/12/07 time : 11.57
				...
				, [lot_no])
					...
					,HOST_NAME()
					,APP_NAME()
					,'EXEC [tg_sp_set_hasuu_lot_V3] Set data to APCSPro @empno = ''' + @empno + ''',@lotno = ''' + @newlotno + ''',@QtyPass = ''' + CAST(@total_pcs as varchar(7)) + ''''
					,@newlotno
		...
		BEGIN CATCH
			...
			INSERT INTO [StoredProcedureDB].[dbo].[exec_sp_history]
				...
				SELECT GETDATE()
					...
					,HOST_NAME()
					,APP_NAME()
					,'EXEC [tg_sp_set_hasuu_lot_V3] Error Mix @empno = ''' + @empno + ''',@lotno = ''' + @newlotno + ''''
					,@newlotno
