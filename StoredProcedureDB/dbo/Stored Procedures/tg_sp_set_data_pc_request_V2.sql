...
-- =============================================
CREATE PROCEDURE [dbo].[tg_sp_set_data_pc_request_V2] 
...
BEGIN
   ...
   INSERT INTO [StoredProcedureDB].[dbo].[exec_sp_history]
	...
	SELECT GETDATE()
		...
		,HOST_NAME()
		,APP_NAME()
		,'EXEC [dbo].[tg_sp_set_data_pc_request_V2] @empno = ''' + @empno + ''',@lotno_standard = ''' + @newlot + ''',@lotno_standard_qty = ''' + CONVERT (varchar (10), @new_qty) + ''',@pdcd_adjust = ''' + @pdcd_Adjust + ''',@out_out_flag_adjust = ''' + @out_out_flag + ''''
		,@newlot
	...
	--insert data tabel mixhist to db-is
			...
			SELECT 
				...
				--	else SUBSTRING(dn.ft_name, 1, CHARINDEX('-', dn.ft_name)-1) end --close 20222/01/21 Time : 22.30
				--,LEFT(
				--	trim(dn.name)
