...
-- =============================================
CREATE PROCEDURE [dbo].[tg_sp_set_data_pc_request_V3_new] 
	...
	BEGIN
		...
		BEGIN CATCH
			...
			SELECT 
				...
				,HOST_NAME()
				,APP_NAME()
				,'EXEC [dbo].[tg_sp_set_data_pc_request_V3_new_version update hasuu qty in label error !!] @empno = ''' + @empno + ''',@lotno_standard = ''' + @newlot + ''',@lotno_standard_qty = ''' + CONVERT (varchar (10), @new_qty) + ''',@pdcd_adjust = ''' + @pdcd_Adjust + ''',@out_out_flag_adjust = ''' + @out_out_flag + ''''
				,@newlot
		END CATCH
		...
		BEGIN CATCH
			...
			SELECT 
				...
				,HOST_NAME()
				,APP_NAME()
				,'EXEC [dbo].[tg_sp_set_data_pc_request_V3_new_version update hasuu qty in tran.lot error !!] @empno = ''' + @empno + ''',@lotno_standard = ''' + @newlot + ''',@lotno_standard_qty = ''' + CONVERT (varchar (10), @new_qty) + ''',@pdcd_adjust = ''' + @pdcd_Adjust + ''',@out_out_flag_adjust = ''' + @out_out_flag + ''''
				,@newlot
		END CATCH
	END
	ELSE
