...
-- =============================================
CREATE PROCEDURE [dbo].[sp_set_scheduler_sequence_gdic_ver_temp] 
...
BEGIN Try
	...
	SET @PackageName = 'SSOP-B20W,SSOP-B28W,SSOP-B10W,SOP-JW8,SSOP-C38W,SSOP-B20WA,SSOPB20WR1,SSOPB28WR6,SSOPB30W19,SSOP-C26W'
	...
	DELETE FROM DBxDW.dbo.[scheduler_temp_01]
	...
	SELECT DISTINCT [FTSetupReport].[MCNo], Mc.id as McId, Rate.oprate, Rate.setupid, LotNo, PackageName, DeviceName
			...
			,(select top 1 (SELECT DATEADD(MINUTE, (deviceflow.process_minutes*(CAST(lot.qty_in AS float)/CAST(device.official_number AS float))), max(lot_record.recorded_at))) AS ENDTIME
				...
				from  [APCSProDB].trans.lot_process_records as lot_record with (NOLOCK)
				  ...
				  where lot.act_job_id  in (106,108,110,119,263,359,361,362,363,364,155,423)  and lot.process_state != 0  and lot.wip_state= 20 and lot_record.record_class in (1,5) and lot.is_special_flow = 0
	...
	(
		...
		, MCName Varchar(30)
		, Prioritys int
		, DeviceName_change varchar(30)
		, DeviceName_Now varchar(30)
		, DateSet Datetime
		, JobName_Af Varchar(20)
	)
	...
	(
		...
		, mc_id int
		, flow_before Varchar(MAX)
		, flow_after Varchar(MAX)
	)
	...
	DELETE DBxDW.dbo.[scheduler_temp_01] where lot_no COLLATE SQL_Latin1_General_CP1_CI_AS IN (select MCName from #AlreadyUpdateGdic)
	...
	DELETE DBxDW.dbo.[scheduler_temp_01] where lot_no COLLATE SQL_Latin1_General_CP1_CI_AS IN (select lot_no from #scheduler_temp_TC)

	insert into DBxDW.dbo.[scheduler_temp_01] select * from #scheduler_temp_TC
	...
	from  [APCSProDB].trans.lot_process_records as lot_record with (NOLOCK)
	  ...
	  inner join [APCSProDB] .[method].device_names as device with (NOLOCK) on lot.act_device_name_id = device.id 
	  inner join [APCSProDB].[method].[device_flows] as deviceflow with (NOLOCK) on deviceflow.device_slip_id = lot.device_slip_id and deviceflow.step_no = lot.step_no
	where lot.act_job_id  in (106,108,110,119,263,359,361,362,363,364,155,423,422,426)  and lot.process_state in ( 2,102)  and lot.wip_state= 20 and lot_record.record_class in (1,5) and lot.is_special_flow = 0
	...
	where lotspecial.job_id  in (106,108,110,155,119,378,263,385,359,361,362,363,364,423,422,424,426) and special.process_state  in ( 2,102)  and lot.wip_state = 20 and lot_record.record_class in (1,5) and lot.is_special_flow = 1 
	...
	INSERT INTO [DBxDW].[dbo].[scheduler_temp_01] SELECT LotNo,JobName,FTDeviceName,'','',MCName,1,Package,EndLot,StartLot FROM #getLotInMC

	-----------------------------------------------------------------------------------------------------------------------------------------------	
	--Get MC with Flow
	CREATE TABLE #MachineFlowMapTable
	(
		MCName varchar(50),
		Flow varchar(50)
	)

	INSERT INTO #MachineFlowMapTable  
	...
	CREATE TABLE #LockNextLots_gdic
	(
		lot_no varchar(10),
		JobName varchar(50),
	...
	where ((lots.process_state in (0,100) and lots.location_id is not null) or (lots.process_state in (1,101)))
		...
		and Machine_table.name NOT IN ('FT-IFZ-036','FT-M-022','FT-M-065','FT-M-151','FT-M-235')
		and Machine_table.name IN (
        SELECT MCName 
        FROM #MachineFlowMapTable 
        WHERE Flow = CASE 
					...
					WHEN Job.name = 'OS+AUTO(1) HV' THEN 'AUTO1 HV'
	...
	CREATE TABLE #CountMc_gdic
	(
		lot_no Varchar(20)
		, machine_count INT	
	)

	INSERT INTO #CountMc_gdic
	SELECT lot_no, COUNT(DISTINCT machine_name) as count_mc FROM #LockNextLots_gdic
	GROUP BY lot_no

	IF EXISTS(SELECT 1 FROM #CountMc_gdic WHERE machine_count > 1)
	BEGIN
		UPDATE machine_states
		SET next_lot_id = NULL
		FROM [APCSProDB].[trans].[machine_states]
		INNER JOIN #CountMc_gdic AS mc_count ON [machine_states].next_lot_id = (SELECT id FROM APCSProDB.trans.lots WHERE lot_no = mc_count.lot_no)
		WHERE mc_count.machine_count > 1
	END

	INSERT INTO [DBxDW].[dbo].[scheduler_temp_01] select * from #LockNextLots_gdic
	WHERE lot_no in (SELECT lot_no FROM #CountMc_gdic WHERE machine_count = 1)
	...
	(
		...
		(
			...
			and [APCSProDB].method.device_names.alias_package_group_id = 33
			and lotspecial.job_id in (106,108,110,155,119,378,263,385,359,361,362,363,364,423,422)
			and lots.is_special_flow = 1 
			and lots.process_state in(0,100,1) 
			and lots.quality_state in(0,4)
			and special.process_state in(0,100,1) 
			and special.quality_state in(0,4)
			and [APCSProDB].[method].device_names.name IN (select #tempTypeSetup.DeivceName from #tempTypeSetup)
			and CASE 
				WHEN job.name = 'OS+AUTO(1)SBLSYL' 
					THEN 'AUTO1'
	...
	and lot_no not in (SELECT lot_no FROM #LockNextLots_gdic)
	...
	SELECT 
		...
		TB_LotWip.RackAddress,
		TB_LotWip.RackName,
		mc.MCNo AS machine_name,
		--TB_LotWip.lot_seq_onMC + 1 as seq_no,

		CASE 
			WHEN EXISTS (SELECT 1 FROM #LockNextLots_gdic WHERE #LockNextLots_gdic.machine_name = mc.MCNo)
	...
	INNER JOIN (
		...
		FROM (
			SELECT 
				(SELECT DISTINCT MAX(a.Seq_MC) 
				 FROM #tempSetupMc_RANK AS a 
				 WHERE a.DeviceName = b.DeviceName 
				   AND a.TestFlow = b.JobName) AS Max_Mc,
				lot_seq,
				lot_no,
				DeviceName,
				JobName,
				...
				RackName,
				PkgName
			FROM #getWIPData_New AS b
		) AS tb
	) AS TB_LotWip 
	ON TB_LotWip.Index_point_mc = mc.Seq_MC 
	   AND TB_LotWip.DeviceName = mc.DeviceName 
	   AND TB_LotWip.JobName = TestFlow
	WHERE NOT EXISTS (SELECT 1 FROM #LockNextLots_gdic WHERE #LockNextLots_gdic.lot_no = TB_LotWip.lot_no)	
	...
	INSERT INTO [DBxDW].[dbo].[scheduler_temp_01] select distinct * from #Result
	...
	drop table #Result
	drop table #CountMc_gdic
	drop table #MachineFlowMapTable
	COMMIT;
END TRY
BEGIN CATCH
