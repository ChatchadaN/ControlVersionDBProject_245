...
-- =============================================
CREATE PROCEDURE [dbo].[tg_sp_get_reprint_label_test]
...
BEGIN
	...
	SELECT GETDATE()
		...
		,HOST_NAME()
		,APP_NAME()
		,'EXEC [dbo].[tg_sp_get_reprint_label_test] @lotno = ''' + @lotno + ''''
	...
	BEGIN
		...
		,case when lots.qty_pass < ISNULL(dn.pcs_per_pack,0) then 'USE' else 'NO USE' end as is_hasuu_stock_in
