...
-- =============================================
CREATE PROCEDURE [dbo].[tg_sp_set_data_label_history_V.2]
...
BEGIN
 ...
 --     ,[update_by] [int] NULL
	...
	SET @sql = @sql + ',re_comment,VERSION,is_logo,SEAL,create_at,create_by,update_at,update_by) values ';
	...
	BEGIN
		...
		BEGIN	
			...
			SET @sql = @sql + ',''' + case when Cast((@Tomson_box) as char(20)) is null or Cast((@Tomson_box) as char(20)) = '' then ' '
			 else Cast((@Tomson_box) as char(20)) end + '''';
			...
			SET @sql = @sql + ',''' + Cast((@QTY) as char(20)) + '''';
			SET @sql = @sql + ',''' + case when @Lotno_subType = 'D' or @Lotno_subType = 'F' then 'MX' else Cast((@Mno_Standard) as char(20)) end + '''';
			SET @sql = @sql + ',''''';
			SET @sql = @sql + ',''' + case when @Lotno_subType = 'D' or @Lotno_subType = 'F' then ' ' else  Cast((@Mno_Hasuu) as char(20)) end + '''';
			SET @sql = @sql + ',''''';
			SET @sql = @sql + ',''' + Cast((2 + @reel_num) as char(3)) + '''';
			SET @sql = @sql + ',''' + Cast(@Device as varchar(19)) + case when LEN(CAST(@QTY as varchar(6))) = 5 then '0' + CAST(@QTY as varchar(6))  + CAST(@Lotno as varchar(10)) + '00' + Cast((2 + @reel_num) as char(3))
			  when LEN(CAST(@QTY as varchar(6))) = 4 then '00' + CAST(@QTY as varchar(6))  + CAST(@Lotno as varchar(10)) + '00' + Cast((2 + @reel_num) as char(3))
			  when LEN(CAST(@QTY as varchar(6))) = 3 then '000' + CAST(@QTY as varchar(6))  + CAST(@Lotno as varchar(10)) + '00' + Cast((2 + @reel_num) as char(3))
			  when LEN(CAST(@QTY as varchar(6))) = 2 then '0000' + CAST(@QTY as varchar(6))  + CAST(@Lotno as varchar(10)) + '00' + Cast((2 + @reel_num) as char(3))
			  else CAST(@QTY as varchar(6)) + CAST(@Lotno as varchar(10)) + '00' + Cast((2 + @reel_num) as char(3)) end + '''';
			SET @sql = @sql + ',''''';
			SET @sql = @sql + ',''''';
			SET @sql = @sql + ',''''';
			SET @sql = @sql + ',''''';
			...
			SET @sql = @sql + ',''' + Cast((SELECT FORMAT(GETDATE(), 'ddMMMyy')) as varchar(10)) + ''''; --SEAL
		...
		BEGIN	
			...
			SET @sql = @sql + ',''' + Cast((@Tomson_Mark_3) as char(4)) + '''';
			SET @sql = @sql + ',''''';
			SET @sql = @sql + ',''' + case when LEN(CAST(@Hasuu_qty as varchar(6))) = 5 then '0' + CAST(@Hasuu_qty as varchar(6)) + ' ' + Cast(SUBSTRING(@Lotno, 1, 4) + ' ' + SUBSTRING(@Lotno, 5, 6) as char(11))
			  ...
			  else CAST(@Hasuu_qty as varchar(6)) + ' ' + Cast(SUBSTRING(@Lotno, 1, 4) + ' ' + SUBSTRING(@Lotno, 5, 6) as char(11)) end
			  + '''';
			SET @sql = @sql + ',''' + case when @Lotno_subType = 'D' or @Lotno_subType = 'F' then 'MX' else Cast((@Mno_Standard) as char(20)) end + '''';
			SET @sql = @sql + ',''''';
			SET @sql = @sql + ',''' + case when @Lotno_subType = 'D' or @Lotno_subType = 'F' then 'MX' else Cast((@Mno_Hasuu) as char(20)) end + '''';
			SET @sql = @sql + ',''''';
			SET @sql = @sql + ',''' + Cast((1 + @reel_num) as char(3)) + '''';
			SET @sql = @sql + ',''' + Cast(@Device as varchar(19)) + case when LEN(CAST(@Hasuu_qty as varchar(6))) = 5 then '0' + CAST(@Hasuu_qty as varchar(6))  + CAST(@Lotno as varchar(10)) + '00' + Cast((1 + @reel_num) as char(3))
			  when LEN(CAST(@Hasuu_qty as varchar(6))) = 4 then '00' + CAST(@Hasuu_qty as varchar(6))  + CAST(@Lotno as varchar(10)) + '00' + Cast((1 + @reel_num) as char(3))
			  when LEN(CAST(@Hasuu_qty as varchar(6))) = 3 then '000' + CAST(@Hasuu_qty as varchar(6))  + CAST(@Lotno as varchar(10)) + '00' + Cast((1 + @reel_num) as char(3))
			  when LEN(CAST(@Hasuu_qty as varchar(6))) = 2 then '0000' + CAST(@Hasuu_qty as varchar(6))  + CAST(@Lotno as varchar(10)) + '00' + Cast((1 + @reel_num) as char(3))
			  else CAST(@Hasuu_qty as varchar(6)) + CAST(@Lotno as varchar(10)) + '00' + Cast((1 + @reel_num) as char(20)) end + '''';
			SET @sql = @sql + ',''''';
			SET @sql = @sql + ',''''';
			SET @sql = @sql + ',''''';
			SET @sql = @sql + ',''' + Cast((@Device) as char(20)) + '''';
		...
		BEGIN	
					...
					SET @sql = @sql + ',''' + case when @Lotno_subType = 'D' or @Lotno_subType = 'F' then 'MX' else Cast((@Mno_Standard) as char(20)) end + '''';
					SET @sql = @sql + ',''''';
					SET @sql = @sql + ',''' + case when @Lotno_subType = 'D' or @Lotno_subType = 'F' then ' ' else case when @i-2 <= @convert_mno_value then Cast((@Mno_Hasuu) as char(20)) else ' ' end end + '''';
					SET @sql = @sql + ',''''';
					SET @sql = @sql + ',''' + Cast((@i-2) as char(3)) + '''';
	...
	BEGIN
		...
		WHILE (@ii <= @reel_num )
		BEGIN
				   SET @sql = @sql + '(''4'''; --type_label : DryPack
					SET @sql = @sql + ',''' + Cast((@Lotno) as char(10)) + '''';
					SET @sql = @sql + ',''' + case when Cast((@Customer_Device) as char(20)) is null or Cast((@Customer_Device) as char(20)) = '' then Cast((@Device) as char(20))
					 else Cast((@Customer_Device) as char(20)) end + '''';
					SET @sql = @sql + ',''' + Cast((@Device) as char(20)) + '''';
					...
					SET @sql = @sql + ',''' + case when @Lotno_subType = 'D' or @Lotno_subType = 'F' then 'MX' else Cast((@Mno_Standard) as char(20)) end + '''';
					SET @sql = @sql + ',''''';
					SET @sql = @sql + ',''' + case when @Lotno_subType = 'D' or @Lotno_subType = 'F' then ' ' else case when @ii <= @convert_mno_value then Cast((@Mno_Hasuu) as char(20)) else ' ' end end + '''';
					SET @sql = @sql + ',''''';
					SET @sql = @sql + ',''' + Cast((@ii) as char(3)) + '''';
					SET @sql = @sql + ',''' + Cast(@Device as varchar(19)) + case when LEN(CAST(@Standard as varchar(6))) = 5 
					  then '0' + CAST(@Standard as varchar(6))  + CAST(@Lotno as varchar(10)) + '00' + Cast((@ii) as char(20)) 
					  when LEN(CAST(@Standard as varchar(6))) = 4 then '00' + CAST(@Standard as varchar(6))  + CAST(@Lotno as varchar(10)) + '00' + Cast((@ii) as char(3)) 
					  when LEN(CAST(@Standard as varchar(6))) = 3 then '000' + CAST(@Standard as varchar(6))  + CAST(@Lotno as varchar(10)) + '00' + Cast((@ii) as char(3)) 
					  when LEN(CAST(@Standard as varchar(6))) = 2 then '0000' + CAST(@Standard as varchar(6))  + CAST(@Lotno as varchar(10)) + '00' + Cast((@ii) as char(3)) 
					  else CAST(@Standard as varchar(6)) + CAST(@Lotno as varchar(10)) + '00' + Cast((@ii) as char(3))  end + '''';
					SET @sql = @sql + ',''''';
					SET @sql = @sql + ',''''';
					SET @sql = @sql + ',''''';
					SET @sql = @sql + ',''' + Cast((@Device) as char(20)) + '''';
		...
		BEGIN
				    ...
				    SET @sql = @sql + ',(''5'''; --type_label : Tomson
					SET @sql = @sql + ',''' + Cast((@Lotno) as char(10)) + '''';
					SET @sql = @sql + ',''' + case when Cast((@Customer_Device) as char(20)) is null or Cast((@Customer_Device) as char(20)) = '' then Cast((@Device) as char(20))
					 else Cast((@Customer_Device) as char(20)) end + '''';
					SET @sql = @sql + ',''' + Cast((@Device) as char(20)) + '''';
					...
					SET @sql = @sql + ',''' + case when @Lotno_subType = 'D' or @Lotno_subType = 'F' then 'MX' else Cast((@Mno_Standard) as char(20)) end + '''';
					SET @sql = @sql + ',''''';
					SET @sql = @sql + ',''' + case when @Lotno_subType = 'D' or @Lotno_subType = 'F' then ' ' else case when @iii <= @convert_mno_value then Cast((@Mno_Hasuu) as char(20)) else ' ' end end + '''';
					SET @sql = @sql + ',''''';
					SET @sql = @sql + ',''' + Cast((@iii) as char(3)) + '''';
					SET @sql = @sql + ',''' + Cast(@Device as varchar(19)) + case when LEN(CAST(@Standard as varchar(6))) = 5 
					  then '0' + CAST(@Standard as varchar(6))  + CAST(@Lotno as varchar(10)) + '00' + Cast((@iii) as char(20)) 
					  when LEN(CAST(@Standard as varchar(6))) = 4 then '00' + CAST(@Standard as varchar(6))  + CAST(@Lotno as varchar(10)) + '00' + Cast((@iii) as char(3)) 
					  when LEN(CAST(@Standard as varchar(6))) = 3 then '000' + CAST(@Standard as varchar(6))  + CAST(@Lotno as varchar(10)) + '00' + Cast((@iii) as char(3)) 
					  when LEN(CAST(@Standard as varchar(6))) = 2 then '0000' + CAST(@Standard as varchar(6))  + CAST(@Lotno as varchar(10)) + '00' + Cast((@iii) as char(3)) 
					  else CAST(@Standard as varchar(6)) + CAST(@Lotno as varchar(10)) + '00' + Cast((@iii) as char(3))  end + '''';
					SET @sql = @sql + ',''''';
					SET @sql = @sql + ',''''';
					SET @sql = @sql + ',''''';
					SET @sql = @sql + ',''' + Cast((@Device) as char(20)) + '''';
