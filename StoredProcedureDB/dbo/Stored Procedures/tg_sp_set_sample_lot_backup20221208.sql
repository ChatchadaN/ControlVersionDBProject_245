...
-- =============================================
Create PROCEDURE [dbo].[tg_sp_set_sample_lot_backup20221208]
...
BEGIN
	...
	--call new store create 2022/12/07 Time : 15.34
	------------------------------------------------------------------------------------------------
	...
	BEGIN
		...
		,@Totalhasuu = case when @is_ajd_qty_standard_tube = 0 then (@lotno_standard_qty)%([device_names].[pcs_per_pack]) --จำนวนงาน hasuu
							else (@lotno_standard_qty)%(@is_ajd_qty_standard_tube) end
		,@Standerd_QTY = case when @is_ajd_qty_standard_tube = 0 then CAST([device_names].[pcs_per_pack] AS char(7))
							else CAST(@is_ajd_qty_standard_tube AS char(7)) end
		,@Qty_Full_Reel_All = case when @is_ajd_qty_standard_tube = 0 then ([device_names].[pcs_per_pack]) * ((@lotno_standard_qty)/([device_names].[pcs_per_pack])) -- จำนวนงานเต็ม reel ทั้งหมด
									else (@is_ajd_qty_standard_tube) * ((@lotno_standard_qty)/(@is_ajd_qty_standard_tube)) end
		,@Qty_Standard_Lsiship =  case when @is_ajd_qty_standard_tube = 0 then (([device_names].[pcs_per_pack]) * ((@lotno_standard_qty)/([device_names].[pcs_per_pack]))) 
										else ((@is_ajd_qty_standard_tube) * ((@lotno_standard_qty)/(@is_ajd_qty_standard_tube)))  end
	...
	SELECT GETDATE()
		...
		,HOST_NAME()
		,APP_NAME()
		,'EXEC [dbo].[tg_sp_set_sample_lot] @empno = ''' + @empno + ''',@lotno_standard = ''' + @lotno_standard + ''',@lotno_standard_qty = ''' + CONVERT (varchar (10), @lotno_standard_qty) + ''',@PC_Instruction_Code = ''' + CONVERT (varchar (3), @pc_inst_code_val) + ''',@is_ajd_qty_standard_tube = ''' + CONVERT (varchar (6), @is_ajd_qty_standard_tube) + ''''
		,@lotno_standard
		...
		BEGIN
			...
			SET 
				...
				, [in_stock] = '0'  --เนื่องจากเป็นงาน shipment hasuu ไปด้วย instock จึงเป็น 0
				, [location_id] = NULL
		...
		END	
			...
			IF @lotno_standard != ''
			BEGIN
				IF @pc_inst_code_val = 131
				BEGIN
					--Add Condition 2022/06/08 Time : 09.21
					UPDATE [APCSProDB].[trans].[lots]
					SET 
						 qty_hasuu = 0
						,qty_combined = 0
						,qty_out = @Total
						,pc_instruction_code = 13  --fix 13 เพราะ Support งาน Hasuu Tube Shipment(E Lot)
					WHERE lot_no = @lotno_standard
				END
				ELSE IF @pc_inst_code_val = 13 --Add Condition For Support Hasuu Reel Shipment 2022/06/17 time : 13.25
				BEGIN
					UPDATE [APCSProDB].[trans].[lots]
					SET 
						 qty_hasuu = 0
						,qty_combined = 0
						,qty_out = @lotno_standard_qty
						,pc_instruction_code = @pc_inst_code_val
					WHERE lot_no = @lotno_standard
				END
				ELSE
				BEGIN
					--update qty_combime is 0 ,support condtion pc_instruction = 11
					UPDATE [APCSProDB].[trans].[lots]
					SET 
						 qty_hasuu = case when @Total >= @pcs_per_pack_int then @Totalhasuu else @lotno_standard_qty end
						,qty_combined = 0
						,qty_out = case when @Total >= @pcs_per_pack_int then @Qty_Full_Reel_All else 0 end  
						,pc_instruction_code = case when @lotno_standard_qty = @pcs_per_pack_int then 0 else @pc_inst_code_val end  --update condition 2022/04/07 time : 13.14
					WHERE lot_no = @lotno_standard
				END
			...
			BEGIN TRY
				...
				,@process_name = 'TP'
				,@is_std_tube_adjust = @is_ajd_qty_standard_tube
			END TRY
			BEGIN CATCH 
