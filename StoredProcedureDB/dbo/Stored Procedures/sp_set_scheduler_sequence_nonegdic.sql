...
-- =============================================
CREATE PROCEDURE [dbo].[sp_set_scheduler_sequence_nonegdic]
...
BEGIN Try
	...
	SELECT distinct  device.name as DeviceName
		...
		, CASE 
			...
			WHEN job.name LIKE '%ASISAMPLE%' 
				THEN TRIM(REPLACE(REPLACE(REPLACE(job.name,'(',''),')',''),'SAMPLE',''))
	...
	WHERE lot.act_job_id  in (119,110,108,106,263,155,403,359,361,362,363,364,50,378,342,370)
	
		and lot.process_state in ( 2,102)  and lot.wip_state= 20 and lot_record.record_class in (1,5) and lot.is_special_flow = 0
	...
	where lotspecial.job_id  in  (119,110,108,106,263,155,403,359,361,362,363,364,50,378,342,370)
		and special.process_state in ( 2,102)  and lot.wip_state = 20 and lot_record.record_class in (1,5) and lot.is_special_flow = 1 
		and TRIM(pk.name) in (SELECT * from STRING_SPLIT ( @PackageName , ',' ))
	...
	INSERT INTO [DBxDW].[dbo].[scheduler_temp] select * from #LockedNextLots_nongdic
	...
	SELECT RANK() OVER(PARTITION BY DeviceName,JobName ORDER BY lot_no ASC) AS lot_Seq,* FROM
		...
		SELECT DISTINCT
				...
				, CASE 
					...
					WHEN jobs.name = 'OS+AUTO(1) HV'		THEN 'AUTO1 HV'
					WHEN jobs.name = 'AUTO(1) HV'		THEN 'AUTO1 HV'
	...
	(
		...
		------------------------------------------------------------------------------------------------------------
		where lots.wip_state = 20 and  [APCSProDB].[method].[packages].name in (SELECT * from STRING_SPLIT ( @PackageName , ',' ))  
		and lotspecial.job_id in (119,110,108,106,263,155,403,359,361,362,363,364,50,378,342,370)
		and lots.is_special_flow = 1
	...
	INSERT INTO [DBxDW].[dbo].[scheduler_temp] select * from #Result
