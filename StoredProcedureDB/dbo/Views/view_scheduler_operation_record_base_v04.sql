CREATE VIEW dbo.view_scheduler_operation_record_base_v04
AS
SELECT            TOP (100) PERCENT t1.lot_id, t1.lot_no, t1.job, CASE WHEN re.recorded_at IS NULL THEN 'MS' ELSE 'MF' END AS STATUS, format(t1.recorded_at, 'yyyy-MM-dd HH:mm:ss') AS [START TIME], CONVERT(varchar, 
                        ISNULL(format(re.recorded_at, 'yyyy-MM-dd HH:mm:ss'), '')) AS [END TIME], mc.name AS HANDLER
...
Begin DesignProperties = 
   ...
   End
End
', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'view_scheduler_operation_record_base_v04';


GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 1, @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'view_scheduler_operation_record_base_v04';
