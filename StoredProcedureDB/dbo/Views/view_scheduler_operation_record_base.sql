CREATE VIEW dbo.view_scheduler_operation_record_base
AS
...
FROM              (SELECT            l.id AS lot_id, RTRIM(l.lot_no) AS lot_no, RTRIM(p.name) AS package, RTRIM(d.name) AS device, r.record_class, r.job_id, j.name AS job, r.recorded_at
                         ...
                         WHERE             (j.id IN (106, 108, 110, 119, 92, 93, 278)) AND (l.act_package_id IN (242)) AND (r.recorded_at > DATEADD(day, - 1, GETDATE()))) AS t1 LEFT OUTER JOIN
...
Begin DesignProperties = 
   ...
   End
End
', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'view_scheduler_operation_record_base';


GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 1, @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'view_scheduler_operation_record_base';
