...
BEGIN
	...
	SET @to_date =   DATEADD(DAY, 1,   DATEADD(millisecond, -1,@to_date))
				...
				SELECT    materials.id								AS material_id 
						...
						, parent_material.barcode					AS repack
				...
				AND (materials.created_at >= @from_date  AND materials.created_at <= @to_date )
