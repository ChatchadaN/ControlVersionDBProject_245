...
-- =============================================
CREATE PROCEDURE [pbi].[sp_pbi_004_production_og] (
...
BEGIN
   ...
   ---------------------------------------------------------------------------	
    ...
    select DISTINCT
		...
		,replace(CONVERT(char(12),DATEADD(hour,FCS.hour_code,CONVERT(datetime,0)),114),':','') as 'Time1'
		,'' as 'Date2'
		,'' as 'Time2'
...
　	    ,'' as 'OperatingCategory'	
	...
	group by 
		...
		,PR.process_no
		,PR.name
	order by DF.step_no, replace(CONVERT(DATE,DY.date_value, 114),'-',''),replace(CONVERT(char(12),DATEADD(hour,FCS.hour_code,CONVERT(datetime,0)),114),':','')
RETURN 0;
