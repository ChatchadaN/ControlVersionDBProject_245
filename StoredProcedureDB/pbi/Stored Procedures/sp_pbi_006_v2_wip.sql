...
-- =============================================
CREATE PROCEDURE [pbi].[sp_pbi_006_v2_wip] (
...
　		,TEMP.OperatingCategory	
	...
	from ( select
		...
		,replace(CONVERT(char(12),DATEADD(hour,(FW.hour_code - 1),CONVERT(datetime,0)),114),':','') as 'Time1'
		,'' as 'Date2'
		,'' as 'Time2'
		,'' as 'OrderNo'
		,'' as 'LotNo'
		,DN.name as 'RohmPN'
		,0 as 'ProcessNo'
		,Case when JBPro.id is null then JBDC.id else JB.id end as 'ProcessCode'
		,Case when JBPro.name is null then JBDC.name else JB.name end as 'ProcessName'
		,'' as 'MachineNo'
　	    ,'' as 'OperatingCategory'	
		,SUM(FW.pcs) as 'ActQty1'
		,FW.lot_count as 'ActQtyLot1'
		,0 as 'ActQty2'
		,0 as 'ActQtyLot2'
		,'' as 'InOut'
		,'' as 'RunPlanstop' 
		,Case when PR.process_no is null then PRDC.process_no else PR.process_no end as 'ProcessCode2'
		,Case when PR.name is null then PRDC.name else PR.name end as 'ProcessName2'
		,'2' as 'InputLevel'
		,'' as 'Level0'
		,'' as 'MachineNo2'
	from APCSProDWH.dwh.fact_wip as FW with(nolock)
	inner join (
		select FW.day_id, MAX(FW.hour_code) as hour_code
		from APCSProDWH.dwh.fact_wip as FW with(nolock)
		group by FW.day_id) as FW_TMP
		on FW_TMP.day_id = FW.day_id and FW_TMP.hour_code = FW.hour_code
	...
	inner join APCSProDWH.dwh.dim_jobs as JB with(nolock)on JB.id = FW.job_id and JB.id >= 0
	left outer join APCSProDB.method.jobs as JBPro with(nolock)on JBPro.id = JB.id
	left outer join APCSProDWH.dwh.dim_processes as PR with(nolock)on PR.id = JBPro.process_id
	inner join APCSProDB.method.package_groups as PKG with(nolock) on PKG.id = PK.package_group_id
	cross join APCSProDB.method.jobs as JBDC with(nolock)
	inner join APCSProDB.method.processes as PRDC with(nolock)on PRDC.id = JBDC.process_id
