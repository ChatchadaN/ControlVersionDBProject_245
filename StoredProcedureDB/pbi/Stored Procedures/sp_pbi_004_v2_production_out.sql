...
-- =============================================
CREATE PROCEDURE [pbi].[sp_pbi_004_v2_production_out] (
...
　	    ,LPR.OperatingCategory	
		,case when LPR.ActQty1 is null then 0 else LPR.ActQty1 end as ActQty1
		,case when LPR.ActQtyLot1 is null then 0 else LPR.ActQtyLot1 end as ActQtyLot1
		,case when LPR.ActQty2 is null then 0 else LPR.ActQty2 end as ActQty2
		,case when LPR.ActQtyLot2 is null then 0 else LPR.ActQtyLot2 end as ActQtyLot2
		,LPR.InOut
		,LPR.RunPlanstop
		,LPR.ProcessCode2
		,LPR.ProcessName2
	...
	select DISTINCT
		...
		,'' as 'LineCode'
		,replace(CONVERT(DATE,LPR.recorded_at, 114),'-','') as 'Date1'
		,replace(CONVERT(char(12),LPR.recorded_at,114) ,':','') as 'Time1'
		,'' as 'Date2'
		,'' as 'Time2'
...
　	    ,'' as OperatingCategory	
		,Case when (LPR.qty_pass <= 0 or LPR.qty_pass is null) then LPR.qty_in - LPR.qty_fail else LPR.qty_pass end as 'ActQty1'
		,0 as 'ActQtyLot1'
		,LPR.qty_last_fail as 'ActQty2'
		,0 as 'ActQtyLot2'
		,'' as 'InOut'
		,'' as 'RunPlanstop' 
	...
	order by LPR.ProcessNo, LPR.Date1, LPR.Time1, LPR.ProcessName
RETURN 0;
