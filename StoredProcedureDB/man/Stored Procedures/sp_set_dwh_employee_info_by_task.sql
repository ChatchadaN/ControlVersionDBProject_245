...
CREATE PROCEDURE [man].[sp_set_dwh_employee_info_by_task]
AS
BEGIN
	...
	SELECT [EmployeeCode] AS [emp_code]
		, [FullnameTH] AS [full_name_th]
		, [FullnameEN] AS [full_name_eng]
		, SUBSTRING([DisplayName], 1, LEN([DisplayName]) - 2) + SPACE(1) + RIGHT([DisplayName], 2) AS [display_name]
		, [DateBirth] AS [date_birth]
		, ( CASE SUBSTRING([EmployeeCode], 1, 1) 
			WHEN '0' THEN 'Tha'
			WHEN 'T' THEN 'Tha' 
			WHEN 'I' THEN 'Tha' 
			WHEN 'G' THEN 'Eng' 
		...
		, NULL AS [extension]
		, [Email] AS [email]
		, NULL AS [picture_data]
		, NULL AS [password]
		, 0 AS [is_admin]
		, [WorkingDate] AS [working_date]
		, NULL AS [emp_card]
		, s.id AS [shift_id]
	...
	INNER JOIN [DWH].[man].[positions] p ON PositionCode = p.positions_code  COLLATE SQL_Latin1_General_CP1_CI_AS
	INNER JOIN [DWH].[man].[shift_calendars] s ON ShiftCalendar = s.shift_code COLLATE SQL_Latin1_General_CP1_CI_AS
	ORDER BY [WorkingDate], [EmployeeCode] ASC;
