...
-- =============================================
CREATE PROCEDURE [man].[sp_get_user_authorization_new_version]
...
BEGIN
	...
	SELECT GETDATE()
		...
		, HOST_NAME()
		, APP_NAME()
		, 'EXEC [man].[sp_get_user_authorization_new_version] @emp_num = ''' + @emp_num + ''''
			+ ', @app_name = ''' + @app_name + ''''
	...
	BEGIN
		SELECT 'TRUE' AS [Is_Pass]
			, 'You are authorized' AS [Error_Message_ENG]
			, N'คุณได้รับอนุญาต' AS [Error_Message_THA]
			, N'' AS [Handling];
	END
	ELSE
	BEGIN
		SELECT 'FALSE' AS [Is_Pass]
			, 'You are unauthorized !!' AS [Error_Message_ENG]
			, N'คุณไม่ได้รับอนุญาต !!' AS [Error_Message_THA]
