...
-- =============================================
create PROCEDURE [etl].[sp_order_01_front_datas_v5] (
...
-- insert into assy_orders  
	...
	BEGIN
		...
		SET @pSqlAssyOrder = @pSqlAssyOrder + N'							and lots.wip_state <= 20 ';
		...
		SET @pSqlAssyOrder = @pSqlAssyOrder + N'	and not exists (select * from ' + @pObjAPCSPro + '.[robin].[assy_orders] as o with (NOLOCK) where o.order_no = t.order_no) ';
	END; -- select
...
-- insert into material_allocates_front
	...
	BEGIN
		...
		SET @pSqlMatAllocate = @pSqlMatAllocate + N'							and not exists (select * from ' + @pObjAPCSPro + N'.[robin].[material_allocates_front] as f with (NOLOCK) where f.lot_id = lots.id) ';
		--debug Feb22
		...
		SET @pSqlMatAllocate = @pSqlMatAllocate + N'			where not exists (select * from ' + @pObjAPCSPro + N'.[robin].[material_allocates_front] as f2 with (NOLOCK) where f2.lot_id = lot.id ) ';
...
-- insert into lot_information_front
	...
	BEGIN
		...
		--SET @pSqlLotInfoFront = @pSqlLotInfoFront + N'			lots.wip_state <=101 and isnull(lots.order_id,0)=0 ';
