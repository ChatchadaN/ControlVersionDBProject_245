...
CREATE PROCEDURE [etl].[sp_etl_1-17_alarm_texts](@ServerName_APCSPro NVARCHAR(128) 
...
BEGIN
	...
	SET @sqlSelect = @sqlSelect + N'	,t1.message_text ';
	SET @sqlSelect = @sqlSelect + N'	,t1.update_flg ';
	SET @sqlSelect = @sqlSelect + N'from ( ';
	SET @sqlSelect = @sqlSelect + N'		select ';
	SET @sqlSelect = @sqlSelect + N'			mc.alarm_text_id id ';
	SET @sqlSelect = @sqlSelect + N'			,mc.alarm_text message_text ';
	SET @sqlSelect = @sqlSelect + N'			,case when (mc.alarm_text_id = dwh.id) then ';
	SET @sqlSelect = @sqlSelect + N'				case when (RTRIM(mc.alarm_text) <> RTRIM(dwh.message_text) COLLATE SQL_Latin1_General_CP1_CI_AS) ';
	SET @sqlSelect = @sqlSelect + N'				then 2 ';
	SET @sqlSelect = @sqlSelect + N'				else 0 end ';
	SET @sqlSelect = @sqlSelect + N'			else 1 end as update_flg ';
	SET @sqlSelect = @sqlSelect + N'		from ';
	SET @sqlSelect = @sqlSelect + N'			' + @pObjAPCSPro + N'.[mc].[alarm_texts] mc with (NOLOCK) ';
	SET @sqlSelect = @sqlSelect + N'			left outer join ' + @pObjAPCSProDWH + N'.[dwh].[dim_alarm_texts] dwh with (NOLOCK) ';
	SET @sqlSelect = @sqlSelect + N'				on dwh.id = mc.alarm_text_id ';
			...
			BEGIN
				...
				IF @update_flg = 1	--INSERT
					BEGIN
		 				...
		 				SET @sqlTreat = N'';
						SET @sqlTreat = @sqlTreat + N'insert into ' + @pObjAPCSProDWH + N'.[dwh].[dim_alarm_texts] ';
						SET @sqlTreat = @sqlTreat + N' (id';
						SET @sqlTreat = @sqlTreat + N' ,message_text) ';
						SET @sqlTreat = @sqlTreat + N' values ';
						SET @sqlTreat = @sqlTreat + N' (';
						SET @sqlTreat = @sqlTreat + convert(varchar, @id) ; 
						--SET @sqlTreat = @sqlTreat + N' ,''' + RTRIM(@message_text) + N'''';
						SET @sqlTreat = @sqlTreat + N' ,N''' + replace(@message_text,'''','''''') + N'''';
						SET @sqlTreat = @sqlTreat + N' )';
						--print @sqltreat;
					...
					BEGIN
						SET @sqlTreat = N'';
						SET @sqlTreat = @sqlTreat + N'update ' + @pObjAPCSProDWH + N'.[dwh].[dim_alarm_texts] WITH (ROWLOCK) ';
						SET @sqlTreat = @sqlTreat + N' set message_text=N''' + RTRIM(@message_text) + N'''';
