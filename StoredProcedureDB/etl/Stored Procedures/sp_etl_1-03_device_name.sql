...
CREATE PROCEDURE [etl].[sp_etl_1-03_device_name](@v_ProServerName NVARCHAR(128) = ''
...
BEGIN
	...
	/*
	SET @sqltmp = N'';
	SET @sqltmp = @sqltmp + N'INSERT INTO [apcsprodwh].[dwh].[dim_devices] ';
	SET @sqltmp = @sqltmp + N'	( ';
	...
	SET @sqltmp = @sqltmp + N'			,rtrim(met.name) as met_name ';
	...
	SET @sqltmp = @sqltmp + N'		WHERE (met.name IS NOT NULL AND RTRIM(met.name) <> '''' ) ';
	SET @sqltmp = @sqltmp + N'			AND met.is_assy_only in(0,1) ';
	SET @sqltmp = @sqltmp + N'		GROUP BY met.id,met.name ,man.factory_id ,man.id, met2.id ';
	SET @sqltmp = @sqltmp + N'		) AS t1 ';
	SET @sqltmp = @sqltmp + N'WHERE NOT EXISTS ';
	SET @sqltmp = @sqltmp + N'		(SELECT ''X'' ';
	SET @sqltmp = @sqltmp + N'			FROM [apcsprodwh].[dwh].[dim_devices] AS dwh with (NOLOCK) ';
	...
	*/
	...
	SET @sqltmp = @sqltmp + N'			isnull(rtrim(met.name),'''') <> '''' ';
	SET @sqltmp = @sqltmp + N'				AND met.is_assy_only IN (0,1) ';
	SET @sqltmp = @sqltmp + N'		GROUP BY ';
	SET @sqltmp = @sqltmp + N'			met.id ';
	SET @sqltmp = @sqltmp + N'			,met.name ';
	SET @sqltmp = @sqltmp + N'			,man.factory_id ';
	SET @sqltmp = @sqltmp + N'			,man.id ';
	SET @sqltmp = @sqltmp + N'			,met2.id ';
	SET @sqltmp = @sqltmp + N'	) AS t1 ';
	SET @sqltmp = @sqltmp + N'	LEFT OUTER JOIN [APCSProDWH].[dwh].[dim_devices] AS dwh WITH (NOLOCK) ';
		...
		WHILE @@FETCH_STATUS = 0
			...
			BEGIN
				...
				IF @update_flg = 1	--INSERT
					BEGIN
		 				SET @sqlTreat = N'';
						SET @sqlTreat = @sqlTreat + N'insert into [apcsprodwh].[dwh].[dim_devices] ';
					...
					BEGIN
						SET @sqlTreat = N'';
						SET @sqlTreat = @sqlTreat + N'update [apcsprodwh].[dwh].[dim_devices] WITH (ROWLOCK) ';
