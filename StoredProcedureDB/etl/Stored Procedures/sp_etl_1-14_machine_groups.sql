...
-- Description:	<>
-- =============================================
CREATE PROCEDURE [etl].[sp_etl_1-14_machine_groups] (@ServerName_APCSPro NVARCHAR(128) 
...
BEGIN
	...
	SET @sqlSelect = @sqlSelect + N'	,t1.product_family_id';
	...
	SET @sqlSelect = @sqlSelect + N'			,mc.product_family_id ';
	...
	SET @sqlSelect = @sqlSelect + N'					(isnull(mc.product_family_id,0) <> isnull(dwh.product_family_id,0)) ';
	SET @sqlSelect = @sqlSelect + N'				then 2 ';
	SET @sqlSelect = @sqlSelect + N'			else 0 end ';
	SET @sqlSelect = @sqlSelect + N'			else 1 end as update_flg ';
	SET @sqlSelect = @sqlSelect + N'		from ';
	SET @sqlSelect = @sqlSelect + N'			' + @pObjAPCSPro + N'.[mc].groups mc with (NOLOCK) ';
	SET @sqlSelect = @sqlSelect + N'			left outer join ' + @pObjAPCSProDWH + N'.[dwh].[dim_machine_groups] dwh with (NOLOCK) ';
			...
			BEGIN
				...
				IF @update_flg = 1	--INSERT
					BEGIN
		 				...
		 				SET @sqlTreat = N'';
						SET @sqlTreat = @sqlTreat + N'insert into ' + @pObjAPCSProDWH + N'.[dwh].[dim_machine_groups] ';
						...
						SET @sqlTreat = @sqlTreat + N' ,' + convert(varchar,@product_family_id) ;
						SET @sqlTreat = @sqlTreat + N' )';
						--print @sqltreat;
					...
					BEGIN
						SET @sqlTreat = N'';
						SET @sqlTreat = @sqlTreat + N'update ' + @pObjAPCSProDWH + N'.[dwh].[dim_machine_groups] WITH (ROWLOCK) ';
						SET @sqlTreat = @sqlTreat + N' set name=''' + RTRIM(@name) + N'''';
						SET @sqlTreat = @sqlTreat + N' ,product_family_id=' + convert(varchar,@product_family_id) ;
