...
CREATE PROCEDURE [etl].[sp_etl_1-09_factories] (@v_ProServerName NVARCHAR(128) = ''
...
BEGIN
	...
	SET @sqltmp = @sqltmp + N'				,man.factory_code AS factory_code ';
	SET @sqltmp = @sqltmp + N'				,CASE WHEN (man.id = dwh.id) THEN ';
	SET @sqltmp = @sqltmp + N'					CASE WHEN (RTRIM(man.name) = RTRIM(dwh.name) COLLATE SQL_Latin1_General_CP1_CI_AS) ';
	SET @sqltmp = @sqltmp + N'						then CASE WHEN (RTRIM(man.factory_code) = RTRIM(dwh.factory_code) COLLATE SQL_Latin1_General_CP1_CI_AS) THEN 0 ';
	...
	SET @sqltmp = @sqltmp + N'FROM ' + @objectname + '[man].[factories] AS man with (NOLOCK) ';
	SET @sqltmp = @sqltmp + N'	LEFT OUTER JOIN [apcsprodwh].[dwh].[dim_factories] AS dwh with (NOLOCK) ';
	...
	--(5) Open Cur
    ---------------------------------------------------------------------------
	EXECUTE ('DECLARE Cur_factories CURSOR FOR ' + @sqltmp ) ;
	OPEN Cur_factories;

	FETCH NEXT FROM Cur_factories
	INTO
		 @man_id
		,@man_name
		,@factory_code
		...
		WHILE @@FETCH_STATUS = 0
			...
			BEGIN
				...
				IF @update_flg = 1	--INSERT
					BEGIN
						INSERT INTO [apcsprodwh].[dwh].[dim_factories]
							(id
							,name
							,factory_code
							)
				...
				ELSE	--UPDATE
					BEGIN
						UPDATE [apcsprodwh].[dwh].[dim_factories]
						SET    name = @man_name
