...
CREATE PROCEDURE [etl].[sp_etl_1-11_product_families] (@v_ProServerName NVARCHAR(128) = ''
...
BEGIN
	...
	DECLARE @man_product_code VARCHAR(2);
	...
	SET @sqltmp = @sqltmp + N'		,t1.man_product_code ';
	...
	SET @sqltmp = @sqltmp + N'				,man.product_code as man_product_code ';
	...
	SET @sqltmp = @sqltmp + N'							CASE WHEN (RTRIM(man.product_code) = RTRIM(dwh.product_code) COLLATE SQL_Latin1_General_CP1_CI_AS) THEN 0 ';
	...
	SET @sqltmp = @sqltmp + N'			FROM ' + @objectname + '[man].[product_families] AS man with (NOLOCK) ';
	SET @sqltmp = @sqltmp + N'				LEFT OUTER JOIN [apcsprodwh].[dwh].[dim_product_families] AS dwh with (NOLOCK) ';
	...
	--(5) Open Cur
    ---------------------------------------------------------------------------
	EXECUTE ('DECLARE Cur_product_families CURSOR FOR ' + @sqltmp ) ;
	OPEN Cur_product_families;

	FETCH NEXT FROM Cur_product_families
	INTO
		 @man_id
		,@man_name
		,@man_factory_id
		...
		WHILE @@FETCH_STATUS = 0
			...
			BEGIN
				...
				IF @update_flg = 1	--INSERT
					BEGIN
						INSERT INTO [apcsprodwh].[dwh].[dim_product_families]
							(id
							,name
							,factory_id
							,product_code
				...
				ELSE	--UPDATE
					BEGIN
						UPDATE [apcsprodwh].[dwh].[dim_product_families]
						SET    name = @man_name
								,factory_id = @man_factory_id
								,product_code = @man_product_code
						WHERE id = @man_id;
					END;
	...
	--(7)close
    ---------------------------------------------------------------------------
	CLOSE Cur_product_families;
	DEALLOCATE Cur_product_families;
