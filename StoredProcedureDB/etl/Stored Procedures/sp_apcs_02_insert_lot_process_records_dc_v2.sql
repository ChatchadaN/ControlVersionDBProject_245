...
-- =============================================
CREATE PROCEDURE [etl].[sp_apcs_02_insert_lot_process_records_dc_v2] (
...
BEGIN
	...
	DECLARE @pSqlInsDCSelect NVARCHAR(4000) = N''; 
...
*/
	...
	BEGIN
		...
		SET @pSqlInsDCSelect = N'';
		SET @pSqlInsDCSelect = @pSqlInsDCSelect + N' select ';
		SET @pSqlInsDCSelect = @pSqlInsDCSelect + N'	lot.id lot_id ';
		--SET @@pSqlInsDCSelect = @@pSqlInsDCSelect + N'	,lot.lot_no ';
		--SET @@pSqlInsDCSelect = @@pSqlInsDCSelect + N'	,lot.wip_state ';
		SET @pSqlInsDCSelect = @pSqlInsDCSelect + N'	,l2.real_start started_at ';
		SET @pSqlInsDCSelect = @pSqlInsDCSelect + N'	,l2.real_day finished_at ';
		SET @pSqlInsDCSelect = @pSqlInsDCSelect + N'	,l2.operator1 started_op ';
		SET @pSqlInsDCSelect = @pSqlInsDCSelect + N'	,l2.operator2 finished_op ';
		SET @pSqlInsDCSelect = @pSqlInsDCSelect + N'	,l2.machine ';
		SET @pSqlInsDCSelect = @pSqlInsDCSelect + N'	,l2.Good_Pieces qty_pass ';
		SET @pSqlInsDCSelect = @pSqlInsDCSelect + N'	,l2.Bad_Pieces qty_fail ';
		SET @pSqlInsDCSelect = @pSqlInsDCSelect + N'	,pj.job_no ';
		SET @pSqlInsDCSelect = @pSqlInsDCSelect + N' from ';
		SET @pSqlInsDCSelect = @pSqlInsDCSelect + N'	' + @pObjAPCSPro + N'.[trans].[lots] lot with (NOLOCK) ';
		SET @pSqlInsDCSelect = @pSqlInsDCSelect + N'	inner join ' + @pObjAPCSProDWH + N'.[dwh].[dim_package_jobs] as pj with (NOLOCK) ';
		SET @pSqlInsDCSelect = @pSqlInsDCSelect + N'		on pj.package_id = lot.act_package_id ';
		SET @pSqlInsDCSelect = @pSqlInsDCSelect + N'			and pj.is_additional_jobs = 1 ';
		SET @pSqlInsDCSelect = @pSqlInsDCSelect + N'	inner join OPENROWSET(''SQLNCLI'', ''Server=' + @ServerName_APCS + ';UID=dbxuser;'', ';
		SET @pSqlInsDCSelect = @pSqlInsDCSelect + N'		''select l2d.LOT_NO,l2d.LAY_NO,l2d.real_start,l2d.real_day,l2d.operator1,l2d.operator2,l2d.machine,l2d.Good_Pieces,l2d.Bad_Pieces  ';
		SET @pSqlInsDCSelect = @pSqlInsDCSelect + N'					 from ' + @DatabaseName_APCS + N'.[dbo].[LOT2_DATA] as l2d with (NOLOCK)  ';
		SET @pSqlInsDCSelect = @pSqlInsDCSelect + N'					 where l2d.LAY_NO = ''''0101''''';
		SET @pSqlInsDCSelect = @pSqlInsDCSelect + N'		'') as l2 ';
		SET @pSqlInsDCSelect = @pSqlInsDCSelect + N'		on lot.lot_no = l2.LOT_NO '; 
		SET @pSqlInsDCSelect = @pSqlInsDCSelect + N'			and l2.lay_no = pj.job_no ';
		SET @pSqlInsDCSelect = @pSqlInsDCSelect + N' where ';
		SET @pSqlInsDCSelect = @pSqlInsDCSelect + N'	lot.wip_state < 20 ';
		PRINT N'@pSqlInsDCSelect=' + @pSqlInsDCSelect;
	END;
	...
	-- for temp_DC_process_records
		...
		BEGIN TRANSACTION;
			...
			SET @pStepNo = 2;
			Set @pSqlInsDC = @pSqlInsDC + @pSqlInsDCSelect;
