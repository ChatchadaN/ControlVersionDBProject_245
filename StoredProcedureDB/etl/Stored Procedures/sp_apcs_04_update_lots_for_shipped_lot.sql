...
-- =============================================
CREATE PROCEDURE [etl].[sp_apcs_04_update_lots_for_shipped_lot] (
	...
	*/
	...
	BEGIN TRY
		...
		*/
		...
		--set oldest data 
		SET @pStarttime = ISNULL(@pStarttime,convert(datetime,'2019-09-01 00:00:00.000',21));
		PRINT '@pStarttime=' + CASE WHEN @pStarttime IS NULL THEN '' ELSE FORMAT(@pStarttime, 'yyyy-MM-dd HH:mm:ss.fff') END;
...
*/
	...
	SET @pSqlUpdate = @pSqlUpdate + N'	wip_state = 101 ';
	...
	SET @pSqlUpdate = @pSqlUpdate + N'	inner join ' + @pObjAPCSPro + N'.[method].[device_slips] ds with (NOLOCK) ';
	SET @pSqlUpdate = @pSqlUpdate + N'  on ds.device_slip_id = lots.device_slip_id ';
	SET @pSqlUpdate = @pSqlUpdate + N'  inner join ' + @pObjAPCSPro + N'.[method].[device_names] dn with (NOLOCK) ';
	SET @pSqlUpdate = @pSqlUpdate + N'	on dn.id = ds.device_id ';
	...
	SET @pSqlUpdate = @pSqlUpdate + N'	and dn.is_assy_only not in (0,1)';
