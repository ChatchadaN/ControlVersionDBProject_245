CREATE PROCEDURE [etl].[sp_etl_2-00_update_wip_state] (
...
*/
	...
	SET @pSqlUpdate = @pSqlUpdate + N'	    ,in_at = convert(datetime,t1.date_value) ';
	SET @pSqlUpdate = @pSqlUpdate + N'	    ,in_date_id = l.in_plan_date_id ';
	...
	SET @pSqlUpdate = @pSqlUpdate + N'				and convert(datetime,dy.date_value) < getdate() '; 
	...
	SET @pSqlUpdate2 = @pSqlUpdate2 + N'	    ,in_at = case when l.in_at is null then convert(datetime,t1.date_value) else l.in_at end ';
	SET @pSqlUpdate2 = @pSqlUpdate2 + N'	    ,in_date_id =  case when l.in_date_id is null then l.in_plan_date_id else l.in_date_id end ';
	...
	--SET @pSqlUpdate2 = @pSqlUpdate2 + N'				and convert(datetime,dy.date_value) < getdate() ';   --Update by Nucha P. 2023/12/15 (Advance Input)
