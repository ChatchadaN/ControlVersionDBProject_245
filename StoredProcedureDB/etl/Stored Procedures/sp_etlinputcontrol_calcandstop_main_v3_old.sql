...
create PROCEDURE [etl].[sp_etlinputcontrol_calcandstop_main_v3_old] 
...
from APCSProDWH.wip_control.monitoring_items  as i with (ROWLOCK)
	...
	BEGIN CATCH
		...
		SET @logtext = '[ERROR] sp_etlinputcontrol_calcandstop_main' +'/ret:' + convert(varchar,@ret)  + N'/num:' + convert(varchar,@errnum) + N'/line:' + convert(varchar,@errline) + '/msg:' + @errmsg + '/Part:' + convert(varchar,@partno);
		PRINT @logtext;
		RETURN -1;
	END CATCH;
