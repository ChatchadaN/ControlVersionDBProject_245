...
CREATE PROCEDURE [etl].[sp_etl_1-08_jobs] (@v_ProServerName NVARCHAR(128) = ''
...
BEGIN
	...
	--SET @sqltmp = @sqltmp + N'				,CASE WHEN (met.id = dwh.id) THEN ';
	--SET @sqltmp = @sqltmp + N'						CASE WHEN (RTRIM(met.name) = RTRIM(dwh.name) COLLATE SQL_Latin1_General_CP1_CI_AS) THEN ';
	--SET @sqltmp = @sqltmp + N'							CASE WHEN ((met_p.process_no IS NULL AND dwh.process_no IS NULL) OR (RTRIM(met_p.process_no) = RTRIM(dwh.process_no) COLLATE SQL_Latin1_General_CP1_CI_AS)) THEN ';
	--SET @sqltmp = @sqltmp + N'								CASE WHEN ((met.job_no IS NULL AND dwh.job_no IS NULL) OR (RTRIM(met.job_no) = RTRIM(dwh.job_no) COLLATE SQL_Latin1_General_CP1_CI_AS)) THEN ';
	--SET @sqltmp = @sqltmp + N'									CASE WHEN ((man.factory_id IS NULL AND dwh.factory_id IS NULL) OR (man.factory_id = dwh.factory_id)) THEN ';
	--SET @sqltmp = @sqltmp + N'										CASE WHEN ((man.id IS NULL AND dwh.product_family_id IS NULL) OR (man.id = dwh.product_family_id)) THEN ';
	--SET @sqltmp = @sqltmp + N'											CASE WHEN ((met.is_skipped IS NULL AND dwh.is_skipped IS NULL) OR (met.is_skipped = dwh.is_skipped)) THEN 0 ';
	--SET @sqltmp = @sqltmp + N'													ELSE 2 ';
	--SET @sqltmp = @sqltmp + N'													END ';
	--SET @sqltmp = @sqltmp + N'											ELSE 2 ';
	--SET @sqltmp = @sqltmp + N'											END ';
	...
	DECLARE @package_jobs_id INT = 0;
	DECLARE @is_additional_jobs INT = 0;

SET @sqltmp2 = N'';
SET @sqltmp2 = @sqltmp2 + N'select ';
SET @sqltmp2 = @sqltmp2 + N'	t1.package_id ';
SET @sqltmp2 = @sqltmp2 + N'	,t1.process_id ';
SET @sqltmp2 = @sqltmp2 + N'	,t1.process_no ';
SET @sqltmp2 = @sqltmp2 + N'	,t1.process_name ';
SET @sqltmp2 = @sqltmp2 + N'	,t1.job_id ';
SET @sqltmp2 = @sqltmp2 + N'	,t1.job_no ';
SET @sqltmp2 = @sqltmp2 + N'	,t1.job_name ';
SET @sqltmp2 = @sqltmp2 + N'	,t1.man_factory_id ';
SET @sqltmp2 = @sqltmp2 + N'	,t1.man_product_family_id ';
SET @sqltmp2 = @sqltmp2 + N'	,t1.is_skipped ';
SET @sqltmp2 = @sqltmp2 + N'	,t1.updateflg ';
SET @sqltmp2 = @sqltmp2 + N'	,isnull(t1.package_jobs_id,0) as package_jobs_id ';
SET @sqltmp2 = @sqltmp2 + N'	,t1.is_additional_jobs ';
...
SET @sqltmp2 = @sqltmp2 + N'			,case when pj.id is null then 1 ';
SET @sqltmp2 = @sqltmp2 + N'									else case when isnull(jp.process_id,0) <> isnull(pj.process_id,0) ';
SET @sqltmp2 = @sqltmp2 + N'												then 2 ';
SET @sqltmp2 = @sqltmp2 + N'												else case when min(isnull(case when pk.is_enabled = 1 then df.is_skipped else jp.is_skipped end,0)) <> isnull(pj.is_skipped,0) ';
SET @sqltmp2 = @sqltmp2 + N'															then 2 ';
SET @sqltmp2 = @sqltmp2 + N'															else case when jp.is_additional_jobs <> isnull(pj.is_additional_jobs,0) ';
SET @sqltmp2 = @sqltmp2 + N'																		then 2 ';
SET @sqltmp2 = @sqltmp2 + N'																		else 0 ';
SET @sqltmp2 = @sqltmp2 + N'																		end ';
SET @sqltmp2 = @sqltmp2 + N'															end ';
SET @sqltmp2 = @sqltmp2 + N'												end ';
SET @sqltmp2 = @sqltmp2 + N'									end as updateflg ';
SET @sqltmp2 = @sqltmp2 + N'			,jp.is_additional_jobs ';
SET @sqltmp2 = @sqltmp2 + N'			,pj.id as package_jobs_id ';
SET @sqltmp2 = @sqltmp2 + N'			,pj.process_id as current_process_id ';
SET @sqltmp2 = @sqltmp2 + N'			,pj.process_no as current_process_no ';
SET @sqltmp2 = @sqltmp2 + N'			,pj.process_name as current_process_name ';
SET @sqltmp2 = @sqltmp2 + N'			,pj.job_id as current_job_id ';
SET @sqltmp2 = @sqltmp2 + N'			,pj.job_no as current_job_no ';
SET @sqltmp2 = @sqltmp2 + N'			,pj.job_name as current_job_name ';
SET @sqltmp2 = @sqltmp2 + N'			,isnull(pj.is_skipped,0) as pj_is_skipped ';
SET @sqltmp2 = @sqltmp2 + N'			,pj.is_additional_jobs as current_is_additional_jobs ';
SET @sqltmp2 = @sqltmp2 + N'			,pj.process_class ';
SET @sqltmp2 = @sqltmp2 + N'			,pj.max_step_no ';
...
SET @sqltmp2 = @sqltmp2 + N'					and ds.is_released in(1) ';
...
SET @sqltmp2 = @sqltmp2 + N'							select j.id as job_id,j.name as job_name,j.job_no,isnull(j.is_skipped,0) as is_skipped,isnull(p.id,0) as process_id,isnull(p.process_no,'''') as process_no,isnull(p.name,'''') as process_name,j.product_family_id,case when j.job_no = ''0101'' then 1 else 0 end as is_additional_jobs ';
SET @sqltmp2 = @sqltmp2 + N'							from ' + @objectname + 'method.jobs AS j with (NOLOCK) ';
...
SET @sqltmp2 = @sqltmp2 + N'							select  j.id as job_id,j.name as job_name,j.job_no,isnull(j.is_skipped,0) as is_skipped,isnull(p.id,0) as process_id,isnull(p.process_no,'''') as process_no,isnull(p.name,'''') as process_name,p.product_family_id,1 as is_additional_jobs ';
SET @sqltmp2 = @sqltmp2 + N'							from APCSProDWH.dwh.dim_jobs as j with (NOLOCK) ';
...
SET @sqltmp2 = @sqltmp2 + N'					and not exists (select * from apcsprodwh.dwh.dim_package_jobs as pj2 where pj2.package_id = pj.package_id and pj2.job_id = pj.job_id and pj2.id > pj.id) ';
...
SET @sqltmp2 = @sqltmp2 + N'			not exists (select * from ' + @objectname + 'method.device_slips as ds2 with (NOLOCK) ';
SET @sqltmp2 = @sqltmp2 + N'						where ds2.device_id = ds.device_id and ds2.is_released in(1) and ds2.version_num > ds.version_num) ';
...
SET @sqltmp2 = @sqltmp2 + N'			,pj.is_additional_jobs ';
SET @sqltmp2 = @sqltmp2 + N'			,pj.process_class ';
SET @sqltmp2 = @sqltmp2 + N'			,pj.max_step_no ';
SET @sqltmp2 = @sqltmp2 + N'			,jp.is_additional_jobs ';
SET @sqltmp2 = @sqltmp2 + N'  ) as t1 ';
SET @sqltmp2 = @sqltmp2 + N'where t1.updateflg > 0 ';
SET @sqltmp2 = @sqltmp2 + N'order by t1.job_name,t1.package_id ';
						...
						IF @update_flg1 = 2	--UPDATE
								BEGIN
									...
									SET    process_id = @process_id 
											...
											,is_additional_jobs = @is_additional_jobs
									WHERE id = @package_jobs_id;
								END;
					END; 
