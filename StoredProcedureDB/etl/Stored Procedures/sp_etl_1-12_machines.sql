...
-- Description:	<>
-- =============================================
CREATE PROCEDURE [etl].[sp_etl_1-12_machines] (	@ServerName_APCSPro NVARCHAR(128) 
...
BEGIN
	...
	SET @sqlSelect = @sqlSelect + N'	,t1.name ';
	SET @sqlSelect = @sqlSelect + N'	,t1.machine_model_id ';
	SET @sqlSelect = @sqlSelect + N'	,t1.headquarter_id ';
	...
	SET @sqlSelect = @sqlSelect + N'		,mc.name ';
	SET @sqlSelect = @sqlSelect + N'		,isnull(mc.machine_model_id,0) machine_model_id ';
	SET @sqlSelect = @sqlSelect + N'		,mc.headquarter_id ';
	SET @sqlSelect = @sqlSelect + N'		,isnull(mc.is_disabled,0) is_disabled ';
	SET @sqlSelect = @sqlSelect + N'		,case when (mc.id = dwh.id) then ';
	SET @sqlSelect = @sqlSelect + N'			case when (RTRIM(mc.name) <> RTRIM(dwh.name) COLLATE SQL_Latin1_General_CP1_CI_AS) ';
	SET @sqlSelect = @sqlSelect + N'						or ';
	SET @sqlSelect = @sqlSelect + N'						(isnull(mc.machine_model_id,0) <> isNULL(dwh.machine_model_id,0)) ';
	SET @sqlSelect = @sqlSelect + N'						or ';
	--temporary
	SET @sqlSelect = @sqlSelect + N'						(isnull(mc.headquarter_id,0) <> isNULL(dwh.headquarter_id,0)) ';
	--SET @sqlSelect = @sqlSelect + N'						(isnull(mc.headquarter_id,0) <> isNULL(dwh.headquater_id,0)) ';
	SET @sqlSelect = @sqlSelect + N'						or ';
	...
	SET @sqlSelect = @sqlSelect + N'		 else 1 ';
	SET @sqlSelect = @sqlSelect + N'		 end as update_flg ';
	SET @sqlSelect = @sqlSelect + N'	from ' + @pObjAPCSPro + N'.[mc].machines mc WITH (NOLOCK) ';
	SET @sqlSelect = @sqlSelect + N'		left outer join ' + @pObjAPCSProDWH + N'.[dwh].[dim_machines] dwh WITH (NOLOCK) ';
			...
			BEGIN
				...
				IF @update_flg = 1	--INSERT
					BEGIN
		 				...
		 				SET @sqlTreat = N'';
						SET @sqlTreat = @sqlTreat + N'insert into ' + @pObjAPCSProDWH + N'.[dwh].[dim_machines] ';
						SET @sqlTreat = @sqlTreat + N' (id';
						SET @sqlTreat = @sqlTreat + N' ,name ';
						SET @sqlTreat = @sqlTreat + N' ,machine_model_id ';
						--temporary
						SET @sqlTreat = @sqlTreat + N' ,headquarter_id ';
						--SET @sqlTreat = @sqlTreat + N' ,headquater_id ';
						...
						SET @sqlTreat = @sqlTreat + N' ,''' + RTRIM(@name) + N'''';
						if @machine_model_id is null 
							SET @sqlTreat = @sqlTreat + N'		,null' ;
						else
							SET @sqlTreat = @sqlTreat + N'		,' + convert(varchar,@machine_model_id) ;
						SET @sqlTreat = @sqlTreat + N' ,' + convert(varchar,@headquarter_id) ;
					...
					BEGIN
						SET @sqlTreat = N'';
						SET @sqlTreat = @sqlTreat + N'update ' + @pObjAPCSProDWH + N'.[dwh].[dim_machines] WITH (ROWLOCK) ';
						SET @sqlTreat = @sqlTreat + N' set name=''' + RTRIM(@name) + N'''';
						if @machine_model_id is null 
							SET @sqlTreat = @sqlTreat + N'		,machine_model_id=null' ;
						else
							SET @sqlTreat = @sqlTreat + N'		,machine_model_id=' + convert(varchar,@machine_model_id) ;

						--temporary
						SET @sqlTreat = @sqlTreat + N' ,headquarter_id=' + convert(varchar,@headquarter_id) ;
						--SET @sqlTreat = @sqlTreat + N' ,headquater_id=' + convert(varchar,@headquarter_id) ;

						if @is_disabled is null 
							SET @sqlTreat = @sqlTreat + N'		,is_disabled=null' ;
						else
