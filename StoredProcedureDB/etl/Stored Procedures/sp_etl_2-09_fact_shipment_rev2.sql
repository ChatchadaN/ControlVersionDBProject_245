...
CREATE PROCEDURE [etl].[sp_etl_2-09_fact_shipment_rev2] (@v_ProServerName NVARCHAR(128) = ''
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @starttime = isnull(dateadd(day,-10,finished_at),'2019-04-01')  FROM [apcsprodwh].[dwh].[function_finish_control] WHERE function_name = OBJECT_NAME(@@PROCID)
	...
	SET @sqltmp = @sqltmp + N'where l.wip_state = 100 '; 
