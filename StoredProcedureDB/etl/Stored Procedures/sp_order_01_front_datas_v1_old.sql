...
-- =============================================
create PROCEDURE [etl].[sp_order_01_front_datas_v1_old] (
...
-- insert into assy_orders  
	...
	BEGIN
		...
		SET @pSqlAssyOrder = @pSqlAssyOrder + N'							lots.order_id = 0 or lots.order_id is null ';
		...
		SET @pSqlAssyOrder = @pSqlAssyOrder + N'						  from ';
		--need to change
		SET @pSqlAssyOrder = @pSqlAssyOrder + N'							OPENDATASOURCE(''SQLNCLI'', ''Server=' + @ServerName_APCS + ';UID=dbxuser;'').[' + @DatabaseName_APCS + '].[dbo].[LCQW_UNION_WORK_DENPYO_PRINT] as d ) den ';
		...
		--need to change
		SET @pSqlAssyOrder = @pSqlAssyOrder + N'					inner join OPENDATASOURCE(''SQLNCLI'', ''Server=' + @ServerName_APCS + ';UID=dbxuser;'').[' + @DatabaseName_APCS + '].[dbo].[LOT1_TABLE_INPUT] as input ';
...
-- insert into material_allocates_front
	...
	BEGIN
		...
		SET @pSqlMatAllocate = @pSqlMatAllocate + N'							lots.order_id = 0 or lots.order_id is null ';
		--debug Feb22
		--SET @pSqlMatAllocate = @pSqlMatAllocate + N'							and lots.wip_state=100 ';
		SET @pSqlMatAllocate = @pSqlMatAllocate + N'					) lot ';
		--need to change
		SET @pSqlMatAllocate = @pSqlMatAllocate + N'				inner join OPENDATASOURCE(''SQLNCLI'', ''Server=' + @ServerName_APCS + ';UID=dbxuser;'').[' + @DatabaseName_APCS + '].[dbo].[LOT1_TABLE_INPUT] input ';
...
-- insert into lot_information_front
	...
	BEGIN
		...
		SET @pSqlLotInfoFront = @pSqlLotInfoFront + N'		from ';
		--need to change
		SET @pSqlLotInfoFront = @pSqlLotInfoFront + N'							OPENDATASOURCE(''SQLNCLI'', ''Server=' + @ServerName_APCS + ';UID=dbxuser;'').[' + @DatabaseName_APCS + '].[dbo].[LCQW_UNION_WORK_DENPYO_PRINT] d) den ';
