...
CREATE PROCEDURE [etl].[sp_etl_1-06_qc_states] (@v_ProServerName NVARCHAR(128) = ''
...
BEGIN
	...
	SET @sqltmp = @sqltmp + N'				LEFT OUTER JOIN [apcsprodwh].[dwh].[dim_qc_states] AS dwh with (NOLOCK) ';
	SET @sqltmp = @sqltmp + N'					ON dwh.code = trans.val ';
	SET @sqltmp = @sqltmp + N'			WHERE RTRIM(trans.name) = ''lots.quality_state'' ';
	...
	--(5) Open Cur
    ---------------------------------------------------------------------------
	EXECUTE ('DECLARE Cur_qc_states CURSOR FOR ' + @sqltmp ) ;
	OPEN Cur_qc_states;

	FETCH NEXT FROM Cur_qc_states
	INTO
		 @trans_code
		,@trans_name
		,@update_flg;
		...
		WHILE @@FETCH_STATUS = 0
			...
			BEGIN
				...
				IF @update_flg = 1	--INSERT
					BEGIN
						INSERT INTO [apcsprodwh].[dwh].[dim_qc_states]
							(code
							,name
							)
						VALUES
				...
				ELSE	--UPDATE
					BEGIN
						UPDATE [apcsprodwh].[dwh].[dim_qc_states]
