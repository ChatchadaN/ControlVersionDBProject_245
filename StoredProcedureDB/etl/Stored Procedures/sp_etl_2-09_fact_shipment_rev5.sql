...
CREATE PROCEDURE [etl].[sp_etl_2-09_fact_shipment_rev5] (@v_ProServerName NVARCHAR(128) = ''
...
BEGIN
	...
	SET @sqltmp = @sqltmp + N'	,t1.new_divided as ship '; 
	SET @sqltmp = @sqltmp + N'	,t1.std_time '; 
	SET @sqltmp = @sqltmp + N'	,t1.lead_time '; 
	SET @sqltmp = @sqltmp + N'	,isnull(t1.lead_time - sum(DATEDIFF(MINUTE, t1.s_recorded_at, t1.f_recorded_at)),0) AS wait_time '; 
	SET @sqltmp = @sqltmp + N'	,isnull(sum(DATEDIFF(MINUTE, t1.s_recorded_at, t1.f_recorded_at)),0) AS process_time '; 
	...
	SET @sqltmp = @sqltmp + N'	,l.qty_divided '; 
	SET @sqltmp = @sqltmp + N'	,case when isnull(l.qty_divided,0) = 0 then isnull(l.qty_out,0) else l.qty_divided end as new_divided ';
	...
	SET @sqltmp = @sqltmp + N'	,isnull(lot_rec_s.recorded_at,l.in_at) as s_recorded_at '; 
	SET @sqltmp = @sqltmp + N'	,isnull(lot_rec_f.recorded_at,l.ship_at) as f_recorded_at '; 
	...
	SET @sqltmp = @sqltmp + N'	inner join ' + @objectname + '[method].[device_names] as d with (NOLOCK) '; 
	SET @sqltmp = @sqltmp + N'		on d.id = l.act_device_name_id '; 
	SET @sqltmp = @sqltmp + N'			and d.is_assy_only in(0,1) '; 
	SET @sqltmp = @sqltmp + N'	inner join ' + @objectname + '[method].[packages] as p with (NOLOCK) '; 
	SET @sqltmp = @sqltmp + N'		on p.id = d.package_id '; 
	SET @sqltmp = @sqltmp + N'	inner join ' + @objectname + '[man].[product_headquarters] as h with (NOLOCK) '; 
	SET @sqltmp = @sqltmp + N'		on h.product_family_id = p.product_family_id '; 
	SET @sqltmp = @sqltmp + N'	inner join ' + @objectname + '[man].[headquarters] as hq with (NOLOCK) '; 
	SET @sqltmp = @sqltmp + N'		on hq.id = h.headquarter_id '; 
	SET @sqltmp = @sqltmp + N'	left outer join ' + @objectname + '[trans].[lot_process_records] AS lot_rec_s with (NOLOCK) '; 
	SET @sqltmp = @sqltmp + N'				ON lot_rec_s.lot_id = l.id '; 
	SET @sqltmp = @sqltmp + N'					and lot_rec_s.record_class = 1 '; 
	SET @sqltmp = @sqltmp + N'	left outer join ' + @objectname + '[trans].[lot_process_records] AS lot_rec_f with (NOLOCK) '; 
	...
	SET @sqltmp = @sqltmp + N'	and l.lot_no like ''%V'' '; 
	SET @sqltmp = @sqltmp + N'	and l.ship_at >= ''' + convert(varchar,@starttime,21) + ''' and l.ship_at < ''' + convert(varchar,@endtime,21) + '''';
	SET @sqltmp = @sqltmp + N'	and not exists (select * from ' + @objectnamedwh + '[dwh].[fact_shipment] as f with (NOLOCK) where f.lot_id = l.id) '; 
	...
	SET @sqltmp = @sqltmp + N'	,t1.lot_id '; 
	SET @sqltmp = @sqltmp + N'	,t1.qty_in '; 
	SET @sqltmp = @sqltmp + N'	,t1.qty_divided '; 
	SET @sqltmp = @sqltmp + N'	,t1.new_divided '; 
	SET @sqltmp = @sqltmp + N'	,t1.std_time '; 
	SET @sqltmp = @sqltmp + N'	,t1.lead_time '; 
