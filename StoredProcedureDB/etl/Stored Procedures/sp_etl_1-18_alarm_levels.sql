...
-- Description:	<>
-- =============================================
CREATE PROCEDURE [etl].[sp_etl_1-18_alarm_levels] (	@ServerName_APCSPro NVARCHAR(128) 
...
BEGIN
	...
	DECLARE @name_eng NVARCHAR(30);
	DECLARE @name_jpn NVARCHAR(30);
	DECLARE @name_sub NVARCHAR(30);
	DECLARE @update_flg INT = 0;

	SET @sqlSelect = N''
	SET @sqlSelect = @sqlSelect + N'select ';
	SET @sqlSelect = @sqlSelect + N'		t1.code ';
	SET @sqlSelect = @sqlSelect + N'		,t1.name_eng ';
	SET @sqlSelect = @sqlSelect + N'		,t1.name_jpn ';
	SET @sqlSelect = @sqlSelect + N'		,t1.name_sub ';
	SET @sqlSelect = @sqlSelect + N'		,t1.update_flg ';
	SET @sqlSelect = @sqlSelect + N'from ( ';
	SET @sqlSelect = @sqlSelect + N'			select '; 
	SET @sqlSelect = @sqlSelect + N'				item.val as code ';
	SET @sqlSelect = @sqlSelect + N'				,item.label_eng as name_eng ';
	SET @sqlSelect = @sqlSelect + N'				,item.label_jpn as name_jpn ';
	SET @sqlSelect = @sqlSelect + N'				,item.label_sub as name_sub ';
	SET @sqlSelect = @sqlSelect + N'				,case when (item.val = dwh.code) then ';
	SET @sqlSelect = @sqlSelect + N'					case when (RTRIM(item.label_eng) <> RTRIM(dwh.name_eng) COLLATE SQL_Latin1_General_CP1_CI_AS) ';
	SET @sqlSelect = @sqlSelect + N'					or ';
	SET @sqlSelect = @sqlSelect + N'					(RTRIM(item.label_jpn) <> RTRIM(dwh.name_jpn) COLLATE SQL_Latin1_General_CP1_CI_AS)';
	SET @sqlSelect = @sqlSelect + N'					or ';
	SET @sqlSelect = @sqlSelect + N'					(RTRIM(item.label_sub) <> RTRIM(dwh.name_sub) COLLATE SQL_Latin1_General_CP1_CI_AS)';
	SET @sqlSelect = @sqlSelect + N'				then 2 '; 
	SET @sqlSelect = @sqlSelect + N'				else 0 end '; 
	SET @sqlSelect = @sqlSelect + N'			else 1 end as update_flg ';
	SET @sqlSelect = @sqlSelect + N'			from ' + @pObjAPCSPro + N'.[mc].[item_labels] AS item with (NOLOCK) ';
	SET @sqlSelect = @sqlSelect + N'				left outer join ' + @pObjAPCSProDWH + N'.[dwh].[dim_alarm_levels] as dwh with (NOLOCK) ';
	SET @sqlSelect = @sqlSelect + N'					on dwh.code = item.val ';
	SET @sqlSelect = @sqlSelect + N'			where RTRIM(item.name) = ''model_alarms.alarm_level'' ';
			...
			BEGIN
				...
				IF @update_flg = 1	--INSERT
					BEGIN
		 				...
		 				SET @sqlTreat = N'';
						SET @sqlTreat = @sqlTreat + N'insert into ' + @pObjAPCSProDWH + N'.[dwh].[dim_alarm_levels] ';
						SET @sqlTreat = @sqlTreat + N'	(code ';
						SET @sqlTreat = @sqlTreat + N'	,name_eng ';
						SET @sqlTreat = @sqlTreat + N'	,name_jpn ';
						SET @sqlTreat = @sqlTreat + N'	,name_sub) ';
						SET @sqlTreat = @sqlTreat + N' values ';
						SET @sqlTreat = @sqlTreat + N'	(';
						SET @sqlTreat = @sqlTreat + convert(varchar, @code) ;
						--2019-09-12 modify
						--SET @sqlTreat = @sqlTreat + N' ,''' + RTRIM(@name_eng) + N'''';
						--SET @sqlTreat = @sqlTreat + N' ,''' + RTRIM(@name_jpn) + N'''';
						SET @sqlTreat = @sqlTreat + N' ,N''' + RTRIM(@name_eng) + N'''';
						SET @sqlTreat = @sqlTreat + N' ,N''' + RTRIM(@name_jpn) + N'''';
						--SET @sqlTreat = @sqlTreat + N' ,''' + RTRIM(@name_sub) + N'''';
						if @name_sub is null
							SET @sqlTreat = @sqlTreat + N' ,null';
						else
							SET @sqlTreat = @sqlTreat + N' ,N''' + RTRIM(@name_sub) + N'''';
						SET @sqlTreat = @sqlTreat + N')';
						--print @sqltreat;
					...
					BEGIN
						SET @sqlTreat = N'';
						SET @sqlTreat = @sqlTreat + N'update ' + @pObjAPCSProDWH + N'.[dwh].[dim_alarm_levels] WITH (ROWLOCK) ';
						--2019-09-12 modify
						--SET @sqlTreat = @sqlTreat + N'	set name_eng=''' + RTRIM(@name_eng) + N'''';
						--SET @sqlTreat = @sqlTreat + N'	,name_jpn=''' + RTRIM(@name_jpn) + N'''';
						SET @sqlTreat = @sqlTreat + N'	set name_eng=N''' + RTRIM(@name_eng) + N'''';
						SET @sqlTreat = @sqlTreat + N'	,name_jpn=N''' + RTRIM(@name_jpn) + N'''';
						if @name_sub is null
							SET @sqlTreat = @sqlTreat + N' ,name_sub=null';
						else
							SET @sqlTreat = @sqlTreat + N' ,name_sub=N''' + RTRIM(@name_sub) + N'''';
						SET @sqlTreat = @sqlTreat + N' where code=' + convert(varchar, @code) ;
					END;
