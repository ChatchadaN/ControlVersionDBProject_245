...
CREATE PROCEDURE [etl].[sp_etlwipcontrol_main] 
...
BEGIN TRY
													...
													(
														...
														from 
																...
																(
																	...
																	where substring(l.lot_no,5,1) = 'A' 
																		and l.wip_state <= 20
		...
		set @logtext = 'INSERT monitoring_item_records  ' + '@partno:1 :[OK] row:' + convert(varchar,@@ROWCOUNT);
		print '---- ' + @logtext + ' ---' ;
		...
		from  APCSProDWH.wip_control.numbers as n with (ROWLOCK)
			cross join 
				(
					select 
						count(t7.monitoring_item_id) as cnt 
					from 
							(
								select 
										t6.monitoring_item_id
		...
		set @logtext = 'INSERT monitoring_item_records  ' + '@partno:3 :[OK] row:' + convert(varchar,@@ROWCOUNT);
		--set @rowcnt = @@ROWCOUNT;
		--set @logtext = 'INSERT monitoring_item_records  ' + '@partno' + convert(varchar,@partno) + ' :[OK] row:' + convert(varchar,@rowcnt);
		print @logtext;
