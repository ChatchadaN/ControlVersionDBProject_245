...
CREATE PROCEDURE [act].[sp_machine_commonfilter_mc_v2] @package_group_id INT = NULL
	,@package_id INT = NULL
	,@process_id INT = NULL
	,@job_id INT = NULL
...
BEGIN
	...
	--Package Group
	SELECT t3.package_group_id AS id
		,t3.package_group_name AS name
	FROM (
		SELECT *
		FROM #table AS t2
		WHERE (
	...
	GROUP BY t3.package_group_id
		,t3.package_group_name
	ORDER BY name;
	...
	FROM APCSProDWH.dwh.dim_locations AS dl WITH (NOLOCK)
	ORDER BY dl.name;
