...
CREATE PROCEDURE [act].[sp_productionipi_wip_transition_forpkg_v2] @package_id INT = NULL
	,@device_id INT = NULL
...
BEGIN
	...
	--DECLARE @target_device NVARCHAR = 'BM'
	DECLARE @from INT = (
			SELECT id
				...
				FROM (
					...
					FROM (
						SELECT wi.day_id AS day_id
							...
							,dense_RANK() OVER (
								PARTITION BY wi.new_day_id ORDER BY wi.tmp_hour_code DESC
								) AS latest_hour_code
						...
						FROM (
							...
							FROM (
								SELECT dd.day_id AS day_id
									...
									,CASE 
										WHEN dd.hour_code - @time_offset <= 0
											THEN dd.hour_code - @time_offset + 24
										ELSE dd.hour_code - @time_offset
								...
								WHERE dd.day_id BETWEEN @from - 1
									...
									AND (
										(
											dd.day_id = @to
											AND dd.hour_code <= @finish_hour_code
											)
										OR dd.day_id <> @to
										)
								) AS d
							LEFT OUTER JOIN (
								...
								FROM (
									...
									INNER JOIN APCSProDB.method.device_names AS d WITH (NOLOCK) ON d.id = fw.device_id
									WHERE fw.day_id BETWEEN @from - 1
											AND @to
										...
										AND fw.process_state_code NOT IN (
											2
											,102
											)
									) AS t
						...
						LEFT OUTER JOIN APCSProDWH.dwh.dim_jobs AS dj WITH (NOLOCK) ON dj.id = wi.job_id
						) AS t1
	...
	LEFT OUTER JOIN APCSProDWH.dwh.dim_days AS dd WITH (NOLOCK) ON dd.id = t5.day_id
	ORDER BY t5.day_id
END
