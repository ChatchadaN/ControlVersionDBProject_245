...
CREATE PROCEDURE [act].[sp_productionmain_04] (
...
BEGIN
	...
	BEGIN
		...
		FROM (
			...
			FROM (
				...
				FROM act.fnc_LeadTime_united(@package_group_id, @package_id, @process_id, @job_id, @device_id, @device_name, @from, @to, @N) AS lt
				LEFT OUTER JOIN APCSProDWH.dwh.dim_days AS dd ON dd.id = lt.new_day_id
				) AS t1
			) AS t2
	...
	BEGIN
		...
		LEFT OUTER JOIN APCSProDWH.dwh.dim_days AS dd ON dd.id = TEMP.new_day_id
	...
	BEGIN
		...
		LEFT OUTER JOIN APCSProDWH.dwh.dim_days AS dd ON dd.id = t3.day_id
		LEFT OUTER JOIN APCSProDWH.dwh.dim_hours AS dh ON dh.code = t3.hour_code
		ORDER BY day_id,
			hour_code
	END
END
