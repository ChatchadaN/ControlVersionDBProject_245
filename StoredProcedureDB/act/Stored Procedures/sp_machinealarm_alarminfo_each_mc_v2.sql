...
CREATE PROCEDURE [act].[sp_machinealarm_alarminfo_each_mc_v2] (
...
BEGIN
	...
	FROM (
		SELECT series.machine_id AS machine_id
			...
			,series.alarm_id_x AS model_alarm_id
		...
		FROM (
			...
			FROM (
				SELECT CASE 
						WHEN @unit_type_duration = 0
							THEN fn.alarm_id_cnt
						ELSE fn.alarm_id_duration
						END AS alarm_id_x
					,fn.selected_alarm
					,v.machine_id
				FROM [act].fnc_machinealarm_alarm_top_rank_v2(@package_group_id, @package_id, @process_id, @job_id, @device_id, @device_name, @local_date_from, @local_date_to, @machine_id_list, @alarm_level, @alarm_level_alarm, @alarm_level_warning, @alarm_level_caution, @alarm_id_list, @top_num, @unit_type_duration, @include_selected_alarm, @lot_id, @process_job_id, @id_from, @id_to) AS fn
			...
			INNER JOIN APCSProDWH.dwh.dim_machines AS dm WITH (NOLOCK) ON dm.id = v1.machine_id
			INNER JOIN APCSProDWH.dwh.dim_machine_models AS mm WITH (NOLOCK) ON mm.id = dm.machine_model_id
			INNER JOIN APCSProDB.mc.model_alarms AS ma WITH (NOLOCK) ON ma.id = v1.alarm_id_x
		...
		LEFT OUTER JOIN (
			...
			) AS a_data ON a_data.model_alarm_id = series.alarm_id_x
			AND a_data.machine_id = series.machine_id
		) AS t
	LEFT OUTER JOIN [act].fnc_machinealarm_alarm_top_rank_v2(@package_group_id, @package_id, @process_id, @job_id, @device_id, @device_name, @local_date_from, @local_date_to, @machine_id_list, @alarm_level, @alarm_level_alarm, @alarm_level_warning, @alarm_level_caution, @alarm_id_list, @top_num, @unit_type_duration, @include_selected_alarm, @lot_id, @process_job_id, @id_from, @id_to) AS tai ON tai.alarm_id_cnt = t.model_alarm_id
		AND @unit_type_duration = 0
	LEFT OUTER JOIN [act].fnc_machinealarm_alarm_top_rank_v2(@package_group_id, @package_id, @process_id, @job_id, @device_id, @device_name, @local_date_from, @local_date_to, @machine_id_list, @alarm_level, @alarm_level_alarm, @alarm_level_warning, @alarm_level_caution, @alarm_id_list, @top_num, @unit_type_duration, @include_selected_alarm, @lot_id, @process_job_id, @id_from, @id_to) AS tai2 ON tai2.alarm_id_duration = t.model_alarm_id
