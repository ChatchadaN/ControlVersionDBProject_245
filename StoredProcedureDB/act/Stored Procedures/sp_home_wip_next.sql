...
CREATE PROCEDURE [act].[sp_home_wip_next] @date_from DATE = ''
	,@date_to DATE = ''
	--@hour_now INT 
AS
BEGIN
	...
	SET @hour = (
			...
			WHERE
				--h = @hour_now
				h = (
					SELECT (convert(INT, val) + DATEPART(hh, GETUTCDATE())) % 24
	...
	SELECT t2.day_id
		...
		t2.delay_state_code
		,t2.total_lot_count
		,ROUND(t2.total_pcs, - 3) / 1000 AS total_kpcs
		,CASE 
			WHEN t2.total_lot_count > 0
				THEN ROUND(CONVERT(FLOAT, t2.pcs_sum * 100) / t2.total_pcs, 1)
			ELSE 0
			END AS delay_rate_pcs
		,CASE 
			WHEN t2.total_lot_count > 0
				THEN ROUND(CONVERT(FLOAT, t2.lot_count_sum) * 100 / t2.total_lot_count, 1)
			ELSE 0
			END AS delay_rate_lot_count
	FROM (
		SELECT t1.*
			,SUM(t1.lot_count_sum) OVER (
	...
	WHERE t2.delay_state_code = 10
	ORDER BY t2.delay_state_code
		,t2.day_id ASC
END
