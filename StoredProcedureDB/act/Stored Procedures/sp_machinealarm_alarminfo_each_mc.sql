...
CREATE PROCEDURE [act].[sp_machinealarm_alarminfo_each_mc] (
...
BEGIN
	...
	SELECT row_number() OVER (
		...
		,
		--row_number() OVER (
		--	PARTITION BY t.machine_id ORDER BY t.alarm_text_id
		--	) AS pid_mc,
		--tai.rank_percent_alarm_cnt as pid_mc,
		CASE 
	...
	FROM (
		...
		FROM (
			...
			LEFT OUTER JOIN apcsprodwh.dwh.dim_machines AS dm WITH (NOLOCK) ON dm.machine_model_id = ac.machine_model_id
			LEFT OUTER JOIN APCSProDWH.dwh.dim_alarm_texts AS atx WITH (NOLOCK) ON atx.id = ac.alarm_text_id
			WHERE (',' + @top_alarm_ids + ',' LIKE '%,' + cast(atx.id AS VARCHAR) + ',%')
				AND dm.id IN (
					SELECT value
		...
		LEFT OUTER JOIN (
			...
			) AS a_data ON a_data.machine_id = series.machine_id
			AND a_data.model_alarm_id = series.alarm_text_id
		) AS t
	LEFT OUTER JOIN [act].fnc_machinealarm_alarm_top_rank(@package_group_id, @package_id, @process_id, @job_id, @device_id, @device_name, @local_date_from, @local_date_to, @machine_id_list, @alarm_level, @alarm_level_alarm, @alarm_level_warning, @alarm_level_caution, @alarm_id_list, @top_num, @unit_type_duration, @include_selected_alarm, @lot_id, @process_job_id) AS tai ON tai.alarm_id_cnt = t.alarm_text_id
		AND @unit_type_duration = 0
	LEFT OUTER JOIN [act].fnc_machinealarm_alarm_top_rank(@package_group_id, @package_id, @process_id, @job_id, @device_id, @device_name, @local_date_from, @local_date_to, @machine_id_list, @alarm_level, @alarm_level_alarm, @alarm_level_warning, @alarm_level_caution, @alarm_id_list, @top_num, @unit_type_duration, @include_selected_alarm, @lot_id, @process_job_id) AS tai2 ON tai2.alarm_id_duration = t.alarm_text_id
