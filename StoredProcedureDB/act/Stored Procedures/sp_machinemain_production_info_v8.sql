...
CREATE PROCEDURE [act].[sp_machinemain_production_info_v8] (
	@date_from DATETIME
	,@date_to DATETIME
...
AS
BEGIN
	--DECLARE @date_from DATETIME = '2022-06-26 00:00:00'
	--DECLARE @date_to DATETIME = '2022-08-07 00:00:00'
	...
	DECLARE @local_date_to DATETIME = CASE 
			...
			END
	DECLARE @local_machine_id INT = convert(INT, @machine_id_list)
	...
	FROM (
		...
		WHERE pj.machine_id = @local_machine_id
			AND (
				(
	...
	FROM (
		...
		FROM (
			...
			WHERE r.machine_id = @local_machine_id
					...
					FROM (
						SELECT t1.*
							...
							,CASE 
								...
								END AS lotend_flag
							,CASE 
								WHEN rec.record_class = 6 -- cancel
									THEN 1
								WHEN rec.record_class = 23 --abnormal end before proess
									THEN 2
								ELSE 0
								END AS cancel_flag
	...
	FROM (
	SELECT a1.*
		,max(a1.alarm_off_at)over(order by a1.alarm_on_at rows between unbounded preceding and current row) as max_alarm_off_at
	FROM (
	SELECT al.id
		,al.machine_id
		,al.updated_at
	...
	WHERE le.machine_id IS NULL
	 ...
	 where a2.alarm_off_at>=a2.max_alarm_off_at
	...
	SELECT ROW_NUMBER() OVER (
		...
		--,t1.qty_in / nullif(t1.process_time, 0) AS uph_act
		,CASE 
			WHEN t1.finished_at <> @cur_date
				THEN t1.qty_in / nullif(t1.process_time, 0)
			ELSE NULL
			END AS uph_act
		,convert(DECIMAL(9, 1), t1.qty_pass) / nullif((t1.qty_in), 0) * 100 AS yield
		,t1.qty_in / nullif(max(CASE 
					WHEN t1.run_state = 4
						THEN t1.sum_each_state_diff
					ELSE 0
					END) OVER (
				PARTITION BY t1.machine_id
				,t1.lot_id
		...
		,CASE 
			WHEN t1.is_special_flow = 0
				THEN CASE 
						WHEN t1.cancel_flag = 1
							THEN rtrim(t1.lot_no) + N' (cancel)'
						WHEN t1.cancel_flag = 2
							THEN rtrim(t1.lot_no) + N' (abnormal end)'
						ELSE rtrim(t1.lot_no)
						END
			ELSE rtrim(t1.lot_no) + N' (special_flow)'
			END AS lot_no_ex
	...
	INNER JOIN APCSProDB.method.packages AS mp WITH (NOLOCK) ON mp.id = tl.act_package_id
