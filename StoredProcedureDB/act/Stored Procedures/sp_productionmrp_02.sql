...
CREATE PROCEDURE [act].[sp_productionmrp_02] (
	...
	FROM (
		SELECT t2.day_id AS day_id
			...
			CASE 
				...
				ELSE max(isnull(t2.start_lots, 0)) OVER (PARTITION BY t2.day_id)
				END AS start_lots
			,CASE 
				WHEN @hour_flag = 1
					THEN isnull(convert(FLOAT, t2.start_pcs) / 1000, 0)
				ELSE max(isnull(convert(FLOAT, t2.start_pcs) / 1000, 0)) OVER (PARTITION BY t2.day_id)
				END AS start_Kpcs
			,
			--end lots/kpocs
			CASE 
		...
		FROM (
			...
			LEFT OUTER JOIN (
				SELECT fs.day_id AS day_id
					...
					,RANK() OVER (
						PARTITION BY fs.day_id ORDER BY fs.hour_code DESC
						) AS latest_hour_code
					,
					--TODO fact_endに変更する際、Lot数取得はsumからcountに。
					--count(fs.lot_count) AS lots,
					sum(fs.lot_count) AS lots
					,sum(fs.pcs) AS pcs
				FROM (
					SELECT day_id AS day_id
						,fs.hour_code AS hour_code
						,fs.lot_count AS lot_count
						,fs.pcs AS pcs
					FROM apcsprodwh.dwh.fact_wip AS fs WITH (NOLOCK)
				...
				AND (
					(
						@hour_flag = 0
						AND fs3.latest_hour_code = 1
						)
					OR (
						@hour_flag = 1
						AND fs3.latest_hour_code > 0
						)
					)
			--------------------------------------------end----------------------
			LEFT OUTER JOIN (
