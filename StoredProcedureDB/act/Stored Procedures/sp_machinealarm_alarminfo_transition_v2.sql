...
CREATE PROCEDURE [act].[sp_machinealarm_alarminfo_transition_v2] (
	...
	FROM (
		...
		FROM (
			...
			FROM (
										...
										ELSE
											...
											CASE 
												WHEN t.alarm_on_at < t.alarm_off_at
													THEN t.alarm_off_at
												WHEN t.alarm_on_at < t.started_at
													THEN t.started_at
												ELSE t.updated_at
												END
				...
				LEFT OUTER JOIN APCSProDWH.dwh.dim_alarm_texts AS atx WITH (NOLOCK) ON atx.id = t1.alarm_text_id
				) AS t2
			) AS t3
		WHERE t3.day_rank = 1
		) AS t4
