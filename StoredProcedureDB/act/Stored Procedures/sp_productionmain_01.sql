...
CREATE PROCEDURE [act].[sp_productionmain_01] (
...
BEGIN
	...
	BEGIN
		SELECT *
		INTO #t_weekly
		FROM [act].fnc_ProductionMain_01_weekly(@package_group_id, @package_id, @process_id, @device_id, @device_name, @from, @to, @span)
	...
	BEGIN
		SELECT *
		INTO #t_daily
		FROM [act].fnc_ProductionMain_01_daily(@package_group_id, @package_id, @process_id, @device_id, @device_name, @from, @to, @span)
		ORDER BY date_value;
	...
	BEGIN
		SELECT *
		INTO #t_shift
		FROM [act].fnc_ProductionMain_01_shift(@package_group_id, @package_id, @process_id, @device_id, @device_name, @from, @to, @span)
		ORDER BY id,
			shift_code
	...
	BEGIN
		...
		FROM (
			...
			FROM [act].fnc_ProductionMain_01_hours(@package_group_id, @package_id, @process_id, @device_id, @device_name, @from, @to, @span)
			) AS pm_temp
