...
CREATE PROCEDURE [act].[sp_home_machine_current_state] (
	@run_state INT = NULL
	,@now_on_alarm INT = - 1
	)
AS
BEGIN
	...
	FROM (
		SELECT t6.machine_id AS machine_id
			...
			,alm_rec.alarm_code AS alarm_code
	...
	WHERE (
			@now_on_alarm >= 0
			AND (t7.now_on_alarm = @now_on_alarm)
			)
		OR (
			@now_on_alarm < 0
			AND t7.now_on_alarm >= 0
			)
