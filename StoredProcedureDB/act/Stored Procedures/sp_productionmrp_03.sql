...
--sp_ProductionMRP_01のAロット対応
CREATE PROCEDURE [act].[sp_productionmrp_03] @package_group_id INT = NULL
	,@package_id INT = NULL
	,@process_id INT = NULL
	,@device_id INT = NULL
...
BEGIN
	...
	--DECLARE @all_jobs INT = 0;
	--declare @device_name nvarchar(32) = null;
	SELECT t5.package_group_id AS package_group_id
		...
		,t5.job_no AS job_no
		,CASE 
			WHEN t5.job_name IS NULL
				THEN t5.process_name
			ELSE t5.job_name
			END AS job_name
		,t5.production_category AS production_category
		...
		,t5.sum_not_A_lot AS sum_not_A_lot
		...
		,t5.sum_not_A_pcs_kpcs AS sum_not_A_pcs_kpcs
	...
	FROM (
		SELECT t4.package_group_id AS package_group_id
			...
			,t4.production_category AS production_category
			,CASE 
				WHEN @all_jobs = 1
					THEN t4.sum_lot
			...
			,CASE 
				WHEN @all_jobs = 1
					THEN t4.sum_not_A_lot
				ELSE sum(t4.sum_not_A_lot) OVER (
						PARTITION BY t4.process_id
						,t4.process_no
						)
				END AS sum_not_A_lot
			,CASE 
				WHEN @all_jobs = 1
			...
			,CASE 
				WHEN @all_jobs = 1
					THEN isnull(convert(FLOAT, t4.sum_not_A_pcs) / 1000, - 3)
				ELSE sum(isnull(convert(FLOAT, t4.sum_not_A_pcs) / 1000, - 3)) OVER (
						PARTITION BY t4.process_id
						,t4.process_no
						)
				END AS sum_not_A_pcs_kpcs
			,rank() OVER (
				PARTITION BY t4.process_id
		...
		FROM (
			SELECT t3.wip_id AS wip_id
				...
				,t3.pcs AS pcs
				,t3.production_category AS production_category
				,row_number() OVER (
				...
				,sum(t3.not_A_lot) OVER (PARTITION BY t3.job_id) AS sum_not_A_lot
				,sum(t3.not_A_pcs) OVER (PARTITION BY t3.job_id) AS sum_not_A_pcs
			FROM (
				SELECT t2.wip_id AS wip_id
					...
					,t2.lot_count * CASE 
						WHEN (
								(t2.production_category <> 0)
								OR (t2.production_category IS NULL)
								)
							THEN 1
						ELSE 0
						END AS not_A_lot
				...
				FROM (
					SELECT wip.id AS wip_id
						...
						,isnull(wip.production_category, 0) AS production_category
					FROM (
					...
					LEFT OUTER JOIN (
						SELECT fw.id AS id
							...
							,fw.pcs AS pcs
							,fw.production_category AS production_category
