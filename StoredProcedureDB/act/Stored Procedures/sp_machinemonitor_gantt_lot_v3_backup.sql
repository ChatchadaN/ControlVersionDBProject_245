...
CREATE PROCEDURE [act].[sp_machinemonitor_gantt_lot_v3_backup] (
	@date_from DATETIME
	,@date_to DATETIME
	,@time_offset INT = 0
...
BEGIN
				...
				FROM (
					...
					FROM (
						...
						FROM (
							...
							FROM (
								SELECT pj0.id AS process_job_id
									,pj0.machine_id AS machine_id
									,pj0.setup_at AS setup_at
									,pj0.started_at AS started_at
									,pj0.finished_at AS finished_at
									,CASE 
										WHEN @local_date_to < GETDATE()
											THEN @local_date_to
										ELSE GETDATE()
										END AS latest_date
									,isnull(convert(DECIMAL(9, 1), datediff(SECOND, @local_date_from, pj0.setup_at)) / 60 / 60, NULL) AS setup_point
									,pj0.finished_day_id AS finished_day_id
									,pj0.lot_id AS lot_id
									,datepart(hh, pj0.setup_at) AS setup_hour
								FROM (
									SELECT *
										,ROW_NUMBER() OVER (
									...
									WHERE (
											...
											(
												(pj.finished_at IS NOT NULL)
												AND (@date_from - 1 <= pj.finished_at)
												)
											OR (
												(@date_from - 1 <= pj.setup_at)
							...
							LEFT OUTER JOIN APCSProDB.trans.lot_process_records AS lpr WITH (NOLOCK) ON lpr.lot_id = pj1.lot_id
								AND lpr.process_job_id = pj1.process_job_id
								AND lpr.recorded_at = pj1.started_at
								AND lpr.record_class = 1
								AND lpr.machine_id = pj1.machine_id
							) AS t1
						WHERE (t1.setup_point <= datediff(hour, @local_date_from, @local_date_to))
						) AS t2
					) AS t3
				WHERE t3.pj_rank = 1
				) AS t4
	...
	SELECT dense_rank() OVER (
		...
		,tt.original_setup_at AS original_setup_at
	...
	LEFT OUTER JOIN (
		SELECT
			...
			,isnull(mil.label_eng, 'unknown') AS device_type_name
			...
			,s4.original_setup_at AS original_setup_at
		...
		FROM (
			SELECT s3.date_value AS date_value
				...
				,s3.original_setup_at AS original_setup_at
			...
			FROM (
				SELECT s2.date_value AS date_value
					...
					,s2.setup_at AS original_setup_at
		...
		LEFT JOIN APCSProDB.method.item_labels AS mil WITH (NOLOCK) ON mil.name = 'device_versions.device_type'
			AND mil.val = dv.device_type
