...
CREATE PROCEDURE [act].[sp_productionmrp_02_custom] (
...
BEGIN
	...
	FROM (
		...
		FROM (
			...
			WHERE process_state_code NOT IN (2, 102)
			) AS wi
	...
	SELECT isnull(tw.sum_lot_count, 0) AS init_lot,
		...
		isnull(tw.sum_lot_count_non_target, 0) AS init_lot_non_target,
		isnull(convert(FLOAT, tw.sum_pcs_non_target) / 1000, 0) AS init_pcs_non_target
	INTO #t_wip_init
	FROM #t_wip AS tw
	WHERE (day_id = @from - 1)
		AND (hour_code = 24);
	...
	FROM (
		...
		FROM (
			SELECT t1.day_id AS day_id,
				...
				wp3.wip_lots_non_target AS wip_lots_non_target,
				wp3.wip_pcs AS wip_pcs,
				wp3.wip_pcs_non_target AS wip_pcs_non_target,
			...
			LEFT OUTER JOIN (
				SELECT wp.day_id AS day_id,
					...
					wp.sum_lot_count_non_target AS wip_lots_non_target,
					wp.sum_pcs AS wip_pcs,
					wp.sum_pcs_non_target AS wip_pcs_non_target,
