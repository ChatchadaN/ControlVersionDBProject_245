...
CREATE FUNCTION [act].[fnc_fact_end_shift] (
...
BEGIN
	...
	SELECT t4.day_id AS day_id,
		CASE 
			WHEN t4.shift_code = 0
				THEN 8
			WHEN t4.shift_code = 1
				THEN 20
			END AS hour_code,
		isnull(t4.lot_count, 0) AS lot_count,
		isnull(t4.input_pcs, 0) AS pass_pcs,
		...
		CASE 
			WHEN count(t4.day_id) OVER (
				...
				THEN isnull(round(convert(FLOAT, sum(t4.lot_count) OVER (
									PARTITION BY t4.shift_code ORDER BY t4.day_id rows BETWEEN 6 preceding
											AND CURRENT row
		...
		CASE 
			WHEN count(t4.day_id) OVER (
				...
				THEN isnull(sum(t4.input_pcs) OVER (
							PARTITION BY t4.shift_code ORDER BY t4.day_id rows BETWEEN 6 preceding
									AND CURRENT row
							) / count(t4.day_id) OVER (
							PARTITION BY t4.shift_code ORDER BY t4.day_id rows BETWEEN 6 preceding
			...
			FROM (
				...
				LEFT OUTER JOIN (
					SELECT
						...
						rank() over (partition by fe.lot_id, fe.process_id,fe.job_id order by fe.day_id desc, fe.hour_code desc ) as lot_rank,
						fe.lot_id,
						fe.input_pcs,
						fe.std_time,
						fe.run_time,
