...
CREATE FUNCTION [act].[fnc_machinealarm_alarm_top_rank] (
					...
					FROM (
						...
						FROM (
							SELECT t1.*
								...
								,CASE 
									WHEN ax.message_text = ''
										THEN t1.machine_model_name + '*' + t1.alarm_code
									ELSE ax.message_text
									END AS message_text
								,row_number() OVER (
							...
							FROM (
								SELECT s3.id AS id
									...
									,s3.machine_model_name AS machine_model_name
									,s3.merge_alarm_id AS alarm_id
									,s3.alarm_code AS alarm_code
									,s3.on_at AS on_at
									,s3.off_at AS off_at
								...
								FROM (
									...
									FROM (
										...
										FULL OUTER JOIN (
												...
												,isnull(convert(DECIMAL(18, 1), datediff_big(SECOND, fmc.alarm_on_at, CASE 
																...
																ELSE
																	...
																	CASE 
																		...
																		WHEN fmc.alarm_on_at < fmc.started_at
																			THEN fmc.started_at
																		ELSE fmc.updated_at
																		END
											...
											LEFT OUTER JOIN APCSProDB.mc.model_alarms AS ac WITH (NOLOCK) ON ac.id = fmc.model_alarm_id
											...
											) AS s1 ON s1.alarm_text_id = my.value
										) AS s2
									) AS s3
								) AS t1
