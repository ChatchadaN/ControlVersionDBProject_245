...
CREATE FUNCTION [act].[fnc_ProductionMain_01_daily] (
...
BEGIN
	...
	BEGIN
		...
		LEFT OUTER JOIN (
			SELECT day_id,
				...
				LotAve7days,
				PcsAve7days
			FROM storedproceduredb.act.fnc_fact_input(@package_group_id, @package_id, @device_id, @device_name, @from, @to, 0)
		...
		LEFT OUTER JOIN (
			SELECT day_id,
				...
				LotAve7days,
				PcsAve7days
			FROM storedproceduredb.act.fnc_fact_shipment(@package_group_id, @package_id, @device_id, @device_name, @from, @to, 0)
			) AS shp ON shp.day_id = dd.id
		LEFT OUTER JOIN (
			SELECT day_id,
				sum_pcs
			FROM storedproceduredb.act.fnc_fact_wip(@package_group_id, @package_id, @process_id, @device_id, @device_name, @from, @to, 0)
	...
	BEGIN
		...
		LEFT OUTER JOIN (
			SELECT *
			FROM storedproceduredb.act.fnc_fact_start(@package_group_id, @package_id, @process_id, @device_id, @device_name, @from, @to, 0)
			) AS ipt ON ipt.day_id = dd.id
		LEFT OUTER JOIN (
			SELECT *
			FROM storedproceduredb.act.fnc_fact_end(@package_group_id, @package_id, @process_id, @device_id, @device_name, @from, @to, 0)
			) AS shp ON shp.day_id = dd.id
		LEFT OUTER JOIN (
			SELECT *
			FROM storedproceduredb.act.fnc_fact_wip(@package_group_id, @package_id, @process_id, @device_id, @device_name, @from, @to, 0)
