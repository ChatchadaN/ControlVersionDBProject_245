...
CREATE FUNCTION [act].[fnc_ProductionMain_01_weekly_v2] (
	...
	BEGIN
		...
		FROM (
			...
			FROM (
				...
				LEFT OUTER JOIN (
					SELECT *
					FROM storedproceduredb.act.fnc_fact_plan_v2(@package_group_id, @package_id, @device_id, @device_name, @from_s, @to_e, @time_offset)
					) AS pln ON pln.day_id = dd.id
				LEFT OUTER JOIN (
					SELECT *
					FROM storedproceduredb.act.fnc_fact_input_v2(@package_group_id, @package_id, @device_id, @device_name, @from_s, @to_e, 0, @time_offset)
					) AS ipt ON ipt.day_id = dd.id
				LEFT OUTER JOIN (
					SELECT *
					FROM storedproceduredb.act.fnc_fact_shipment_v2(@package_group_id, @package_id, @device_id, @device_name, @from_s, @to_e, 0, @time_offset)
					) AS shp ON shp.day_id = dd.id
				LEFT OUTER JOIN (
					SELECT *
					FROM storedproceduredb.act.fnc_fact_wip_v2(@package_group_id, @package_id, @process_id, @device_id, @device_name, @from_s, @to_e, 0, @time_offset)
	...
	BEGIN
		...
		FROM (
			...
			FROM (
				...
				LEFT OUTER JOIN (
					SELECT *
					FROM storedproceduredb.act.fnc_fact_start_v2(@package_group_id, @package_id, @process_id, @device_id, @device_name, @from_s, @to_e, 0, @time_offset)
					) AS ipt ON ipt.day_id = dd.id
				LEFT OUTER JOIN (
					SELECT *
					FROM storedproceduredb.act.fnc_fact_end_v2(@package_group_id, @package_id, @process_id, @device_id, @device_name, @from_s, @to_e, 0, @time_offset)
					) AS shp ON shp.day_id = dd.id
				LEFT OUTER JOIN (
					SELECT *
					FROM storedproceduredb.act.fnc_fact_capa_v2(@package_group_id, @package_id, @process_id, @device_id, @device_name, @from_s, @to_e, @time_offset)
