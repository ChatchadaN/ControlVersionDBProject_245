...
CREATE FUNCTION [act].[fnc_ProductionMain_02_shift_v2] (
	...
	FROM (
		...
		FROM (
			SELECT dd.id
				...
				--,isnull(convert(DECIMAL(12, 3), wip_all.sum_pcs) / 1000, 0) AS wip_all_Kpcs
				...
				,isnull(convert(DECIMAL(12, 3), max(wip_all.sum_pcs * (
								CASE 
									WHEN wip_all.delay_state_code = 0
										THEN 1
									ELSE 0
