...
create FUNCTION [act].[fnc_ProductionMain_02_daily_v2_backup] (
...
BEGIN
	...
	FROM (
		...
		LEFT OUTER JOIN (
			SELECT *
			FROM act.fnc_fact_wip_delay_v2(@package_group_id, @package_id, @process_id, @device_id, @device_name, @from, @to, 0, @time_offset)
