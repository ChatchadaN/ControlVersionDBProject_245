...
CREATE FUNCTION [act].[fnc_fact_wip_v2] (
...
BEGIN
	...
	SELECT t4.day_id AS day_id
		,t4.hour_code AS hour_code
		,t4.sum_lot_count AS sum_lot_count
		,t4.sum_pcs AS sum_pcs
	FROM (
		...
		FROM (
			SELECT t2.day_id
				...
				,t2.hour_code
				,CASE 
					WHEN t2.hour_code - @time_offset <= 0
						THEN t2.hour_code - @time_offset + 24
					ELSE t2.hour_code - @time_offset
