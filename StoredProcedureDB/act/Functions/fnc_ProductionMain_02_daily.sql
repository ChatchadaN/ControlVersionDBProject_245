...
CREATE FUNCTION [act].[fnc_ProductionMain_02_daily] (
...
BEGIN
	...
	FROM (
		...
		LEFT OUTER JOIN (
			SELECT *
			--FROM act.fnc_fact_wip_delay(@package_group_id, @package_id, @process_id, @device_id, @device_name,  @from, @to, 0)
			FROM act.fnc_fact_wip_delay_united(@package_group_id, @package_id, @process_id, @device_id, @device_name,  @from, @to, 0)
