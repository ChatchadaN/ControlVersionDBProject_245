...
CREATE FUNCTION [act].[fnc_fact_end_shift_v2] (
...
BEGIN
	...
	FROM (
		SELECT t3.new_day_id AS day_id
			...
			,sum(t3.input_pcs) AS input_pcs
			,avg(t3.std_time) AS std_time
			,avg(t3.run_time) AS run_time
		...
		FROM (
			SELECT t2.day_id AS day_id
				...
				,isnull(sum(t2.input_pcs), 0) AS input_pcs
