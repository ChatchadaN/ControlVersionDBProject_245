...
CREATE FUNCTION [act].[fnc_LeadTime_united] (
...
BEGIN
	...
	BEGIN
		...
		SELECT t2.day_id AS day_id,
			...
			CASE 
				WHEN (t2.hour_code < 8 + 1)
					OR (t2.hour_code > 20 + 1)
					THEN 1
				ELSE 0
				END AS shift_code,
			CASE 
				WHEN t2.is_assy_only IN (0, 1)
					THEN 0
				ELSE 1
		...
		FROM (
			...
			LEFT OUTER JOIN (
				...
				LEFT OUTER JOIN APCSProDB.method.device_slips AS ds ON ds.device_slip_id = tl.device_slip_id
				LEFT OUTER JOIN APCSProDB.method.device_names AS dn ON dn.id = fs.device_id
	...
	BEGIN
		...
		FROM (
			...
			LEFT OUTER JOIN (
				...
				LEFT OUTER JOIN APCSProDB.method.device_slips AS ds ON ds.device_slip_id = tl.device_slip_id
				LEFT OUTER JOIN APCSProDB.method.device_names AS dn ON dn.id = fe.device_id
				LEFT OUTER JOIN APCSProDB.method.device_flows AS df ON df.device_slip_id = tl.device_slip_id
