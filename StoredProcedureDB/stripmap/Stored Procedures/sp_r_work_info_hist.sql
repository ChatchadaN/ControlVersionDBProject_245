...
-- =============================================
CREATE PROCEDURE[stripmap].[sp_r_work_info_hist]
...
BEGIN
	...
	SET @CMD_TEXT += N'	' + 'LPR.process_state as PROCESS_STATE, ';
	...
	SET @CMD_TEXT += N'	' + 'WUR.recorded_at as RECORDED_AT, ';
	SET @CMD_TEXT += N'	' + 'case when WUR.id is null then LPR.id else WUR.id end as HISTORY_ID, ';
	...
	SET @CMD_TEXT += N'from ' + @DATABASE_NAME + '.trans.work_update_records as WUR with(nolock) ';
	SET @CMD_TEXT += N'inner join ' + @DATABASE_NAME + '.trans.works as WK with(nolock) on WK.id = WUR.work_id ';
	...
	SET @CMD_TEXT += N'  select ';
	SET @CMD_TEXT += N'  max(LPR.id) as max_id, WK.id as work_id, WUR.id as workrecord_id ';
	SET @CMD_TEXT += N'  from ' + @DATABASE_NAME + '.trans.lot_process_records as LPR with(nolock) ';
	SET @CMD_TEXT += N'  inner join ' + @DATABASE_NAME + '.trans.works as WK with(nolock) on WK.lot_id = LPR.lot_id ';
	SET @CMD_TEXT += N'  inner join ' + @DATABASE_NAME + '.trans.work_update_records as WUR with(nolock) on WUR.work_id = WK.id and WUR.recorded_at >= LPR.recorded_at ';
	SET @CMD_TEXT += N'  group by WK.id , WUR.id) as TMP on TMP.workrecord_id = WUR.id and TMP.work_id = WK.id ';
	SET @CMD_TEXT += N'inner join ' + @DATABASE_NAME + '.trans.lot_process_records as LPR with(nolock) on LPR.id = TMP.max_id ';
