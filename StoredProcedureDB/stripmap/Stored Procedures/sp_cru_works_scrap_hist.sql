...
-- =============================================
CREATE PROCEDURE[stripmap].[sp_cru_works_scrap_hist]
...
BEGIN
	...
	SET @CMD_TEXT += N'WK SET ';
	SET @CMD_TEXT += N' ' + 'WK.map_state = 9, ';
	SET @CMD_TEXT += N' ' + 'WK.updated_at = GETDATE(), ';
	SET @CMD_TEXT += N' ' + 'WK.updated_by = ' + CONVERT(varchar,@USER_ID) + ' ';
