...
-- =============================================
CREATE PROCEDURE[stripmap].[sp_cru_countup_numbers_trans]
...
BEGIN
	...
	SET @CMD_TEXT += N'from ' + @DATABASE_NAME + '.trans.numbers as NU ';
	...
	IF @CURRENT_NUM is NULL
		BEGIN
			...
			SET @CMD_TEXT += N'	' + @DATABASE_NAME + '.trans.numbers(id, name) ';
			SET @CMD_TEXT += N'select ';
