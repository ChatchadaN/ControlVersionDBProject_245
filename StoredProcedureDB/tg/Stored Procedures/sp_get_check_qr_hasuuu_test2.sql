...
-- =============================================
CREATE PROCEDURE [tg].[sp_get_check_qr_hasuuu_test2]
...
BEGIN
	...
	select getdate()
		...
		, host_name()
		, app_name()
		, 'exec start [tg].[sp_get_check_qr_hasuuu] @lot_no = ''' + isnull(cast(@lot_no as varchar(10)),'NULL') 
	...
	DECLARE @qr_lot_no varchar(10)
		...
		, @get_lot_by_eslno varchar(10) 

	--Add condition 2025/02/07 time : 17.57 by Aomsin
	...
	--set @qr_qty = ''
	set @qr_reel = substring(@qrcode_detail,37,2) -- 2023-07-19
	--set @qr_reel = ''
	...
	begin
		select @db_version = substring(qrcode_detail,36,1)
			...
			--, @db_qty =  ''
			, @db_reel = substring(qrcode_detail,37,2) -- 2023-07-19
			--, @db_reel = ''
	...
	begin
		...
		begin
			...
			--set @db_qty =  ''
			set @db_reel = substring(@qrcode_detail,37,2) -- 2023-07-19
			--set @db_reel =  ''
		end
		else
		begin
