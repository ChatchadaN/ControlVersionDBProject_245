...
-- =============================================
CREATE PROCEDURE [rans].[sp_get_scheduler_temp_mp] 
...
BEGIN
	...
	--	,[device_name_change]
	--	,[jobs].[name] AS [flow]
	--	,[lot_wip_plans].[created_at]
	--	,[lot_wip_plans].[created_by]
	--	,[lot_wip_plans].[updated_at]
	--	,[lot_wip_plans].[updated_by]
	--FROM [APCSProDWH].[rans].[lot_wip_plans] WITH (NOLOCK)
	--LEFT JOIN [APCSProDB].[mc].[machines] WITH (NOLOCK) ON [lot_wip_plans].[machine_id] = [machines].[id]
	--LEFT JOIN [APCSProDB].[method].[jobs] WITH (NOLOCK) ON [lot_wip_plans].[job_id] = [jobs].[id]
	--LEFT JOIN [APCSProDB].[trans].[locations] WITH (NOLOCK) ON [lot_wip_plans].[location_id_lot_no_2] = [locations].[id]
	...
	--FROM [APCSProDWH].[rans].[lot_wip_plans] WITH (NOLOCK)
	--LEFT JOIN [APCSProDB].[trans].[lots] WITH (NOLOCK) ON [lot_wip_plans].[lot_no_1] = [lots].[lot_no]
	--LEFT JOIN [APCSProDB].[mc].[machines] WITH (NOLOCK) ON [lot_wip_plans].[machine_id] = [machines].[id]
	--LEFT JOIN [APCSProDB].[method].[jobs] WITH (NOLOCK) ON [lot_wip_plans].[job_id] = [jobs].[id]
	--LEFT JOIN [APCSProDB].[trans].[locations] WITH (NOLOCK) ON [lot_wip_plans].[location_id_lot_no_2] = [locations].[id]
	--LEFT JOIN [APCSProDB].[mc].[models] WITH (NOLOCK) ON [machines].[machine_model_id] = [models].[id]
