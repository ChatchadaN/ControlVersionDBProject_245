...
-- =============================================
CREATE PROCEDURE [rans].[sp_set_scheduler_temp_mp_ver_temp] 
...
BEGIN
	...
	LEFT JOIN [APCSProDB].[rcs].[rack_addresses] ON [machine_location_settings].[location_id] = rack_addresses.id
	...
	BEGIN
		DELETE FROM [APCSProDWH].[rans].[lot_wip_plans_01] WHERE [process] = 'MP';
		PRINT(format(getdate(), 'yyyy-MM-dd HH:mm:ss.fff') + ' ----> delete table APCSProDWH.rans.lot_wip_plans_01 <---- ');
		...
		PRINT(format(getdate(), 'yyyy-MM-dd HH:mm:ss.fff') + ' ----> insert table APCSProDWH.rans.lot_wip_plans_01 <---- ');
	END
