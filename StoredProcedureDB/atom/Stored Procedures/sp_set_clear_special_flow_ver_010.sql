...
-- =============================================
CREATE PROCEDURE [atom].[sp_set_clear_special_flow_ver_010]
...
BEGIN
	...
	SELECT GETDATE()
		...
		, HOST_NAME()
		, APP_NAME()
		, 'EXEC [atom].[sp_set_clear_special_flow_ver_010] @lot_id = ''' + ISNULL( CAST( @lot_id AS VARCHAR ), '' ) 
	...
	SELECT GETDATE() --AS [record_at]
		...
		, APP_NAME() --AS [appname]
		, N'[StoredProcedureDB].[atom].[sp_set_clear_special_flow_ver_010]' --AS [storedprocedname]
				...
				BEGIN
					...
					BEGIN
						...
						BEGIN
							...
							--# Update QTY [trans].[lots]
							UPDATE [lots]
							SET [lots].[qty_pass] = [special_flows].[qty_pass]
								, [lots].[qty_fail] = ISNULL([special_flows].[qty_fail],0) + ISNULL([lots].[qty_fail],0)
								, [lots].[qty_last_pass] = [special_flows].[qty_last_pass]
								, [lots].[qty_last_fail] = [special_flows].[qty_last_fail]
								, [lots].[qty_pass_step_sum] = [special_flows].[qty_pass_step_sum]
								, [lots].[qty_fail_step_sum] = ISNULL([special_flows].[qty_fail_step_sum],0) + ISNULL([lots].[qty_fail_step_sum],0)
							FROM [APCSProDB].[trans].[lots]
