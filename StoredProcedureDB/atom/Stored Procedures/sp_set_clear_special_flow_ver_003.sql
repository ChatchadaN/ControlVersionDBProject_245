...
-- =============================================
CREATE PROCEDURE [atom].[sp_set_clear_special_flow_ver_003]
...
BEGIN
	...
	SELECT GETDATE()
		...
		, HOST_NAME()
		, APP_NAME()
		, 'EXEC [atom].[sp_set_clear_special_flow_3] @lot_id = ''' + ISNULL(CAST(@lot_id AS varchar),'') + ''', @special_id = ''' + ISNULL(CAST(@special_id AS varchar),'') + ''', @lot_special_id = ''' 
