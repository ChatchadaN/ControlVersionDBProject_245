...
-- =============================================
CREATE PROCEDURE [atom].[sp_set_recall_lot_in_tranlot_002_backup20231011]
...
BEGIN
	...
	SELECT GETDATE()
		...
		, HOST_NAME()
		, APP_NAME()
		, ISNULL('EXEC [atom].[sp_set_recall_lot_in_tranlot_002] @new_lotno = ''' + @new_lotno + '''','EXEC [atom].[sp_set_recall_lot_in_tranlot_002] @new_lotno = NULL')
	...
	BEGIN TRY
		...
		IF EXISTS (
			-- check slip recall device_type = 7 --
			SELECT [lot_no]
			FROM (
				SELECT [lots].[lot_no]
				...
				INNER JOIN [APCSProDB].[method].[device_versions] WITH (NOLOCK) ON [device_versions].[device_name_id] = [device_names].[id] 
					AND [device_versions].[device_type] = 7 
			...
			BEGIN
				...
				FROM (
					SELECT @new_lotno AS [lot_no]
						...
						, [device_names].[package_id] AS [act_package_id]
						, [device_names].[id] AS [act_device_name_id]
						, [device_flows].[device_slip_id] AS [device_slip_id]
						, NULL AS [order_id]
						...
						, [device_flows].[step_no] AS [start_step_no]
						, GETDATE() AS [created_at]
						, @empid AS [created_by]
						, [lots].[lot_no] AS [external_lot_no]
						, 70 AS [production_category]
						, ROW_NUMBER() OVER (ORDER BY [device_slips].[version_num] DESC) AS [row]
						, ROW_NUMBER() OVER (PARTITION BY [device_slips].[version_num] ORDER BY [device_slips].[version_num] DESC,[device_flows].[step_no] ASC) AS [row_step]
					FROM [APCSProDB].[trans].[lots] WITH (NOLOCK)
		...
		BEGIN ---- not exists ----
			...
			COMMIT TRANSACTION;
			SELECT 'FALSE' AS [Is_Pass] 
				, 'Slip recall not register !!' AS [Error_Message_ENG]
				, N'Slip recall ยังไม่ได้ถูกลงทะเบียน !!' AS [Error_Message_THA] 
				, N'กรุณาติดต่อ system' AS [Handling];
			RETURN;
			-----------------------------------------------------------------------------
		END ---- not exists ---
