...
-- =============================================
CREATE PROCEDURE [atom].[sp_set_clear_special_flow_backup20230803]
...
BEGIN
	...
	SELECT GETDATE()
		...
		, HOST_NAME()
		, APP_NAME()
		, 'EXEC [atom].[sp_set_clear_special_flow_new] @lot_id = ''' + ISNULL(CAST(@lot_id AS varchar),'') + ''', @special_id = ''' + ISNULL(CAST(@special_id AS varchar),'') + ''', @lot_special_id = ''' 
...
--GO
--/****** Object:  StoredProcedure [atom].[sp_set_clear_special_flow]    Script Date: 1/7/2022 3:25:28 PM ******/
--SET ANSI_NULLS ON
--GO
...
---- Description:	<Description,,>
---- =============================================
--ALTER PROCEDURE [atom].[sp_set_clear_special_flow]
--	-- Add the parameters for the stored procedure here
--	@lot_id int,
--	@special_id int,
--	@flowfon int = 0
...
--		, HOST_NAME()
--		, APP_NAME()
--		, 'EXEC [atom].[sp_set_clear_special_flow] @lot_id = ''' + ISNULL(CAST(@lot_id AS varchar),'') + ''', @special_id = ''' + ISNULL(CAST(@special_id AS varchar),'')  
--			+ ''', @flowfon = ''' + ISNULL(CAST(@flowfon AS varchar),'') +''''
--		, (select cast(lot_no as varchar) from [APCSProDB].[trans].[lots] where id = @lot_id)
--	-->> log exec
