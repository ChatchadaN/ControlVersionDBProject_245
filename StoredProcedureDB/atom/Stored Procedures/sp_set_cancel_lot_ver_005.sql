...
CREATE PROCEDURE [atom].[sp_set_cancel_lot_ver_005]
...
BEGIN
	...
	SELECT GETDATE()
		...
		, HOST_NAME()
		, APP_NAME()
		, 'EXEC [atom].[sp_set_cancel_lot_ver_005] @lot_id = ' + ISNULL(CAST(@lot_id AS VARCHAR),'') 
	...
	-- DECLARE
	-----------------------------------------------------------------
	DECLARE @update_at VARCHAR(50) = GETDATE()
