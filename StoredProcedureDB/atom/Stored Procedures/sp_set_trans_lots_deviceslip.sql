...
-- =============================================
CREATE PROCEDURE [atom].[sp_set_trans_lots_deviceslip] 
...
BEGIN
	...
	select @PACKAGE_ID=packages.id,@DEVICE_NAME_ID=device_names.id
	from
	[APCSProDB].[method].device_slips 
	...
	where device_slips.device_slip_id = @DEVICE_SLIP_ID

	UPDATE 
		[APCSProDB].trans.lots
	SET
		[APCSProDB].trans.lots.act_package_id = @PACKAGE_ID,
		[APCSProDB].trans.lots.act_device_name_id = @DEVICE_NAME_ID,
		[APCSProDB].trans.lots.device_slip_id = @DEVICE_SLIP_ID
