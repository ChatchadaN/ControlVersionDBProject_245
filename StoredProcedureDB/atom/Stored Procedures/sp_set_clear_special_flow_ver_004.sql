...
-- =============================================
CREATE PROCEDURE [atom].[sp_set_clear_special_flow_ver_004]
...
BEGIN
	...
	SELECT GETDATE()
		...
		, HOST_NAME()
		, APP_NAME()
		, 'EXEC [atom].[sp_set_clear_special_flow_v_4] @lot_id = ''' + ISNULL(CAST(@lot_id AS varchar),'') 
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					----------<<< delete lot_process_records 25
					delete from [APCSProDB].[trans].[lot_process_records]
					WHERE [lot_id] = @lot_id 
						and [record_class]  in (25,4)
						and [step_no] = @step_no;
					---------->>> delete lot_process_records 25 step_no
					--------------------------------------------------------------
				...
				ELSE BEGIN
					...
					----------<<< update lot_process_records 25 step_no
					UPDATE [APCSProDB].[trans].[lot_process_records]
					SET [step_no] = [step_no] - 1
					WHERE [lot_id] = @lot_id 
						and [record_class] = 25
						and [step_no] between (@step_no + 1) and (
