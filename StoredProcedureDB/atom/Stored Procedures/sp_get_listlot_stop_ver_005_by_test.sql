...
-- =============================================
CREATE PROCEDURE [atom].[sp_get_listlot_stop_ver_005_by_test] 
...
BEGIN
	...
	BEGIN
		...
		--	AND ISNULL([lot_hold_controls].[is_held],0) = 0;
	...
	BEGIN
		...
		--		, ROW_NUMBER() OVER (PARTITION BY [lot_id], [stop_step_no] ORDER BY [lot_id],[stop_instruction_id] DESC) AS [count_step]
		...
		--    AND [lot_stop_instructions].[step_no] = [device_flows].[step_no] 
		LEFT JOIN [APCSProDB].[trans].[lot_hold_controls] ON [lot_hold_controls].[lot_id] = [lots].[id]
			AND [lot_hold_controls].[system_name] = 'lot stop instruction'
		CROSS APPLY (
			SELECT TOP 1 [lot_id] 
