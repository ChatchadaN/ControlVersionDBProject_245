...
-- =============================================
CREATE PROCEDURE [atom].[sp_set_next_flow_ver_003]
...
BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					SELECT GETDATE()
						...
						, HOST_NAME()
						, APP_NAME()
						, 'EXEC [atom].[sp_set_next_flow_ver_003] update special flow is now(1) special_flow_id = ' + IIF(CAST(IIF(@id IS NULL,-1,@id) AS VARCHAR(20)) = -1,'NULL',CAST(IIF(@id IS NULL,-1,@id) AS VARCHAR(20)))
						, @lot_no
					--------------------------------------------------------------
				...
				BEGIN
					...
					SELECT GETDATE()
						...
						, HOST_NAME()
						, APP_NAME()
						, 'EXEC [atom].[sp_set_next_flow_ver_003] update special flow is after(0) special_flow_id = ' + IIF(CAST(IIF(@id IS NULL,-1,@id) AS VARCHAR(20)) = -1,'NULL',CAST(IIF(@id IS NULL,-1,@id) AS VARCHAR(20)))
						, @lot_no
					--------------------------------------------------------------
					SELECT 'TRUE' AS Status
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						SELECT GETDATE()
							...
							, HOST_NAME()
							, APP_NAME()
							, 'EXEC [atom].[sp_set_next_flow_ver_003] update qty trans.lots to trans.special_flows'
							, @lot_no
					END
				END
				...
				SELECT GETDATE()
					...
					, HOST_NAME()
					, APP_NAME()
					, 'EXEC [atom].[sp_set_next_flow_ver_003] not update special flow (1)'
					, @lot_no
				--------------------------------------------------------------
				SELECT 'TRUE' AS Status
	...
	BEGIN CATCH 
		...
		SELECT GETDATE()
			...
			, HOST_NAME()
			, APP_NAME()
			, 'EXEC [atom].[sp_set_next_flow_ver_003] error update'
			, @lot_no
		--------------------------------------------------------------
