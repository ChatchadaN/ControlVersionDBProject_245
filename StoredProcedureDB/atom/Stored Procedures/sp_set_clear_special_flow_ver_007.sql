...
-- =============================================
CREATE PROCEDURE [atom].[sp_set_clear_special_flow_ver_007]
...
BEGIN
	...
	SELECT GETDATE()
		...
		, HOST_NAME()
		, APP_NAME()
		, 'EXEC [atom].[sp_set_clear_special_flow_ver_007] @lot_id = ''' + ISNULL( CAST( @lot_id AS VARCHAR ), '' ) 
	...
	SELECT GETDATE() --AS [record_at]
		...
		, APP_NAME() --AS [appname]
		, N'[StoredProcedureDB].[atom].[sp_set_clear_special_flow_ver_007]' --AS [storedprocedname]
	...
	DECLARE @status_step_no INT = 0, --# 0:ไม่เจอ step_no  1:master  2:special
	...
	---# (3) CHECK STEP NO
	-->>-------------------------------------------------------------------------
	--- (3.1) CHECK STEP NO TYPE 0:ไม่เจอ step_no  1:master  2:special
	IF ( EXISTS( SELECT 1 FROM [APCSProDB].[method].[device_flows] WHERE [device_slip_id] = ( SELECT [device_slip_id] FROM [APCSProDB].[trans].[lots] WHERE [lots].[id] = @lot_id ) 
			AND [step_no] = @step_no 
			AND is_skipped != 1 ) )
	BEGIN
		---หา step_no จาก master
		SET @status_step_no = 1; --# 1:master
	END
	ELSE IF ( EXISTS( SELECT 2 FROM [APCSProDB].[trans].[special_flows]
	...
	BEGIN
		...
		SET @status_step_no = 2; --# 2:special
	END
	ELSE
	BEGIN
		SET @status_step_no = 0; --# 0:ไม่เจอ step_no
	END
	--- (3.2) CHECK STEP NO DELETE 0:ลบไม่ได้ 1:ลบได้
	...
	INNER JOIN [APCSProDB].[trans].[special_flows] ON [lots].[is_special_flow] = 1
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						WHERE [special_flow_id] = @special_id
							AND [step_no] = ( SELECT MAX( [step_no] ) FROM [APCSProDB].[trans].[lot_special_flows] WHERE [special_flow_id] = @special_id );
