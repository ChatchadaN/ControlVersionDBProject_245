...
-- =============================================
CREATE PROCEDURE [atom].[sp_set_trans_special_flow_ver_010_backup20230801]
...
BEGIN
	...
	select GETDATE()
		...
		, HOST_NAME()
		, APP_NAME()
		, 'exec [atom].[sp_set_trans_special_flow_ver_010] @lot_id = ''' + ISNULL(CAST(@lot_id AS varchar),'') 
	...
	SELECT GETDATE() --AS [record_at]
		...
		, APP_NAME() --AS [appname]
		, N'[StoredProcedureDB].[atom].[sp_set_trans_special_flow_ver_010]' --AS [storedprocedname]
