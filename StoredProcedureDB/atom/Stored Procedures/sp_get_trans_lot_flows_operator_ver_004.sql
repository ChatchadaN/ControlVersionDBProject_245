...
-- =============================================
CREATE PROCEDURE [atom].[sp_get_trans_lot_flows_operator_ver_004]
...
BEGIN
	...
	begin
		if exists(select * from @table where flow = 0 and job_name in ('TSUGITASHI'))
		begin
			set @step_no_mix = (select top 1 step_no from @table where flow = 0 and job_name in ('TSUGITASHI') order by step_no asc);
		end
		else if exists(select * from @table where flow = 0 and job_name in ('TP-TP','TP','FL','FT-TP','FLFTTP'))
		begin
		...
		else if exists(select * from @table where flow = 1 and job_name in ('TP-TP','TP','FL','FT-TP','FLFTTP'))
		begin
