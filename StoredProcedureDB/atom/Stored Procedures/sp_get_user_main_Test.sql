...
-- =============================================
CREATE  PROCEDURE [atom].[sp_get_user_main_Test]	-- Add the parameters for the stored procedure here	
	@lot_no varchar(10) = '%'
...
BEGIN
	...
	FROM (select 
			...
			--,case when lots.process_state in(0,100) then   
			--(select top 1 [recorded_at] 			
			--	FROM [APCSProDB].[trans].[lot_process_records]
			--	inner join [APCSProDB].[trans].[item_labels] as [recode_class] on [recode_class].[name] = 'lot_process_records.record_class' and [recode_class].[val] = [lot_process_records].record_class and [lot_process_records].record_class = 2
			--	where lot_id = [lots].[id]
			--	order by [lot_process_records].step_no desc
			--) when lots.process_state in (1,101) then 
			--(select top 1 [recorded_at] 			
			--	FROM [APCSProDB].[trans].[lot_process_records]
			--	inner join [APCSProDB].[trans].[item_labels] as [recode_class] on [recode_class].[name] = 'lot_process_records.record_class' and [recode_class].[val] = [lot_process_records].record_class and [lot_process_records].record_class = 5
			--	where lot_id = [lots].[id]
			--	order by [lot_process_records].step_no desc
			--) when lots.process_state in (2,102) then
			--(select top 1 [recorded_at]
			--	FROM [APCSProDB].[trans].[lot_process_records]
			--	inner join [APCSProDB].[trans].[item_labels] as [recode_class] on [recode_class].[name] = 'lot_process_records.record_class' and [recode_class].[val] = [lot_process_records].record_class and [lot_process_records].record_class = 1
			--	where lot_id = [lots].[id]
			--	order by [lot_process_records].step_no desc
			--)
			--else [lots].[updated_at] end as update_time
			, [lots].[updated_at] as update_time
			...
			---** delete **---
			--, case when [lot_hold_controls].is_held is not null then [lot_hold_controls].is_held else NULL end as [is_held]
			--, case when [lot_stop_instructions].is_finished is not null then [lot_stop_instructions].is_finished else NULL end as [is_finished]
			---** delete **---
		...
		--left join [APCSProDB].[trans].[lot_stop_instructions] on [lot_stop_instructions].lot_id = lots.id
		---** delete **---
		...
		--left join (
		--	SELECT [lot_no],[fab_wf_lot_no] 
		--	FROM [APCSProDB].[robin].[lot1_table_input] with (NOLOCK) 
		--	WHERE SUBSTRING([lot_no],1,2) > 20
		--) as [fabwafer]  on [fabwafer].lot_no = [lots].lot_no
	...
	--	and [item_labels2].[label_eng] like @process_state
	--	and [item_labels3].[label_eng] like @quality_state
	--	and [lots].[wip_state] in ('10','20','0')
