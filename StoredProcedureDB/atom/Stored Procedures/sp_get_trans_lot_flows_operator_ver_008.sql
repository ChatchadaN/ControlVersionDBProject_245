...
-- =============================================
CREATE PROCEDURE [atom].[sp_get_trans_lot_flows_operator_ver_008]
	...
	LEFT JOIN ( ----# join sub query StepNoDetail
		...
		INNER JOIN (
			...
			SELECT [lot_process_records].[id]
				, [lot_process_records].[record_class]
				, [lot_process_records].[step_no]
				, ISNULL([lpr].[qty_in], [lot_process_records].[qty_in] ) AS [qty_in]
				, ISNULL([lpr].[qty_pass], [lot_process_records].[qty_pass] ) AS [qty_pass]
				, ISNULL([lpr].[qty_fail], [lot_process_records].[qty_fail] ) AS [qty_fail]
				, ISNULL([lpr].[qty_last_pass], [lot_process_records].[qty_last_pass] ) AS [qty_last_pass]
				, ISNULL([lpr].[qty_pass_step_sum], [lot_process_records].[qty_pass_step_sum] ) AS [qty_pass_step_sum]
				, ISNULL([lpr].[qty_fail_step_sum], [lot_process_records].[qty_fail_step_sum] ) AS [qty_fail_step_sum]
				, ISNULL([lpr].[qty_frame_in], [lot_process_records].[qty_frame_in] ) AS [qty_frame_in]
				, ISNULL([lpr].[qty_frame_pass], [lot_process_records].[qty_frame_pass] ) AS [qty_frame_pass]
				, ISNULL([lpr].[qty_frame_fail], [lot_process_records].[qty_frame_fail] ) AS [qty_frame_fail]
				, ISNULL([lpr].[qty_p_nashi], [lot_process_records].[qty_p_nashi] ) AS [qty_p_nashi]
				, ISNULL([lpr].[qty_combined], [lot_process_records].[qty_combined] ) AS [qty_combined]
				, ISNULL([lpr].[qty_hasuu], [lot_process_records].[qty_hasuu] ) AS [qty_hasuu]
				, ISNULL([lpr].[qty_out], [lot_process_records].[qty_out] ) AS [qty_out]
				, ISNULL([lpr].[qty_front_ng], [lot_process_records].[qty_front_ng] ) AS [qty_front_ng]
				, ISNULL([lpr].[qty_marker], [lot_process_records].[qty_marker] ) AS [qty_marker] 
				, ISNULL([lpr].[qty_cut_frame], [lot_process_records].[qty_cut_frame] ) AS [qty_cut_frame]
				, ISNULL([lpr].[carrier_no], [lot_process_records].[carrier_no] ) AS [carrier_no]
				, ISNULL([lpr].[next_carrier_no], [lot_process_records].[next_carrier_no] ) AS [next_carrier_no]
				, ISNULL([lpr].[machine_id], [lot_process_records].[machine_id] ) AS [machine_id]
				, ISNULL([lpr].[job_id], [lot_process_records].[job_id] ) AS [job_id]
				, ISNULL([lpr].[updated_by], [lot_process_records].[updated_by] ) AS [user_id]
			FROM [APCSProDB].[trans].[lot_process_records]
			OUTER APPLY ( ----# join LotEnd
				SELECT TOP 1 *
				FROM [APCSProDB].[trans].[lot_process_records] AS [lpr]
				WHERE [lpr].[record_class] = 2  ----# 2:LotEnd
					AND [lpr].[lot_id] = @lot_id
					AND [lpr].[step_no] = [lot_process_records].[step_no]
