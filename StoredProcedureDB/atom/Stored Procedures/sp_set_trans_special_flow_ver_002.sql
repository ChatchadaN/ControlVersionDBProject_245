...
-- =============================================
CREATE PROCEDURE [atom].[sp_set_trans_special_flow_ver_002]
...
BEGIN
	...
	SELECT GETDATE()
		...
		,HOST_NAME()
		,APP_NAME()
		,'EXEC [atom].[sp_set_trans_special_flow_v2] @lot_id = ''' + CAST(@lot_id AS varchar) + ''', @step_no = ''' + CAST(@step_no AS varchar) + ''', @back_step_no = ''' 
