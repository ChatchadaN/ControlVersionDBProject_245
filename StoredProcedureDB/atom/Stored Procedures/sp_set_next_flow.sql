...
CREATE PROCEDURE [atom].[sp_set_next_flow]
...
BEGIN
	...
	BEGIN TRY 
		...
		BEGIN
			...
			DECLARE @special_flow_id_update INT = NULL
				...
				, @step_no INT = NULL
				, @step_flow INT = NULL
				, @step_flow_master INT = NULL
				, @step_no_min INT = NULL
			...
			----# (1.1) SET DATA STEP_NO, IS_SPECIAL_FLOW, STEP_NO_MASTER ,STEP_NO_MIN
			SELECT @step_no = ISNULL([lot_special_flows].[step_no], [lots].[step_no])
			...
			SELECT @step_no_min = MIN([lot_special_flows].[step_no]) 
			FROM [APCSProDB].[trans].[lots] 
			...
			INNER JOIN [APCSProDB].[trans].[lot_special_flows] 
				ON [special_flows].[id] = [lot_special_flows].[special_flow_id]
			WHERE [lots].[id] = @lot_id;
			----# (1.1) END SET DATA STEP_NO, IS_SPECIAL_FLOW, STEP_NO_MASTER ,STEP_NO_MIN
			...
			BEGIN
				...
				BEGIN
					...
					SELECT GETDATE()
						...
						, HOST_NAME()
						, APP_NAME()
						, 'EXEC [atom].[sp_set_next_flow_ver_004] update special flow is now(1) special_flow_id = ' + IIF(CAST(IIF(@id IS NULL,-1,@id) AS VARCHAR(20)) = -1,'NULL',CAST(IIF(@id IS NULL,-1,@id) AS VARCHAR(20)))
						, @lot_no
					--------------------------------------------------------------
				...
				BEGIN
					...
					SELECT GETDATE()
						...
						, HOST_NAME()
						, APP_NAME()
						, 'EXEC [atom].[sp_set_next_flow_ver_004] update special flow is after(0) special_flow_id = ' + IIF(CAST(IIF(@id IS NULL,-1,@id) AS VARCHAR(20)) = -1,'NULL',CAST(IIF(@id IS NULL,-1,@id) AS VARCHAR(20)))
						, @lot_no
					--------------------------------------------------------------
					SELECT 'TRUE' AS Status
			...
			ELSE
			BEGIN
				----# UPDATE INTO SPECIAL FLOW STEP 1
				IF (@step_no = @step_no_min) --AND @lot_no = '1234A1234V'
				BEGIN
					UPDATE [APCSProDB].[trans].[special_flows]
					SET [special_flows].[qty_in] = [lots].[qty_pass]
						, [special_flows].[qty_pass] = [lots].[qty_pass]
						, [special_flows].[qty_fail] = 0
						, [special_flows].[qty_last_pass] = NULL
						, [special_flows].[qty_last_fail] = NULL
						, [special_flows].[qty_pass_step_sum] = NULL
						, [special_flows].[qty_fail_step_sum] = NULL
						, [special_flows].[qty_divided] = NULL
						, [special_flows].[qty_combined] = [lots].[qty_combined]
						, [special_flows].[qty_hasuu] = [lots].[qty_hasuu]
						, [special_flows].[qty_out] = [lots].[qty_out]
						, [special_flows].[qty_frame_in] = [lots].[qty_frame_pass]
						, [special_flows].[qty_frame_pass] = [lots].[qty_frame_pass]
						, [special_flows].[qty_frame_fail] = [lots].[qty_frame_fail]
						, [special_flows].[qty_frame_last_pass] = NULL
						, [special_flows].[qty_frame_last_fail] = NULL
						, [special_flows].[qty_frame_pass_step_sum] = NULL
						, [special_flows].[qty_frame_fail_step_sum]	= NULL
					...
					BEGIN
						...
						SELECT GETDATE()
							...
							, HOST_NAME()
							, APP_NAME()
							, 'EXEC [atom].[sp_set_next_flow_ver_004] update qty trans.lots to trans.special_flows'
							, @lot_no
					END
				END
				...
				SELECT GETDATE()
					...
					, HOST_NAME()
					, APP_NAME()
					, 'EXEC [atom].[sp_set_next_flow_ver_004] not update special flow (1)'
					, @lot_no
				--------------------------------------------------------------
				SELECT 'TRUE' AS Status
			...
			----# (3) SET CHECK STOP LOT
			--EXEC [StoredProcedureDB].[atom].[sp_set_stop_lot_by_auto] @lot_no = @lot_no;
			EXEC [StoredProcedureDB].[atom].[sp_set_stop_lot_by_auto_ver_002] @lot_no = @lot_no;
			----# (3) SET CHECK STOP LOT
	...
	BEGIN CATCH 
		...
		SELECT GETDATE()
			...
			, HOST_NAME()
			, APP_NAME()
			, 'EXEC [atom].[sp_set_next_flow_ver_004] error update'
			, @lot_no
		--------------------------------------------------------------
