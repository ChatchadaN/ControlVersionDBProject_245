...
-- =============================================
CREATE PROCEDURE [atom].[sp_get_listlot_stop_ver_002] 
...
BEGIN
	...
	SELECT [id]
		...
		, [is_held]
		, [is_finished]
	FROM (select distinct [lots].[id] as id
			...
			--for list function about stop lot/lock lot
		...
		WHERE ([day_indate].[date_value] <= convert(date, getdate())
			...
			and [lots].[wip_state] in (10,20,0)) OR [lots].[lot_no] in ('9999A9999V','9999D9999V','1234A1234V')

	) as TableAtom
	WHERE [TableAtom].[lot_no] like '%'+@lot_no+'%'
	order by [lot_no]
END
