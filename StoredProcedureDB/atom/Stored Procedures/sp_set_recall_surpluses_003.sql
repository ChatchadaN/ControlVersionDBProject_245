...
CREATE PROCEDURE [atom].[sp_set_recall_surpluses_003]
	-- Add the parameters for the stored procedure here
	@new_lotno VARCHAR(10)
	, @original_lotno VARCHAR(10)
	, @empid INT
	, @pc_instruction_code INT -- 20251008 By Bussamas
AS
BEGIN
	...
	SELECT GETDATE()
		...
		, HOST_NAME()
		, APP_NAME()
		, ISNULL('EXEC [atom].[sp_set_recall_surpluses_003] @new_lotno = ''' + @new_lotno + '''','EXEC [atom].[sp_set_recall_surpluses_002] @new_lotno = NULL')
			+ ISNULL(', @original_lotno = ''' + @original_lotno + '''',', @original_lotno = NULL')
			+ ISNULL(', @empid = ' + CAST(@empid AS VARCHAR),', @empid = NULL')
			+ ISNULL(', @pc_instruction_code = ' + CAST(@pc_instruction_code AS VARCHAR),', @pc_instruction_code = NULL') -- 20251008 By Bussamas
		, @new_lotno;
	----------------------------------------------------------------------------
	----- # create lot in trans.surpluses
	----------------------------------------------------------------------------
	DECLARE @trans_lots_id INT
		, @trans_lots_original_id INT
		, @date_mix DATETIME
		, @qty_hasuu INT
	...
	SELECT @trans_lots_original_id = [id] 
	FROM [APCSProDB].[trans].[lots] WITH (NOLOCK) 
	WHERE [lot_no] = @original_lotno;
	...
	BEGIN TRY
		...
		BEGIN
			...
			SELECT @trans_surpluses_id AS [id]
				...
				-- , 2 AS [in_stock]
				, CASE 
					WHEN @pc_instruction_code = 11 THEN 0
					ELSE 2
				END AS [in_stock] -- 20251008 By Bussamas
				, NULL AS [location_id]
