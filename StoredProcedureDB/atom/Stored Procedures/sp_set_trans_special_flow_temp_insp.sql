...
-- =============================================
CREATE PROCEDURE [atom].[sp_set_trans_special_flow_temp_insp]
...
BEGIN
	...
	DECLARE @first_step_no INT = NULL;
	DECLARE @back_step_no INT;
	DECLARE @user_id INT = 1;
	DECLARE @flow_pattern_id INT;
	...
	select @lot_id = id, @back_step_no = step_no, @device_slip_id = device_slip_id 
	from [APCSProDB].[trans].[lots] 
	where [lot_no] = @lot_no and process_state in (0,100) and quality_state != 4 and is_special_flow != 1

	SELECT @first_step_no = [step_no]
	FROM [APCSProDB].[method].[device_flows]
	where device_slip_id  = @device_slip_id
	and next_step_no = @back_step_no
	and step_no < next_step_no
	and is_skipped = '0'

	IF EXISTS(select [special_flows].[id]
		...
		and [lot_special_flows].[step_no] > @first_step_no
		and [lot_special_flows].[step_no] < @back_step_no)
	BEGIN
		select @step_no = MAX([lot_special_flows].[step_no])
		from [APCSProDB].[trans].[special_flows]
		inner join [APCSProDB].[trans].[lot_special_flows] on [lot_special_flows].[special_flow_id] = [special_flows].[id]
		where [special_flows].[lot_id] = @lot_id
		and [lot_special_flows].[step_no] > @first_step_no
	...
	IF (@lot_id is not null and @step_no is not null)
	BEGIN
