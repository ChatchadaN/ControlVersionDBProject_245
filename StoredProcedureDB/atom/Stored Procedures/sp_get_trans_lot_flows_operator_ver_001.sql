...
-- =============================================
CREATE PROCEDURE [atom].[sp_get_trans_lot_flows_operator_ver_001]
...
BEGIN
	...
	(
		...
		, [LotDetail].[emp_num]
		--, [LotStart].[StartTime]
		, case when [LotDetail].[record_class] in (46,47) then [TG_record].[Time] else [LotStart].[StartTime] end as [StartTime]
		...
		--, [LotSetup].[SetupTime]
		, case when [LotDetail].[record_class] in (46,47) then [TG_record].[Time] else case when ([LotStart].[StartTime] <= [LotEnd].[EndTime]) then cast([LotEnd].[EndTime] as datetime2) end end [EndTime]
		, case when [LotDetail].[record_class] in (46,47) then [TG_record].[Time] else [LotSetup].[SetupTime] end as [SetupTime] 
		from
		(
		...
		(
			...
			from [APCSProDB].[trans].[lot_process_records]
			where [record_class] IN (46,47) and [lot_id] = @lot_id
			---->> new join by bass 04/07/2022
		) as TG_record
		on LotDetail.step_no = TG_record.step_no and LotDetail.machine_id = TG_record.machine_id
			and TG_record.rowmax = 1 ----new join by bass 04/07/2022
	...
	outer apply (
		...
		where lot_id =  @lot_id
			...
			and job_id = [device_flows].[job_id]
	) as [ocr_record]
		outer apply (
		select top 1 job_id
		from  [APCSProDB].[trans].[lot_process_records]
	...
	outer apply (
		...
		where lot_id =  @lot_id
			and record_class = 130
			and job_id = [lot_special_flows].[job_id]
	) as [ocr_record]
	outer apply (
		select top 1 job_id
		from  [APCSProDB].[trans].[lot_process_records]
