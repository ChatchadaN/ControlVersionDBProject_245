...
CREATE PROCEDURE [atom].[sp_set_recall_lot_in_tranlot_004]
...
BEGIN
	...
	SELECT GETDATE()
		...
		, HOST_NAME()
		, APP_NAME()
		, ISNULL('EXEC [atom].[sp_set_recall_lot_in_tranlot_004] @new_lotno = ''' + @new_lotno + '''','EXEC [atom].[sp_set_recall_lot_in_tranlot_003] @new_lotno = NULL')
			...
			+ ISNULL(', @standard_qty = ' + CAST(@standard_qty AS VARCHAR),', @standard_qty = NULL')
			+ ISNULL(', @pc_instruction_code = ' + CAST(@pc_instruction_code AS VARCHAR),', @pc_instruction_code = NULL')
		, @new_lotno;
	----------------------------------------------------------------------------
		...
		BEGIN ---- exists ----
			...
			BEGIN
				...
				INSERT INTO [APCSProDB].[trans].[lots]
					...
					, [external_lot_no]
					, [production_category]
					, [pc_instruction_code])  /* 12 / 9 / 2025 Recall add pc_instruction_code by Bussamas */
				SELECT @trans_lots_id AS [id]
					...
					, [lots].[pc_instruction_code]  /* 12 / 9 / 2025 Recall add pc_instruction_code by Bussamas */
				FROM (
					SELECT @new_lotno AS [lot_no]
						...
						, 0 AS [qty_divided]
						--, (@qty % (@standard_qty * (@qty/@standard_qty))) AS [qty_hasuu]
						--, (@standard_qty * (@qty/@standard_qty)) AS [qty_out]
						, IIF(@qty < @standard_qty,@qty,(@qty % (@standard_qty * (@qty/@standard_qty)))) AS [qty_hasuu]
						, IIF(@qty < @standard_qty,0,(@standard_qty * (@qty/@standard_qty))) AS [qty_out] 
						, 0 AS [is_exist_work]
						...
						, @pc_instruction_code AS [pc_instruction_code]   /* 12 / 9 / 2025 Recall add pc_instruction_code by Bussamas */
					FROM [APCSProDB].[trans].[lots] WITH (NOLOCK)
					CROSS APPLY (
						SELECT [device_slips].*
						FROM (
