...
-- =============================================
CREATE PROCEDURE [atom].[sp_set_trans_special_flow_ver_009]
...
BEGIN
	...
	select GETDATE()
		...
		, HOST_NAME()
		, APP_NAME()
		, 'exec [atom].[sp_set_trans_special_flow_ver_009] @lot_id = ''' + ISNULL(CAST(@lot_id AS varchar),'') 
	...
	begin try
			...
			begin
				...
				begin
					...
					begin
						...
						where [lot_id] = @lot_id 
							and [record_class] = 25
							and [lot_process_records].[step_no] between (@step_no + 1) and ((@step_no + 1) + @count_flow);
					end
					...
					inner join [APCSProDB].[trans].[numbers] as nu on [nu].[name] = 'lot_process_records.id'
					inner join [APCSProDB].[trans].[lot_special_flows] on [lot_special_flows].[special_flow_id] = @update_spid
						and [lot_special_flows].[step_no] between (@step_no + 1) and ((@step_no + 1) + (@count_flow - 1))
					where [lots].[id] = @lot_id;
		...
		begin
			...
			select 'TRUE' AS Is_Pass 
				, 'Add special flow sucess.' AS Error_Message_ENG
	...
	begin catch
		...
		select GETDATE()
			...
			, HOST_NAME()
			, APP_NAME()
			, 'rollback exec [atom].[sp_set_trans_special_flow_ver_009] @lot_id = ''' + ISNULL(CAST(@lot_id AS varchar),'') 
				+ ''', @is_special_flow = ''' + ISNULL(CAST(@is_special_flow AS varchar),'')
				+ ''', @step_no = ''' + ISNULL(CAST(@step_no AS varchar),'') 
				+ ''', @flow_pattern_id = '''+ ISNULL(CAST(@flow_pattern_id AS varchar),'') 
				+ ''', @machine_id = ''' + ISNULL(CAST(@machine_id AS varchar),'') 
