...
CREATE PROCEDURE [atom].[sp_set_stop_lot_by_auto_ver_002_bk] 
	...
	BEGIN
		...
		BEGIN
			...
			SELECT GETDATE()
				...
				, HOST_NAME()
				, APP_NAME()
				, 'EXEC [atom].[sp_set_stop_lot_by_auto_ver_002] update is_held = 0 (stop lot) lot_no = ' + CAST(@lot_no AS VARCHAR(20))
				, @lot_no;
		END
	END
