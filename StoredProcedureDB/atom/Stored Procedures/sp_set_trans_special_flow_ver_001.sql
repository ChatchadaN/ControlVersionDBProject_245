...
-- =============================================
CREATE PROCEDURE [atom].[sp_set_trans_special_flow_ver_001]
...
BEGIN
	...
	SELECT GETDATE()
		...
		,HOST_NAME()
		,APP_NAME()
		,'EXEC [atom].[sp_set_trans_special_flow] @lot_id = ''' + CAST(@lot_id AS varchar) + ''', @step_no = ''' + CAST(@step_no AS varchar) + ''', @back_step_no = ''' 
	...
	ELSE
		...
		SET
		  [special_flows].[created_at] = GETDATE()
	    , [special_flows].[updated_at] = GETDATE()
		, [special_flows].[updated_by] = @user_id
	...
	INNER JOIN [APCSProDB].[trans].[lot_special_flows] ON [lot_special_flows].[special_flow_id] = [lots].special_flow_id
	WHERE [lots].[id] = @lot_id
