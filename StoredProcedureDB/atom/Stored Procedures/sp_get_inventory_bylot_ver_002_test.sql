...
-- =============================================
CREATE PROCEDURE [atom].[sp_get_inventory_bylot_ver_002_test]
...
BEGIN
	...
	BEGIN 
		...
		END
		---- # IF 2 have instock != 2
		ELSE IF  EXISTS ( SELECT 'xx' FROM [APCSProDB].[trans].[surpluses] WHERE [lot_id] = @lot_id AND [in_stock] not in (2,3))
		BEGIN 
			---- # ----------------------------------------------------------- # ----
