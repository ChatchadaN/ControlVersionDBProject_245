...
-- =============================================
CREATE PROCEDURE [atom].[sp_set_stop_lot_ver_004] 
...
BEGIN
	...
	SELECT GETDATE()
		...
		, HOST_NAME()
		, APP_NAME()
		, 'exec [atom].[sp_set_stop_lot_ver_004] @lot_id = ''' + ISNULL(CAST(@lot_id AS varchar),'') 
	...
	BEGIN TRY
		...
		BEGIN 
			...
			WHERE [lots].[id] = @lot_id AND process_state = 0; 
			-----------------------------------------------------------------
			...
			DECLARE @process_state AS INT,
					@next_step AS INT;
			SELECT @process_state = [lots].[process_state], @next_step = [device_flows].[next_step_no] FROM [APCSProDB].[trans].[lots] 
			INNER JOIN [APCSProDB].[method].[device_flows] ON 
			[APCSProDB].[trans].[lots].[device_slip_id] = [APCSProDB].[method].[device_flows].[device_slip_id] AND
			[APCSProDB].[trans].[lots].[step_no] = [APCSProDB].[method].[device_flows].[step_no] 
			WHERE [lots].[id] =  @lot_id;
