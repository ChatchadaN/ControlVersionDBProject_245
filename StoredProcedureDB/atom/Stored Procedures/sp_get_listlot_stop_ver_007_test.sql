...
-- =============================================
CREATE PROCEDURE [atom].[sp_get_listlot_stop_ver_007_test] 
...
BEGIN
	...
	(
		...
		[fabwf] VARCHAR(20),
		[is_combine] INT NULL,
		[abnormal_mode] NVARCHAR(255) NULL,
		[is_have_hasuu] INT NULL
	)
	...
	BEGIN
		...
		SELECT [lots].[id] AS [id] 
			...
			, [job_stop].[name] AS [operation_stop]
		...
		CROSS APPLY (
			SELECT TOP 1 [lot_id] 
				...
				, [comments].[val] AS [abnormal_mode]
			FROM [APCSProDB].[trans].[lot_stop_instructions] WITH (NOLOCK)
			LEFT JOIN [APCSProDB].[trans].[comments] ON [lot_stop_instructions].[display_message_id] = [comments].[id]
		...
		LEFT JOIN [APCSProDB].[method].[device_flows] AS [device_flow_stop] WITH (NOLOCK) ON [device_flow_stop].[device_slip_id] = [lots].[device_slip_id]
			AND [device_flow_stop].[step_no] = [lot_stop_instructions].[step_no]
		LEFT JOIN [APCSProDB].[method].[jobs] AS [job_stop] WITH (NOLOCK) ON [device_flow_stop].[job_id] = [job_stop].[id]
	...
	BEGIN
		...
		CROSS APPLY (
			...
			CROSS APPLY (
				...
				LEFT JOIN [APCSProDB].[trans].[comments] ON [lsi].[display_message_id] = [comments].[id]
