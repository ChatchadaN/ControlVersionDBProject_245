...
-- =============================================
CREATE PROCEDURE [atom].[sp_set_trans_special_flow_ver_013]
...
BEGIN
	...
	SELECT GETDATE() --AS [record_at]
		...
		, 'EXEC [StoredProcedureDB].[atom].[sp_set_trans_special_flow_ver_013] @lot_id = ' + ISNULL( CAST( @lot_id AS VARCHAR ), 'NULL' ) 
	...
	SELECT GETDATE() --AS [record_at]
		...
		, HOST_NAME() --AS [hostname]
		, APP_NAME() --AS [appname]
		, N'[StoredProcedureDB].[atom].[sp_set_trans_special_flow_ver_013]' --AS [storedprocedname]
		, ( SELECT CAST( [lot_no] AS VARCHAR ) FROM [APCSProDB].[trans].[lots] WHERE [id] = @lot_id ) --AS [lot_no]
	...
	DECLARE @table_sblsyl TABLE (
		[job_id] INT,
		[is_sblsyl] TINYINT,
		[sbl_upper_limit] DECIMAL(6,3),
		[syl_lower_limit] DECIMAL(6,3)
	)
	...
	--- (3.5) CHECK FLOW
		...
		FROM (
			...
			WHERE [device_slip_id] = ( SELECT [device_slip_id] FROM [APCSProDB].[trans].[lots] WHERE [lots].[id] = @lot_id ) AND ISNULL([is_skipped], 0) != 1
			UNION ALL
	...
	SELECT [jobs].[id] AS [job_id]
		...
		, [device_flows_sblsyl].[sbl_upper_limit]
		, [device_flows_sblsyl].[syl_lower_limit]
	FROM [APCSProDB].[method].[flow_patterns]
	INNER JOIN [APCSProDB].[method].[flow_details] ON [flow_patterns].[id] = [flow_details].[flow_pattern_id]
	INNER JOIN [APCSProDB].[method].[jobs] ON [flow_details].[job_id] = [jobs].[id]
	LEFT JOIN [APCSProDB].[method].[device_flows] ON [device_flows].[device_slip_id] = 
		(
			SELECT [device_slip_id] 
			FROM [APCSProDB].[trans].[lots] 
			WHERE [lots].[id] = @lot_id
		)
		AND [jobs].[id] = [device_flows].[job_id]
		AND [device_flows].[is_sblsyl] = 1
	LEFT JOIN [APCSProDB].[method].[device_flows_sblsyl] ON [device_flows].[id] = [device_flows_sblsyl].[device_flow_id]
	WHERE [flow_patterns].[assy_ft_class] = @assy_ft_class
		AND [flow_patterns].[link_flow_no] = @link_flow_no;
	------------------------------------------------------------------------------------------------------------------------------
	--<<--------------------------------------------------------------------------
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					SELECT [nu].[id] + ROW_NUMBER() OVER ( ORDER BY [flow_details].[flow_pattern_id] ) AS [id]
						...
						, ( SELECT TOP 1 [is_sblsyl] FROM @table_sblsyl WHERE ([job_id] = [jobs].[id]) AND ([is_sblsyl] = 1) AND ([sbl_upper_limit] IS NOT NULL) AND ([syl_lower_limit] IS NOT NULL) )  AS [is_sblsyl]
					FROM [APCSProDB].[method].[flow_details] 
					INNER JOIN [APCSProDB].[method].[flow_patterns] ON [flow_details].[flow_pattern_id] = [flow_patterns].[id]
					...
					--- INSERT lot_special_flows_sblsyl
					IF EXISTS ( 
						SELECT [job_id] 
						FROM @table_sblsyl 
						WHERE [is_sblsyl] = 1
							AND [sbl_upper_limit] IS NOT NULL
							AND [syl_lower_limit] IS NOT NULL
					)
					BEGIN
						INSERT INTO [APCSProDB].[trans].[lot_special_flows_sblsyl]
							( [lot_special_flow_id]
							, [sbl_upper_limit]
							, [syl_lower_limit]
							, [created_at]
							...
							, [updated_at]
							, [updated_by] )
						SELECT [lot_special_flows].[id] AS [lot_special_flow_id]
							, [sblsyl].[sbl_upper_limit] AS [sbl_upper_limit]
							, [sblsyl].[syl_lower_limit] AS [syl_lower_limit]
							, GETDATE() AS [created_at]
							, @user_id AS [created_by]
							, NULL AS [updated_at]
							, NULL AS [updated_by]
						FROM [APCSProDB].[trans].[lot_special_flows]
						INNER JOIN @table_sblsyl AS [sblsyl] ON [lot_special_flows].[job_id] = [sblsyl].[job_id]
						LEFT JOIN [APCSProDB].[trans].[lot_special_flows_sblsyl] ON [lot_special_flows].[id] = [lot_special_flows_sblsyl].[lot_special_flow_id]
						WHERE [lot_special_flows].[special_flow_id] = @update_spid
							AND [lot_special_flows].[step_no] BETWEEN (@q1_step_no + 1) AND (@q1_step_no + @count_flow)
							AND [sblsyl].[is_sblsyl] = 1
							AND [sblsyl].[sbl_upper_limit] IS NOT NULL
							AND [sblsyl].[syl_lower_limit] IS NOT NULL
							AND [lot_special_flows_sblsyl].[lot_special_flow_id] IS NULL;
					END
					--- SET RESULT
					SET @result = 1;
			...
			BEGIN
				...
				BEGIN
					...
					LEFT JOIN [APCSProDB].[trans].[lot_special_flows_sblsyl] ON [lot_special_flows].[id] = [lot_special_flows_sblsyl].[lot_special_flow_id]
					WHERE [lot_special_flows].[special_flow_id] = @update_spid
						AND [lot_special_flows].[step_no] BETWEEN (@step_no + 1) AND (@step_no + @count_flow)
						AND [sblsyl].[is_sblsyl] = 1
						AND [sblsyl].[sbl_upper_limit] IS NOT NULL
						AND [sblsyl].[syl_lower_limit] IS NOT NULL
						AND [lot_special_flows_sblsyl].[lot_special_flow_id] IS NULL;
