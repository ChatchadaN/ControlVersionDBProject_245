...
-- =============================================
CREATE PROCEDURE [atom].[sp_get_lot_in_process_002]	-- Add the parameters for the stored procedure here	
	@lot_no varchar(10) = '%'
	, @lot_type varchar(1) = '%'
...
BEGIN
	...
	from (
		select [lots].[id]
			...
			, case 
				when [lots].[is_special_flow] = 1 then 'Other' 
	...
	where [lots].[lot_no] like '%' + @lot_no + '%'
		...
		and upper([lots].[delay_status]) like @status
