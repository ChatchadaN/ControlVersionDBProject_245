...
-- =============================================
CREATE PROCEDURE [atom].[sp_set_lot_inventory_v2]
	  @lot_no			VARCHAR(20)
	, @stock_class		INT
	, @emp_no			VARCHAR(6)
	, @qty				INT
...
BEGIN
	...
	BEGIN
		SELECT	  'FALSE' AS Is_Pass 
				...
				, N'กรุณาตรวจสอบ lot ที่เว็บ ATOM' AS Handling
		RETURN;
	END
	...
	BEGIN
		SELECT	  'FALSE' AS Is_Pass 
				...
				, N'กรุณาตรวจสอบการลงทะเบียน Rack กับ Class ที่เว็บ ATOM' AS Handling
		FROM [APCSProDB].[inv].[Inventory_classfications] WHERE [class_no] = @class_no
		RETURN;
	END

	--IF NOT EXISTS (SELECT [class_id] FROM [APCSProDB].[inv].[class_locations] cl
	--				INNER JOIN [APCSProDB].[inv].[Inventory_classfications] ic ON ic.id = cl.class_id
	--				WHERE cl.location_name = @rack_no)  -- add by AUN 25/02/26
	--BEGIN
	--	SELECT	  'FALSE' AS Is_Pass 
	--			, 'This Rack' + @rack_no + ' not yet register class' AS Error_Message_ENG
	--			, N'Rack' + @rack_no + N' นี้ยังไม่ลงทะเบียน Class' AS Error_Message_THA 
	--			, N'กรุณาตรวจสอบการลงทะเบียน Rack กับ Class ที่เว็บ ATOM' AS Handling
	--	FROM [APCSProDB].[inv].[Inventory_classfications] WHERE [class_no] = @class_no
	--	RETURN;
	--END
	...
	BEGIN
		SELECT	  'FALSE'							AS Is_Pass 
				...
				, N'กรุณาตรวจสอบการ inventory ที่เว็บ ATOM'					AS Handling
		RETURN;
	END
