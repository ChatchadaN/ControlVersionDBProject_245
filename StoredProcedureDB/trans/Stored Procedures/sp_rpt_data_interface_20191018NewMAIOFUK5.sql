...
Create PROCEDURE [trans].[sp_rpt_data_interface_20191018NewMAIOFUK5]
@FILE_DATE DATE,
@LOCATION_ID INT,
@USER_ID INT
AS
BEGIN
	...
	DELETE FROM [APCSPRODB].[TRANS].[ADM00001]
...
select t3.writetime
	...
	,len(case when t3.record_type = 2 then 'GEE' + FORMAT(t3.seqno6, '000000') + [storedproceduredb].[material].[fn_space_right]('FUK5', 9) + SPACE(62) else t3.rovansdata end) as databyte
	,t3.sendendfl
	,t3.makdt
	,t3.makp
	,t3.makc
...
from (
	...
	from (
		...
		from (
			...
			where mt.material_state = 1
				and ct.short_name = '03'  /*03:frame*/
		) as t1
		union all
...
select t3.writetime
	,t3.seqno6
	,t3.rovansfile
	,case when t3.record_type = 2 then 'GEE' + FORMAT(t3.seqno6, '000000') + [storedproceduredb].[material].[fn_space_right]('MAIO', 9) + SPACE(62) else t3.rovansdata end as rovansdata
	,t3.sendendfl
	,t3.makdt
	,t3.makp
	,t3.makc
...
from (
	...
	from (
		...
		from (
			...
			where mr.recorded_at > '2019-10-01'
