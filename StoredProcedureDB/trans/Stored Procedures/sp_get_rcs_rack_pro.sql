			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						SELECT TOP (@qty) rack_id, rack_controls.[name] as rack,[value] as job_id ,package_id, device_id,  priority_rack
							,rack_addresses.id,rack_addresses.address , rack_addresses.item
						FROM @RackSetCondition_TB rkcon
						INNER JOIN APCSProDB.rcs.rack_controls ON rkcon.rack_id = rack_controls.id
						INNER JOIN APCSProDB.rcs.rack_addresses ON rack_controls.id = rack_addresses.rack_control_id
						CROSS APPLY string_split(rkcon.job_id, ',')
						WHERE rack_addresses.item IS NULL
						AND rack_addresses.is_enable != 0
						ORDER BY priority_rack,rack_controls.[name] , rack_addresses.address ASC

						--CHECK RACK HAVE ADDRESS
						IF EXISTS(SELECT 1 FROM @RackAuto_TB)
						BEGIN
							SELECT @rackCount = COUNT(Distinct rack_id) FROM @RackAuto_TB
							SELECT @addressCount = COUNT(Distinct rack_address_id) FROM @RackAuto_TB
						
							--Check first rack address enough
							IF @rackCount > 1
							BEGIN
								--First rack address not enough
								--Find First rack_id
   								DECLARE @FirstRackId INT
   								SELECT TOP 1 @FirstRackId = rack_id FROM @RackAuto_TB ORDER BY priority_rack, rack_name

								--temp table for another rack 
   								DECLARE @OtherRacks TABLE (rack_id INT, priority_rack INT)					
							
								INSERT INTO @OtherRacks
   								SELECT DISTINCT rack_id, MIN(priority_rack)
   								FROM @RackAuto_TB
   								WHERE rack_id != @FirstRackId
   								GROUP BY rack_id					

								-- Cursor วนเช็ค rack ทีละตัว
   								DECLARE @CandidateRackId INT
   								DECLARE @CandidateAddressCount INT

								DECLARE rack_cursor CURSOR FOR
   								SELECT rack_id FROM @OtherRacks ORDER BY priority_rack

								OPEN rack_cursor
   								FETCH NEXT FROM rack_cursor INTO @CandidateRackId
	
								WHILE @@FETCH_STATUS = 0
								BEGIN
								
									-- เช็คจำนวน address ว่างของ rack นี้
       								SELECT @CandidateAddressCount = COUNT(*)
       								FROM APCSProDB.rcs.rack_addresses
       								WHERE rack_control_id = @CandidateRackId
         							  AND item IS NULL
         							  AND is_enable != 0

									IF @CandidateAddressCount >= @qty
									BEGIN
									
										-- เจอ rack ที่ address พอ → ดึง address และ RETURN
           								SELECT TOP 1 
               								@newAddressID = id,
               								@newAddress = address,
               								@newRackName = (SELECT name FROM APCSProDB.rcs.rack_controls WHERE id = @CandidateRackId)
           								FROM APCSProDB.rcs.rack_addresses
           								WHERE rack_control_id = @CandidateRackId
             							  AND item IS NULL
             							  AND is_enable != 0
...
           								RETURN;
								...
								-- ถ้าไม่มี rack ไหน address พอ
   								CLOSE rack_cursor
   								DEALLOCATE rack_cursor

   								SELECT 'FALSE' AS Is_Pass
       								, 'Rack No Space!!' AS Error_Message_ENG
       								, N'พื้นที่ Rack ไม่เพียงพอ!!' AS Error_Message_THA
       								, '' AS Handling
       								, ISNULL(@newRackName, '') AS new_name 
       								, ISNULL(@newAddress, '') AS new_address 
       								, @newAddressID AS new_address_id
...
   								RETURN;
			...
			BEGIN
				...
				BEGIN
					...
					SELECT TOP (@qty) rack_id, rack_controls.[name] as rack,package_id, device_id,  priority_rack
						,rack_addresses.id,rack_addresses.address , rack_addresses.item
					FROM @RackSetCondition_TB rkcon
					INNER JOIN APCSProDB.rcs.rack_controls ON rkcon.rack_id = rack_controls.id
					INNER JOIN APCSProDB.rcs.rack_addresses ON rack_controls.id = rack_addresses.rack_control_id
					...
					--CHECK RACK HAVE ADDRESS
					IF EXISTS(SELECT 1 FROM @RackAutoHASUU_TB)
					BEGIN
						SELECT @rackCount = COUNT(Distinct rack_id) FROM @RackAutoHASUU_TB
						SELECT @addressCount = COUNT(Distinct rack_address_id) FROM @RackAutoHASUU_TB

						--ถ้าพบ @rackCount มากกกว่า 1 คือพท rack แรก ไม่พอ
						IF @rackCount > 1
						BEGIN
							-- หา rack_id แรก
   							DECLARE @FirstRackHASUUId INT
   							SELECT TOP 1 @FirstRackHASUUId = rack_id FROM @RackAutoHASUU_TB ORDER BY priority_rack, rack_name
						
							-- เตรียม temp table สำหรับ rack อื่น
   							DECLARE @OtherRacksHASUU TABLE (rack_id INT, priority_rack INT)

							INSERT INTO @OtherRacksHASUU
   							SELECT DISTINCT rack_id, MIN(priority_rack)
   							FROM @RackAutoHASUU_TB
   							WHERE rack_id != @FirstRackHASUUId
   							GROUP BY rack_id

							-- Cursor วนเช็ค rack ทีละตัว
   							DECLARE @CandidateRackId_HASUU INT
   							DECLARE @CandidateAddressCount_HASUU INT	

							DECLARE rack_cursor_hasuu CURSOR FOR
   							SELECT rack_id FROM @OtherRacksHASUU ORDER BY priority_rack

							OPEN rack_cursor_hasuu
   							FETCH NEXT FROM rack_cursor_hasuu INTO @CandidateRackId_HASUU

							WHILE @@FETCH_STATUS = 0
							BEGIN
								-- เช็คจำนวน address ว่างของ rack นี้
       							SELECT @CandidateAddressCount_HASUU = COUNT(*)
       							FROM APCSProDB.rcs.rack_addresses
       							WHERE rack_control_id = @CandidateRackId_HASUU
         						  AND item IS NULL
         						  AND is_enable != 0

								IF @CandidateAddressCount_HASUU >= @qty
								BEGIN
									-- เจอ rack ที่ address พอ → ดึง address และ RETURN
           							SELECT TOP 1 
               							@newAddressID = rack_addresses.id,
               							@newAddress = rack_addresses.[address],
               							@newRackName = (SELECT name FROM APCSProDB.rcs.rack_controls WHERE id = @CandidateRackId_HASUU)
           							FROM APCSProDB.rcs.rack_addresses
           							WHERE rack_control_id = @CandidateRackId_HASUU
             						  AND item IS NULL
             						  AND is_enable != 0
           							ORDER BY rack_addresses.address

									CLOSE rack_cursor_hasuu
           							DEALLOCATE rack_cursor_hasuu
...
           							RETURN;
								...
								FETCH NEXT FROM rack_cursor INTO @CandidateRackId_HASUU
...
   							RETURN;
		...
		BEGIN
			...
			BEGIN
				SELECT @oldRackName = rack_controls.name
				,@oldAddress = rack_addresses.address
				,@oldAddressID = rack_addresses.id
				FROM APCSProDB.rcs.rack_addresses
				INNER JOIN APCSProDB.rcs.rack_controls ON rack_addresses.rack_control_id = rack_controls.id
