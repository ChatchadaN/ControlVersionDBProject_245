...
-- =============================================
CREATE PROCEDURE [trans].[sp_set_data_to_is_by_task_001]
...
BEGIN
	...
	BEGIN
		...
		BEGIN 
			...
			BEGIN
				-- Insert
				IF NOT EXISTS(SELECT [LotNo] FROM [TESTDB].[testbass].[dbo].[H_STOCK] WHERE [LotNo] = @h_LotNo)
				-- IF NOT EXISTS(SELECT [LotNo] FROM [ISDB].[DBLSISHT].[dbo].[H_STOCK] WHERE [LotNo] = @h_LotNo)
				BEGIN
					PRINT 'Insert H_STOCK LotNo : ' + @h_LotNo;
					INSERT INTO [TESTDB].[testbass].[dbo].[H_STOCK] -- [ISDB].[DBLSISHT].[dbo].[H_STOCK]
					(
						[Stock_Class]
						, [PDCD]
						, [LotNo]
			...
			BEGIN
				...
				UPDATE [TESTDB].[testbass].[dbo].[H_STOCK] -- [ISDB].[DBLSISHT].[dbo].[H_STOCK]
			...
			BEGIN
				...
				DELETE FROM [TESTDB].[testbass].[dbo].[H_STOCK] -- [ISDB].[DBLSISHT].[dbo].[H_STOCK]
				WHERE [LotNo] = @h_LotNo;
				------------------------------------------------------------------------------------
				PRINT 'Update H_STOCK_IF_HIST History_ID : ' + CAST(@h_History_ID AS VARCHAR) + ', LotNo : ' + @h_LotNo + ' State_Flag = 1';
				UPDATE APCSProDWH.dbo.H_STOCK_IF_HIST
	...
	BEGIN
		...
		BEGIN 
			...
			BEGIN
				-- Insert
				IF NOT EXISTS(SELECT [LotNo] FROM [TESTDB].[testbass].[dbo].[LSI_SHIP] WHERE [LotNo] = @l_LotNo)
				-- IF NOT EXISTS(SELECT [LotNo] FROM [ISDB].[DBLSISHT].[dbo].[LSI_SHIP] WHERE [LotNo] = @l_LotNo)
				BEGIN
					PRINT 'Insert LSI_SHIP LotNo : ' + @l_LotNo;
					INSERT INTO [TESTDB].[testbass].[dbo].[LSI_SHIP] -- [ISDB].[DBLSISHT].[dbo].[LSI_SHIP]
					(
						[LotNo]
						, [Type_Name]
						, [ROHM_Model_Name]
			...
			BEGIN
				...
				UPDATE [TESTDB].[testbass].[dbo].[LSI_SHIP] -- [ISDB].[DBLSISHT].[dbo].[LSI_SHIP]
			...
			BEGIN
				...
				DELETE FROM [TESTDB].[testbass].[dbo].[LSI_SHIP] -- [ISDB].[DBLSISHT].[dbo].[LSI_SHIP]
				WHERE [LotNo] = @l_LotNo; 
				------------------------------------------------------------------------------------
				PRINT 'Update LSI_SHIP_IF_HIST History_ID : ' + CAST(@l_History_ID AS VARCHAR) + ', LotNo : ' + @l_LotNo + ' State_Flag = 1';
				UPDATE APCSProDWH.dbo.LSI_SHIP_IF_HIST
	...
	BEGIN
		...
		BEGIN 
			...
			BEGIN
				-- Insert
				IF NOT EXISTS(SELECT [LotNo] FROM [TESTDB].[testbass].[dbo].[MIX_HIST] WHERE [HASUU_LotNo] = @m_HASUU_LotNo AND [LotNo] = @m_LotNo)
				...
				BEGIN
					...
					INSERT INTO [TESTDB].[testbass].[dbo].[MIX_HIST] -- [ISDB].[DBLSISHT].[dbo].[MIX_HIST]
					(
						[M_O_No]
						, [FREQ]
						, [HASUU_LotNo]
			...
			BEGIN
				...
				DELETE FROM [TESTDB].[testbass].[dbo].[MIX_HIST] -- [ISDB].[DBLSISHT].[dbo].[MIX_HIST]
	...
	BEGIN
		...
		BEGIN 
			...
			BEGIN
				-- Insert
				IF NOT EXISTS(SELECT [LotNo] FROM [TESTDB].[testbass].[dbo].[PACKWORK] WHERE [LotNo] = @p_LotNo)
				-- IF NOT EXISTS(SELECT [LotNo] FROM [ISDB].[DBLSISHT].[dbo].[PACKWORK] WHERE [LotNo] = @p_LotNo)
				BEGIN
					PRINT 'Insert PACKWORK LotNo : ' + @p_LotNo;
					INSERT INTO [TESTDB].[testbass].[dbo].[PACKWORK] -- [ISDB].[DBLSISHT].[dbo].[PACKWORK]
					(
						[LotNo]
						, [Type_Name]
						, [ROHM_Model_Name]
			...
			BEGIN
				...
				DELETE FROM [TESTDB].[testbass].[dbo].[PACKWORK] -- [ISDB].[DBLSISHT].[dbo].[PACKWORK]
				WHERE [LotNo] = @p_LotNo;
				------------------------------------------------------------------------------------
				PRINT 'Update PACKWORK_IF_HIST History_ID : ' + CAST(@p_History_ID AS VARCHAR) + ', LotNo : ' + @p_LotNo + ' State_Flag = 1';
				UPDATE APCSProDWH.dbo.PACKWORK_IF_HIST
	...
	BEGIN
		...
		BEGIN 
			...
			BEGIN
				-- Insert
				IF NOT EXISTS(SELECT [LotNo] FROM [TESTDB].[testbass].[dbo].[WH_UKEBA] WHERE [LotNo] = @wh_LotNo)
				-- IF NOT EXISTS(SELECT [LotNo] FROM [ISDB].[DBLSISHT].[dbo].[WH_UKEBA] WHERE [LotNo] = @wh_LotNo)
				BEGIN
					PRINT 'Insert WH_UKEBA LotNo : ' + @wh_LotNo;
					INSERT INTO [TESTDB].[testbass].[dbo].[WH_UKEBA] -- [ISDB].[DBLSISHT].[dbo].[WH_UKEBA]
					(
						[Record_Class]
						, [ROHM_Model_Name]
						, [LotNo]
			...
			BEGIN
				...
				DELETE FROM [TESTDB].[testbass].[dbo].[WH_UKEBA] -- [ISDB].[DBLSISHT].[dbo].[WH_UKEBA]
	...
	BEGIN	
		...
		BEGIN 
			...
			BEGIN
				-- Insert
				IF NOT EXISTS(SELECT [LotNo] FROM [TESTDB].[testbass].[dbo].[WORK_R_DB] WHERE [LotNo] = @w_LotNo)
				-- IF NOT EXISTS(SELECT [LotNo] FROM [ISDB].[DBLSISHT].[dbo].[WORK_R_DB] WHERE [LotNo] = @w_LotNo)
				BEGIN
					PRINT 'Insert WORK_R_DB LotNo : ' + @w_LotNo;
					INSERT INTO [TESTDB].[testbass].[dbo].[WORK_R_DB] -- [ISDB].[DBLSISHT].[dbo].[WORK_R_DB]
					(
						[LotNo]
						, [Process_No]
						, [Process_Date]
			...
			BEGIN
				...
				DELETE FROM [TESTDB].[testbass].[dbo].[WORK_R_DB] -- [ISDB].[DBLSISHT].[dbo].[WORK_R_DB]
				WHERE [LotNo] = @w_LotNo;
				------------------------------------------------------------------------------------
				PRINT 'Update WORK_R_DB_IF_HIST History_ID : ' + CAST(@w_History_ID AS VARCHAR) + ', LotNo : ' + @w_LotNo + ' State_Flag = 1';
				UPDATE APCSProDWH.dbo.WORK_R_DB_IF_HIST
