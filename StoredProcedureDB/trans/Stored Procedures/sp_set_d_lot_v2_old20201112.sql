...
-- =============================================
create PROCEDURE [trans].[sp_set_d_lot_v2_old20201112] 
...
BEGIN
	...
	,isnull(t2.job_id_dlot,t2.job_id) as job_id
	,t2.QTY_IN
	,t2.QTY_PASS
	...
	,isnull(t2.start_step_no_dlot,t2.start_step_no) as start_step_no
	,t2.created_at
	from
	(
	select
	...
	,df_d.step_no as start_step_no_dlot
			...
			from
				(
					select 
						...
						--,sum(t0.QTY_IN) over (partition by hasuu_lotno,rohm_model_name) as qty_in_sum
						--,sum(t0.QTY_PASS) over (partition by hasuu_lotno,rohm_model_name) as qty_pass_sum
					from 
						(
							select 
								L1.[HASUU_LotNo]
							...
							from  OPENROWSET('SQLNCLI', 'Server= 10.28.1.144;Database=DBLSISHT;Uid=ship;Pwd=ship',
											'select MH.[HASUU_LotNo],MH.LotNo,case when MH.[HASUU_LotNo] = MH.LotNo then 1 else 0 end as is_oneself,MH.QTY,MH.[Type_Name]
													,MH.[ROHM_Model_Name]
													,MH.[ASSY_Model_Name]
													,MH.[TIRank]
													,MH.[Rank]
											...
											where MH.hasuu_lotno like ''2%D%'' 
												and MH.mixd_date > dateadd(DAY,-20,getdate())
											') as L1
								left outer join [APCSProDB].[method].[device_names] as dn with (NOLOCK) 
									on dn.name = L1.ROHM_Model_Name and dn.assy_name = L1.ASSY_MODEL_Name 
								left outer join [APCSProDB].[method].[packages] as pc with (NOLOCK) 
									on pc.id = dn.package_id 
										and pc.is_enabled = 1
								left outer join [APCSProDB].[trans].[lots] as l with (NOLOCK) 
									on l.lot_no = l1.hasuu_lotno
							where l.id is null
							group by 
								L1.[HASUU_LotNo]
								,pc.product_family_id
	...
	left outer join [APCSProDB].[method].[device_versions] as dv_d with (NOLOCK) on dv_d.device_name_id = t1.device_name_id and dv_d.device_type = 6 
	left outer join [APCSProDB].[method].[device_slips] as ds_d with (NOLOCK) on ds_d.device_id = dv_d.device_id and ds_d.is_released in(1,2)
		and not exists 
		(
			select * from [APCSProDB].[method].[device_slips] as ds2_d with (NOLOCK) 
	...
	left outer join [APCSProDB].[method].[flow_details] as fd_d with (NOLOCK) on fd_d.flow_pattern_id = dfp_d.flow_pattern_id --and fd.step_no = 1 
	--left outer join [APCSProDB].[method].[device_flows] as df with (NOLOCK) on df.device_slip_id = ds.device_slip_id and df.job_id = fd.job_id 
	...
	left outer join [APCSProDB].[trans].[numbers] as nu with (NOLOCK) on nu.name = 'lots.id' 
	where t2.step_rank = 1 and t2.step_rank_dlot = 1
