...
-- =============================================
CREATE PROCEDURE [trans].[sp_set_wip_state_memberlot]
...
BEGIN
	...
	declare @wip_state_old int
		, @pc_code int

	select @wip_state_old = wip_state 
	...
	BEGIN TRY 
		...
		SELECT 'TRUE' AS Status , '' AS Error_Message_ENG, N'' AS Error_Message_THA, N'' AS Handling
		--RETURN --- 07/03/2022 15.42
	END TRY
	BEGIN CATCH 
		SELECT 'FALSE' AS Status , 'Update wip error. !!' AS Error_Message_ENG, N'บันทึกข้อมูล wip state ผิดพลาด !!' AS Error_Message_THA, N'กรุณาติดต่อ system' AS Handling
		RETURN
	END CATCH
END
