...
CREATE PROCEDURE [trans].[sp_rpt_data_interface]
@FILE_DATE DATETIME, -- Update by AUN 2024/10/22
@LOCATION_ID INT,
@USER_ID INT
AS
BEGIN
...
--				,right(space(12) + mt.barcode,12) as seq_no
...
from (
	...
	from (
		...
		from (
			...
			where mr.recorded_at between dateadd(DAY,-1,convert(datetime,@FILE_DATE)) and convert(datetime,@FILE_DATE)
			--dateadd(hour,-8,convert(datetime,@FILE_DATE)) and DATEADD(hour,16,convert(datetime,@FILE_DATE)) update by AUN 2024/10/09
				and mr.record_class in (0, 2) and ar.po_no IS NOT NULL -- update by AUN 2024/10/22
				--and not exists (
				--	select *
				...
				--		and mr2.location_id = 3
				--	)
				and ct.short_name = '03'

	UNION all

	Select '11' as RECORD_CLASS,tQI.IN_DATE,tQI.IN_LOC,tQI.OUT_LOC,tqi.PROD_NAME,tQI.QUANTITY
	,tQI.QTY2,tQI.is_red,tQI.INVOICE_NO,tQI.PO_NO,tQI.line_no,tQI.REC_NO
		 from(
			select mr.id,mr.record_class
				,case mr.record_class when 0 then format(ar.recorded_at, 'yyMMdd') else format(mr.recorded_at, 'yyMMdd') end as IN_DATE
				,case mr.record_class 
					when 0 then pr.supplier_cd 
					else 
			...
			--dateadd(hour,-8,convert(datetime,@FILE_DATE)) and DATEADD(hour,16,convert(datetime,@FILE_DATE)) update by AUN 2024/10/09
				...
				--		and mr2.location_id = 3
				--	)
				and ct.short_name = '03') as tQI
				Where IN_LOC = 'QI999' or OUT_LOC ='QI999'
	) as t1 
...
--	EXEC [trans].[sp_rpt_data_interface_13] '2019-10-21 01:30:40.853', 2, 1
