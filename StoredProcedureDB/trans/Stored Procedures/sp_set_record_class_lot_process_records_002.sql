...
-- =============================================
CREATE PROCEDURE [trans].[sp_set_record_class_lot_process_records_002]
...
BEGIN
	...
	DECLARE @user_id INT = IIF(@opno = '','',(SELECT top 1 id FROM [APCSProDB].[man].[users] WHERE [users].[emp_num] = RIGHT('000000'+ CONVERT(VARCHAR,TRIM(@opno)),6)))
	DECLARE @lot_id INT = (SELECT top 1 id FROM [APCSProDB].[trans].[lots] WHERE [lots].[lot_no] = @lot_no)
	DECLARE @mc_id INT = IIF(@mcno = '',NULL,(SELECT top 1 id FROM [APCSProDB].[mc].[machines] WHERE [machines].[name] = @mcno))
	DECLARE @lot_process_record_id BIGINT = NULL
	...
	SELECT GETDATE()
		...
		, HOST_NAME()
		, APP_NAME()
		, 'EXEC [trans].[sp_set_record_class_lot_process_records_002] @lot_no = ''' + ISNULL(CAST(@lot_no AS varchar),'') + ''', @opno = ''' + ISNULL(CAST(@opno AS varchar),'') + ''', @@record_class = ''' 
	...
	BEGIN
		...
		BEGIN
			...
			SELECT 
				...
				,IIF(@user_id = '',1,@user_id) as [operated_by]
		...
		END
			...
			begin
				...
				select @lot_process_record_id as [id]
					...
					,[special_flows].[qty_pass] as [qty_in]
					,[special_flows].[qty_pass] as [qty_pass]
					,0 as [qty_fail] 
					,0 as [qty_last_pass]
					,0 as [qty_last_fail]
					,0 as [qty_pass_step_sum]  ---Good
					,0 as [qty_fail_step_sum]  ---NG
					,0 as [qty_divided]
					,[lots].[qty_hasuu] as [qty_hasuu]  ---Surplus
					,[lots].[qty_out] as [qty_out]  ---Shipment
					,NULL as [recipe]
					,NULL as [recipe_version]
					,[special_flows].[machine_id] as [machine_id]
					,NULL as [position_id]
					,[special_flows].[process_job_id] as [process_job_id]
					,1 as [is_onlined]
					,0 as [dbx_id]
					,[special_flows].[wip_state] as [wip_state]
					,[special_flows].[process_state] as [process_state]
					,[special_flows].[quality_state] as [quality_state]
					,0 as [first_ins_state]
					,0 as [final_ins_state]
					...
					,[lots].[created_at] as [created_at]
					,[lots].[created_by] as [created_by]
					,[special_flows].[updated_at] as [updated_at]
					,[special_flows].[updated_by] as [updated_by]
					...
					,NULL as [map_edit_state]
					,[lots].[qty_frame_pass] as [qty_frame_in]
					,0 as [qty_frame_pass]  ---FramesGood
					,0 as [qty_frame_fail]  ---FramesNG
					,NULL as [qty_frame_last_pass]
					,NULL as [qty_frame_last_fail]
					,NULL as [qty_frame_pass_step_sum]
					,NULL as [qty_frame_fail_step_sum]
					...
					,NULL as [is_3h]
					,NULL as [running_special_flow_id]
					,0 as [qty_p_nashi]  ---PNashi
					,0 as [qty_front_ng]  ---FrontNG
					,0 as [qty_marker]  ---Marker
					,0 as [qty_cut_frame]
					,NULL as [is_temp_divided]
					,NULL as [temp_divided_count]
					,NULL as [next_sideway_step_no]
				...
				select @lot_process_record_id as [id]
					...
					,NULL as [position_id]
					,[lots].[process_job_id] as [process_job_id]
					,1 as [is_onlined]
					,0 as [dbx_id]
