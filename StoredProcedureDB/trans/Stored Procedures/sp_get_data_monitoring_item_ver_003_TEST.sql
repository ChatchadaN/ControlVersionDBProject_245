...
-- =============================================
CREATE PROCEDURE [trans].[sp_get_data_monitoring_item_ver_003_TEST]
...
BEGIN
	...
	BEGIN
		...
		SELECT [monitoring_item_records_temp].monitoring_item_id, [monitoring_item_records_temp].date , CAST(AVG([monitoring_item_records_temp].current_value) AS DECIMAL(9,1)) AS current_value
		, CAST(AVG([monitoring_item_records_temp].alarm_value) AS DECIMAL(9,1)) AS target_value
		, alarmed.is_alarmed
		...
		INNER JOIN [APCSProDB].[method].[packages] ON [wip_count_target].[package_id] = [packages].[id]
			...
			AND [packages].[package_group_id] like @package_group_id
		LEFT JOIN   
				(	SELECT  COUNT(monitoring_item_records.is_alarmed) AS is_alarmed  
							, convert(varchar,monitoring_item_records.recorded_at, 23)  AS recorded_at 
							, monitoring_item_id
					FROM  APCSProDWH.wip_control.monitoring_item_records
					WHERE monitoring_item_records.is_alarmed = 1 
					GROUP BY  convert(varchar,monitoring_item_records.recorded_at, 23)  , monitoring_item_records.monitoring_item_id , monitoring_item_records.is_alarmed
				)	AS alarmed
		ON		alarmed.recorded_at			= [monitoring_item_records_temp].date
		AND		alarmed.monitoring_item_id	= [monitoring_item_records_temp].monitoring_item_id 
	 
		WHERE ([monitoring_item_records_temp].[date] between @date_start and @date_end)
		GROUP BY [monitoring_item_records_temp].monitoring_item_id, [monitoring_item_records_temp].date, alarmed.is_alarmed, alarmed.recorded_at
		...
		SELECT d_data.top_id 
			...
			, package_name
			, 'Good ' + TRIM(package_name) + ' [' + items_name + ']' AS items_name
			--, CAST(ISNULL(d_temp.current_value,0) AS DECIMAL(9,1)) AS current_value
			, IIF(CAST(ISNULL(d_temp.current_value,0) AS DECIMAL(9,1)) > CAST(ISNULL(d_temp.target_value,ISNULL([monitoring_items].alarm_value,null)) AS DECIMAL(9,1))
				,null
				,CAST(ISNULL(d_temp.current_value,null) AS DECIMAL(9,1))
			) AS current_value
			, 'Bad ' + TRIM(package_name) + ' [' + items_name + ']' AS items_name2
			, IIF(CAST(ISNULL(d_temp.current_value,null) AS DECIMAL(9,1)) > CAST(ISNULL(d_temp.target_value,ISNULL([monitoring_items].alarm_value,null)) AS DECIMAL(9,1))
				,CAST(ISNULL(d_temp.current_value,null) AS DECIMAL(9,1))
				,null
			) AS current_value2
			...
			, 'Stop ' + TRIM(package_name) + ' [' + items_name + ']' AS items_name_stop
			, ISNULL(d_temp.is_alarmed, null) AS stop_value
	...
	BEGIN
		...
		SELECT	  [monitoring_item_records_temp].monitoring_item_id
				, [monitoring_item_records_temp].[time]
				, CAST(AVG([monitoring_item_records_temp].current_value) AS DECIMAL(9,1)) AS current_value
				, CAST(AVG([monitoring_item_records_temp].alarm_value) AS DECIMAL(9,1)) AS target_value
				, alarmed.is_alarmed
		FROM [APCSProDWH].[wip_control].[monitoring_item_records_temp]
		INNER JOIN [APCSProDWH].[wip_control].[monitoring_items] ON [monitoring_item_records_temp].[monitoring_item_id] = [monitoring_items].[id]
		...
		INNER JOIN [APCSProDB].[method].[packages] ON [wip_count_target].[package_id] = [packages].[id]
			...
			AND [packages].[id] like @package_id
			AND [packages].[package_group_id] like @package_group_id
		LEFT JOIN (	  SELECT  alarmecount.[time] , alarmecount.[day],  COUNT(alarmecount.is_alarmed) AS is_alarmed , alarmecount.monitoring_item_id 
					  FROM   (SELECT  convert(char(2), recorded_at, 108) [time] 
							  , convert(varchar,monitoring_item_records.recorded_at, 23) [day] 
							  , monitoring_item_records.is_alarmed AS is_alarmed  
							  , monitoring_item_id
							  FROM  APCSProDWH.wip_control.monitoring_item_records
							  WHERE  monitoring_item_records.is_alarmed = 1 
							) alarmecount 
					GROUP BY alarmecount.[time] , alarmecount.[day]  ,alarmecount.monitoring_item_id ,  alarmecount.is_alarmed
					) alarmed
		ON		alarmed.[day]			= [monitoring_item_records_temp].date
		AND		alarmed.time			= [monitoring_item_records_temp].[time]
		AND		alarmed.monitoring_item_id	= [monitoring_item_records_temp].monitoring_item_id 
		WHERE ([monitoring_item_records_temp].[date] = FORMAT(GETDATE(), 'yyyy-MM-dd'))
		GROUP BY [monitoring_item_records_temp].monitoring_item_id, [monitoring_item_records_temp].[time],  alarmed.is_alarmed 
		...
		SELECT d_data.top_id 
			...
			, 'Good ' + TRIM(package_name) + ' [' + items_name + ']' AS items_name
			--, CAST(ISNULL(d_temp.current_value,0) AS DECIMAL(9,1)) AS current_value
			, IIF(CAST(ISNULL(d_temp.current_value,0) AS DECIMAL(9,1)) > CAST(ISNULL(d_temp.target_value,ISNULL([monitoring_items].alarm_value,0)) AS DECIMAL(9,1))
				,0
				,CAST(ISNULL(d_temp.current_value,0) AS DECIMAL(9,1))
			) AS current_value
			, 'Bad ' + TRIM(package_name) + ' [' + items_name + ']' AS items_name2
			, IIF(CAST(ISNULL(d_temp.current_value,0) AS DECIMAL(9,1)) > CAST(ISNULL(d_temp.target_value,ISNULL([monitoring_items].alarm_value,0)) AS DECIMAL(9,1))
				,CAST(ISNULL(d_temp.current_value,0) AS DECIMAL(9,1))
			...
			, 'Stop ' + TRIM(package_name) + ' [' + items_name + ']' AS items_name_stop
			, ISNULL(d_temp.is_alarmed, 0) AS stop_value
