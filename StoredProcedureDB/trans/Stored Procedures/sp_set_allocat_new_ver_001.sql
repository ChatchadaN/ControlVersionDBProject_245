...
-- =============================================
CREATE PROCEDURE [trans].[sp_set_allocat_new_ver_001]
...
BEGIN
	...
	-------------------------------------------------------------------------------------------------------
	BEGIN TRY
	BEGIN TRANSACTION Tran_Allocat

		TRUNCATE TABLE [APCSProDWH].[atom].[allocat]
		-- (1) delete table allocat
		--DELETE FROM [APCSProDWH].[atom].[allocat] WITH (HOLDLOCK)
		-- (2) insert from is.allocat to pro.allocat
		INSERT INTO [APCSProDWH].[atom].[allocat]
		SELECT * FROM [APCSProDWH].[atom].[allocat_table] WITH (HOLDLOCK)
		--WAITFOR DELAY '00:00:05' 
		COMMIT TRANSACTION Tran_Allocat
	END TRY 
	BEGIN CATCH
		ROLLBACK TRANSACTION Tran_Allocat
	END CATCH
	...
	TRUNCATE TABLE [APCSProDWH].[atom].[allocat_table];

	-------------------------------------------------------------------------------------------------------
	-- (4) insert or update allocat to allocat_temp
	-------------------------------------------------------------------------------------------------------
   	MERGE [APCSProDWH].[atom].[allocat_temp] AS [atemp]
	USING [APCSProDWH].[atom].[allocat] AS [a] ON ([atemp].[LotNo] = [a].[LotNo]) 
	...
	FROM [APCSProDWH].[atom].[allocat_temp] AS [allocat]
