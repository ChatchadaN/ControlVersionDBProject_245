...
-- =============================================
CREATE PROCEDURE [trans].[sp_set_version_up_device_slip]
...
BEGIN
	...
	SET
		[APCSProDB].[trans].[lots].[device_slip_id] = [new_device_slip].[device_slip_id]
	...
	inner join [APCSProDB].[method].[device_slips] as [new_device_slip] on [new_device_slip].[version_num] = [device_versions].[version_num] and [new_device_slip].[device_id] = [device_versions].[device_id]
	where [lots].[wip_state] = 20 and [packages].[name] in ('TSSOP-C48V','TSSOPC48VM') and [packages].[is_enabled] = 0
	...
	INSERT INTO [APCSProDB].[trans].[lot_process_records]([id]
      ...
      ,[updated_by])
		...
		WHERE [lots].[lot_no] in (select [lots].[lot_no]
			from [APCSProDB].[trans].[lots]
			inner join [APCSProDB].[method].[device_slips] on [device_slips].[device_slip_id] = [lots].[device_slip_id]
			inner join [APCSProDB].[method].[device_versions] on [device_versions].[device_id] = [device_slips].[device_id]
			inner join [APCSProDB].[method].[device_names] on [device_names].[id] = [device_versions].[device_name_id]
			inner join [APCSProDB].[method].[packages] on [packages].[id] = [device_names].[package_id]
			inner join [APCSProDB].[method].[device_slips] as [new_device_slip] on [new_device_slip].[version_num] = [device_versions].[version_num] and [new_device_slip].[device_id] = [device_versions].[device_id]
			where [lots].[wip_state] = 20 and [packages].[name] like 'TO263-%' and [packages].[is_enabled] = 0)
