...
-- =============================================
create PROCEDURE [trans].[sp_set_lapis_outsource_lot_v1]
...
BEGIN
	...
	IF NOT EXISTS (SELECT TOP 1 id FROM APCSProDB.method.packages WHERE external_package_code = @package) BEGIN
		SELECT 'FALSE' AS Is_Pass,'Package is not register. !!' AS Error_Message_ENG,N'Package นี้ยังไม่ได้ลงทะเบียน !!' AS Error_Message_THA
		RETURN
	END

	SET @act_package_id = (SELECT TOP 1 id FROM APCSProDB.method.packages WHERE external_package_code = @package)
	SET @mag = CASE WHEN @mag2 IS NULL THEN @mag1 ELSE CONCAT( @mag1,'|',@mag2) END
