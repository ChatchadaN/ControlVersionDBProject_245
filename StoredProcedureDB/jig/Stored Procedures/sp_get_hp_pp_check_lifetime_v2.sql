...
-- =============================================
CREATE   PROCEDURE [jig].[sp_get_hp_pp_check_lifetime_v2]
...
BEGIN
	...
	SET @STDLifeTime = (SELECT (CAST(APCSProDB.jig.productions.expiration_value  AS INT) * 1000 )
	...
	SET @Period =		(SELECT (CAST(APCSProDB.jig.production_counters.alarm_value  AS INT) * 1000 )
						FROM APCSProDB.trans.jigs INNER JOIN
		 ...
		 IF  @LifeTime >= @STDLifeTime 
			BEGIN	
				SELECT   'FALSE' AS Is_Pass
						...
						,N'LifeTime  หมดอายุ !!' AS [Handling]
		...
		ELSE  	IF (@Safety >= @Period  )
			 BEGIN 
				...
				,N'ถึง Period Lifetime แล้ว กรุณานำ HP/PP ไป Cleaning !!' AS [Handling]
				FROM APCSProDB.trans.jigs where jigs.id = @HPPP_ID 
			 END 
		ELSE BEGIN
