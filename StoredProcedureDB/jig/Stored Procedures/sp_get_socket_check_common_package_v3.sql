...
-- =============================================
CREATE PROCEDURE [jig].[sp_get_socket_check_common_package_v3]
...
BEGIN
	...
	BEGIN
		...
		SELECT @pcs_per_test = APCSProDB.method.jig_sets.code, @process = APCSProDB.method.processes.name
		FROM      APCSProDB.trans.jigs INNER JOIN
								 APCSProDB.jig.productions ON APCSProDB.trans.jigs.jig_production_id = APCSProDB.jig.productions.id INNER JOIN
		...
		--//////////////////// data output
		SELECT    'TRUE' AS Is_Pass,'' AS Error_Message_ENG  ,N'' AS Error_Message_THA ,'' AS Handling , APCSProDB.method.jig_sets.code AS PiecePerTest, APCSProDB.method.jig_sets.name AS Package,productions.name AS SocketType
		FROM      APCSProDB.trans.jigs INNER JOIN
								 APCSProDB.jig.productions ON APCSProDB.trans.jigs.jig_production_id = APCSProDB.jig.productions.id INNER JOIN
								 APCSProDB.method.jig_set_list ON APCSProDB.jig.productions.id = APCSProDB.method.jig_set_list.jig_group_id INNER JOIN
								 APCSProDB.method.jig_sets ON APCSProDB.method.jig_set_list.jig_set_id = APCSProDB.method.jig_sets.id
