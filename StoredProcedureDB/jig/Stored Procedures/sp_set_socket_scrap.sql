...
CREATE  PROCEDURE [jig].[sp_set_socket_scrap]
(
		  @Process_id		INT				= NULL
...
BEGIN  	
		...
		BEGIN TRY
		IF (@Img1 IS NULL AND @File IS NULL )
		BEGIN 
				IF (@jig_state = 12)--12	On Machine
				BEGIN 
					SELECT    'FALSE' AS Is_Pass
							, N'('+@Barcode+') used in the machine.!!' AS Error_Message_ENG
							, N'('+@Barcode+ N') นี้ถูกใช้งานอยู่ในเครื่องจักร !!' AS Error_Message_THA
							, '' AS Handling
							, '' AS Warning
					RETURN
				END
				...
				BEGIN 
						...
						SELECT  TOP (1)  'TRUE' AS Is_Pass
										, N'('+@Barcode+') Scrap Successfully !!'		AS Error_Message_ENG
										, N'('+@Barcode+ N') ถูก Scrap เเล้ว !!'			AS Error_Message_THA
										, ''											AS Handling
										, ''											AS Warning
										, jigs.id										AS id
		...
		BEGIN 			
				...
				SET @binary_file =  (SELECT TOP 1  binary_file FROM APCSProDB.trans.jigs 
