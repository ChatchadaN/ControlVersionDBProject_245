...
-- =============================================
CREATE PROCEDURE [jig].[sp_set_tsukiage_setup_v1]
...
BEGIN
	...
	SET @MCType = (SELECT (CASE WHEN UPPER(@MCType) in ('AD8312','AD833','AD8312PLUS') THEN 'ASM' 
					...
					WHEN UPPER(@MCType) in ('2009SSI','2100HS','2100XP','ESEC2009 SSI','ESEC2100 HS','ESEC2100 XP') THEN 'ESECS'
					ELSE @MCType END))	
	...
	BEGIN
			...
			SELECT 'FALSE' AS Is_Pass,N'This JIG ('+ @QRCode + N') Is not Tsukiage !!' AS Error_Message_ENG,
	...
	IF @Status = 'On Machine' BEGIN
		...
		IF @MCOld IS NULL or @MCOld <> @MCNo BEGIN
				SELECT 'FALSE' AS Is_Pass,N'This JIG ('+ @QRCode + N') Is use on another Machine !!' AS Error_Message_ENG,
	...
	ELSE BEGIN
		...
		SET @TsukiageNo = (SELECT PIN_NO From [APCSDB].[dbo].[LCQW_UNION_WORK_DENPYO_PRINT] where LOT_NO_2 = @LotNo)
	END
	...
	IF @Status  = 'To Machine' BEGIN
			...
			BEGIN
				...
				ELSE BEGIN
					--//////////UPDATE JIG NEW
					SET @Idx = (SELECT COUNT(idx)FROM APCSProDB.trans.machine_jigs WHERE machine_jigs.machine_id = @MCId)	
	...
	--		SELECT 'TRUE' AS Is_Pass,(SELECT smallcode FROM APCSProDB.trans.jigs WHERE barcode = @QRCode) AS SmallCode,@X AS X,@Y AS Y,@TsukiageNo AS TsukiageNo				
	--	RETURN 
	--END
	END TRY
	BEGIN CATCH
		SELECT 'FALSE' AS Is_Pass,N'Update data failed !! (JIG Tsukiage Service)' AS Error_Message_ENG,
