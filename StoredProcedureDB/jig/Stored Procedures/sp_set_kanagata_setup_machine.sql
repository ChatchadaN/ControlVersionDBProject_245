...
-- =============================================
CREATE PROCEDURE [jig].[sp_set_kanagata_setup_machine] 
...
BEGIN
	...
	IF (SELECT COUNT(machines.name) FROM APCSProDB.trans.jigs LEFT JOIN
            APCSProDB.trans.machine_jigs ON machine_jigs.jig_id = jigs.id LEFT JOIN 
			APCSProDB.mc.machines ON machines.id = machine_jigs.machine_id WHERE jigs.qrcodebyuser = @KanagataName) > 1 BEGIN

		SELECT 'FALSE' AS Is_Pass,'This Kanagata ('+ @KanagataName +') is not register !!' AS Error_Message_ENG
			,N'ข้อมูล Kanagata ผิดปกติ ('+ @KanagataName + N') นี้อยู่ในเครื่องจักรมากกว่า 1 เครื่อง !!' AS Error_Message_THA
	...
	IF NOT EXISTS (SELECT jigs.id FROM APCSProDB.trans.jigs INNER JOIN APCSProDB.jig.productions ON APCSProDB.trans.jigs.jig_production_id = APCSProDB.jig.productions.id 
	...
	BEGIN
		SELECT 'FALSE' AS Is_Pass,'This Kanagata ('+ @KanagataName +') is not register !!' AS Error_Message_ENG
		,'Kanagata ('+ @KanagataName + N') ยังไม่ถูกลงทะเบียน !!' AS Error_Message_THA
		,N'กรุณาตรวจสอบข้อมูลที่เว็บ JIG หรือติดต่อ System' AS Handling
		RETURN
	END
	...
	--/////////////// RETURN DATA
		SELECT 'TRUE' AS Is_Pass,jigs.id,@KanagataName AS Kanagata_Name,jigs.status,productions.name AS basetype FROM APCSProDB.trans.jigs 
				...
				IF EXISTS (SELECT machine_id FROM APCSProDB.trans.machine_jigs WHERE machine_id = @MC_ID and idx = 1) BEGIN
					...
					IF @JIG_ID <> @JIG_OLD BEGIN
						...
						IF NOT EXISTS( SELECT 1 FROM APCSProDB.trans.machine_jigs WHERE jig_id = @JIG_ID AND machine_id = @MC_ID) BEGIN
							--//////////UPDATE JIG OLD
							UPDATE APCSProDB.trans.jigs set status = 'To Machine',[jig_state] = 11,updated_at = GETDATE(),updated_by = @OPID where id = @JIG_OLD OR root_jig_id = @JIG_OLD

							--//////////UPDATE JIG NEW
							UPDATE APCSProDB.trans.jigs set location_id = NULL,status = 'On Machine',[jig_state] = 12,updated_at = GETDATE(),updated_by = @OPID where id = @JIG_ID OR root_jig_id = @JIG_ID
							UPDATE APCSProDB.trans.machine_jigs SET  jig_id = @JIG_ID,updated_at = GETDATE(),updated_by = @OPID WHERE machine_id = @MC_ID and idx = 1 
							...
							INSERT INTO APCSProDB.trans.jig_records ([day_id],[record_at],[jig_id],[jig_production_id],[location_id],[created_at],[created_by],[operated_by],transaction_type,mc_no,record_class) 
										 ...
										 (SELECT jig_production_id FROM APCSProDB.trans.jigs where id = @JIG_ID),NULL, GETDATE(), @OPID, @UserID,'On Machine',@MCNo,12)
							...
							INSERT INTO APCSProDB.trans.jig_records ([day_id],[record_at],[jig_id],[jig_production_id],[location_id],[created_at],[created_by],[operated_by],transaction_type,record_class) 
										 ...
										 (SELECT jig_production_id FROM APCSProDB.trans.jigs where id = @JIG_OLD),NULL, GETDATE(), @OPID, @UserID,'To Machine',11)
						END
						END
					END
					ELSE  BEGIN
					--//////////UPDATE JIG NEW
						UPDATE APCSProDB.trans.jigs set location_id = NULL,status = 'On Machine',[jig_state] = 12,updated_at = GETDATE(),updated_by = @OPID where id = @JIG_ID OR root_jig_id = @JIG_ID
						INSERT INTO APCSProDB.trans.machine_jigs (machine_id,idx,jig_id,created_at,created_by) VALUES (@MC_ID,1,@JIG_ID,GETDATE(),@OPID)
			...
			--/////////////////// Mold Setup Kanagata On machine
				...
				WHILE @idx <= 4 BEGIN
					IF NOT EXISTS (SELECT 1 FROM APCSProDB.trans.machine_jigs WHERE machine_id = @MC_ID AND idx = @idx) BEGIN
						--//////////UPDATE JIG NEW
						UPDATE APCSProDB.trans.jigs set location_id = NULL,status = 'On Machine',[jig_state] = 12,updated_at = GETDATE(),updated_by = @OPID where id = @JIG_ID
						INSERT INTO APCSProDB.trans.machine_jigs (machine_id,idx,jig_id,created_at,created_by) VALUES (@MC_ID,@idx,@JIG_ID,GETDATE(),@OPID)
