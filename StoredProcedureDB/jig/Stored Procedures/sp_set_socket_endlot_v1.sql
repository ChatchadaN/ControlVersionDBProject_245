...
-- =============================================
Create PROCEDURE [jig].[sp_set_socket_endlot_v1] 
...
BEGIN
	...
	BEGIN TRY
		...
		INSERT INTO APCSProDB.trans.jig_records ([day_id], [record_at], [jig_id], [jig_production_id], [created_at], [created_by], [operated_by], transaction_type,mc_no,lot_no,record_class) 
					...
					(SELECT jig_production_id FROM APCSProDB.trans.jigs where id = @JIG_ID), GETDATE(),@OPNo,@OPNo,'End Lot',@MCNo,@LOTNo,15)
		...
		SELECT 'TRUE' AS Is_Pass,@QRCode AS QRCode,
							(SELECT smallcode FROM APCSProDB.trans.jigs WHERE barcode = @QRCode) AS Smallcode,
							(SELECT productions.name FROM APCSProDB.trans.jigs INNER JOIN APCSProDB.jig.productions ON jig_production_id = productions.id WHERE barcode = @QRCode) AS SocketType,
							(SELECT CONVERT(int, jig_conditions.value / 1000) AS Life_Time FROM  APCSProDB.trans.jigs INNER JOIN APCSProDB.trans.jig_conditions ON jig_conditions.id = jigs.id WHERE barcode = @QRCode) AS Life_Time,
							(SELECT CONVERT(int, productions.expiration_value / 1000) AS STD_Life_Time FROM APCSProDB.trans.jigs INNER JOIN APCSProDB.jig.productions ON jig_production_id = productions.id 
								INNER JOIN [APCSProDB].[jig].[production_counters] ON production_counters.production_id = productions.id WHERE barcode = @QRCode) AS STD_Life_Time
	END TRY
	BEGIN CATCH
