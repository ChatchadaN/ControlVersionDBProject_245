...
-- =============================================
CREATE PROCEDURE [jig].[sp_set_socket_setup_v1]
...
BEGIN
	...
	IF @Status_JIG_IN = 'To Machine' BEGIN
		...
		IF NOT EXISTS (SELECT 1 FROM APCSProDB.trans.machine_jigs WHERE machine_id = @MC_ID AND idx between 2 and 17) BEGIN
			--//////////UPDATE JIG NEW
			UPDATE APCSProDB.trans.jigs set location_id = NULL,status = 'On Machine',[jig_state] = 12,updated_at = GETDATE(),updated_by = @OPNo where id = @JIG_ID_IN
			INSERT INTO APCSProDB.trans.machine_jigs (machine_id,idx,jig_id,created_at,created_by) VALUES (@MC_ID,2,@JIG_ID_IN,GETDATE(),@OPNo)
			...
			INSERT INTO APCSProDB.trans.jig_records ([day_id],[record_at],[jig_id],[jig_production_id],[location_id],[created_at],[created_by],[operated_by],transaction_type,mc_no,record_class) 
						 ...
						 (SELECT jig_production_id FROM APCSProDB.trans.jigs where id = @JIG_ID_IN),NULL, GETDATE(), @OPNo, @OPNo,'On Machine',@MCNo,12)			
		END
		...
		ELSE IF EXISTS(SELECT jigs.id FROM APCSProDB.trans.jigs WHERE barcode = @QRCodeOut) BEGIN
			...
			IF  @Status_JIG_OUT <> 'On Machine' BEGIN
				SELECT 'FALSE' AS Is_Pass,'Socket ('+ (SELECT jigs.smallcode FROM APCSProDB.trans.jigs WHERE barcode = @QRCodeOut) + ') is not On Machine.' AS Error_Message_ENG
				,N'Socket ('+ (SELECT jigs.smallcode FROM APCSProDB.trans.jigs WHERE barcode = @QRCodeOut) + N') ไม่ได้อยู่ในเครื่องจักร !!' AS Error_Message_THA
				RETURN
			END
			ELSE IF @Status_JIG_OUT = 'On Machine' BEGIN				
				--//////////UPDATE JIG NEW
				UPDATE APCSProDB.trans.jigs SET location_id = NULL,status = 'On Machine',[jig_state] = 12,updated_at = GETDATE(),updated_by = @OPNo where id = @JIG_ID_IN
				UPDATE APCSProDB.trans.machine_jigs SET jig_id = @JIG_ID_IN,updated_at = GETDATE(),updated_by = @OPNo WHERE machine_id = @MC_ID AND jig_id = @JIG_ID_OUT
				...
				--//////////UPDATE JIG OLD
				UPDATE APCSProDB.trans.jigs set location_id = NULL,status = 'To Machine',[jig_state] = 11,updated_at = GETDATE(),updated_by = @OPNo where id = @JIG_ID_OUT

				INSERT INTO APCSProDB.trans.jig_records ([day_id],[record_at],[jig_id],[jig_production_id],[location_id],[created_at],[created_by],[operated_by],transaction_type,mc_no,record_class) 
								 values ((SELECT id FROM APCSProDB.trans.days where date_value =  CONVERT(date,GETDATE(),111)),GETDATE(),@JIG_ID_OUT,
								 (SELECT jig_production_id FROM APCSProDB.trans.jigs where id = @JIG_ID_OUT),NULL, GETDATE(), @OPNo, @OPNo,'To Machine',NULL,11)			
			END
		END
		...
		ELSE BEGIN
			...
			WHILE @idx <= 17 BEGIN
				IF NOT EXISTS (SELECT 1 FROM APCSProDB.trans.machine_jigs WHERE machine_id = @MC_ID AND idx = @idx) BEGIN
					--//////////UPDATE JIG NEW
					UPDATE APCSProDB.trans.jigs set location_id = NULL,status = 'On Machine',[jig_state] = 12,updated_at = GETDATE(),updated_by = @OPNo where id = @JIG_ID_IN
					INSERT INTO APCSProDB.trans.machine_jigs (machine_id,idx,jig_id,created_at,created_by) VALUES (@MC_ID,@idx,@JIG_ID_IN,GETDATE(),@OPNo)
					...
					SELECT 'TRUE' AS Is_Pass,@QRCodeIn AS QRCode,
						(SELECT smallcode FROM APCSProDB.trans.jigs WHERE barcode = @QRCodeIn) AS Smallcode,
						(SELECT productions.name FROM APCSProDB.trans.jigs INNER JOIN APCSProDB.jig.productions ON jig_production_id = productions.id WHERE barcode = @QRCodeIn) AS SocketType,
						(SELECT CONVERT(int, jig_conditions.value / 1000) AS Life_Time FROM  APCSProDB.trans.jigs INNER JOIN APCSProDB.trans.jig_conditions ON jig_conditions.id = jigs.id WHERE barcode = @QRCodeIn) AS Life_Time,
						(SELECT CONVERT(int, productions.expiration_value/ 1000) AS STD_Life_Time FROM APCSProDB.trans.jigs INNER JOIN APCSProDB.jig.productions ON jig_production_id = productions.id 
							INNER JOIN [APCSProDB].[jig].[production_counters] ON production_counters.production_id = productions.id WHERE barcode = @QRCodeIn) AS STD_Life_Time
					RETURN
				END
	...
	END
	ELSE BEGIN
		SELECT 'FALSE' AS Is_Pass,'Socket ('+ (SELECT jigs.smallcode FROM APCSProDB.trans.jigs WHERE barcode = @QRCodeIn) + ') status is not scan out of stock.' AS Error_Message_ENG
		,'Socket ('+ (SELECT jigs.smallcode FROM APCSProDB.trans.jigs WHERE barcode = @QRCodeIn) + N') ยังไม่ถูกสแกนออกจาก Stock !!' AS Error_Message_THA
		RETURN
	END
END
