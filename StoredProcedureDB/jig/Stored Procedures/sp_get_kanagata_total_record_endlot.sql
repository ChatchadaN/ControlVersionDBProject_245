...
-- =============================================
CREATE PROCEDURE [jig].[sp_get_kanagata_total_record_endlot]
...
BEGIN
	...
	SET @root_id = (SELECT root_jig_id FROM APCSProDB.trans.jigs WHERE qrcodebyuser = IIF(@KanagataName ='' ,'%', @KanagataName))
	...
	WHERE	(jigs.root_jig_id =  @root_id  OR @root_id IS NULL)
