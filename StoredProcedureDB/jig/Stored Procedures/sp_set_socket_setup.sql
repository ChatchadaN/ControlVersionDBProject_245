...
-- =============================================
CREATE PROCEDURE [jig].[sp_set_socket_setup]
...
BEGIN
	...
	SELECT    GETDATE()
			...
			, HOST_NAME()
			, APP_NAME()
			, 'EXEC [jig].[sp_set_socket_setup] @JIG_ID = ''' + ISNULL(CAST(@JIG_ID_IN AS nvarchar(MAX)),'') + ''', @QRCodeIn = ''' + ISNULL(CAST(@QRCodeIn AS nvarchar(MAX)),'') + ''', @QRCodeOut = ''' + ISNULL(CAST(@QRCodeOut AS nvarchar(MAX)),'') + ''',@OPNo = ''' 
...
SET @STDLifeTime = (SELECT APCSProDB.jig.production_counters.alarm_value / 1000
	...
	IF @Status_JIG_IN = 'To Machine' BEGIN
		...
		IF NOT EXISTS (SELECT 1 FROM APCSProDB.trans.machine_jigs WHERE machine_id = @MC_ID AND idx between 2 and 17) BEGIN
			...
			,status = 'On Machine'
			,[jig_state] = 12,updated_at = GETDATE()
			,updated_by = @OPID 
			where id = @JIG_ID_IN

			INSERT INTO APCSProDB.trans.machine_jigs 
			(machine_id,idx,jig_id,created_at,created_by) 
			VALUES (@MC_ID,2,@JIG_ID_IN,GETDATE(),@OPID)
		...
		ELSE IF EXISTS(SELECT jigs.id FROM APCSProDB.trans.jigs WHERE barcode = @QRCodeOut) BEGIN
			...
			IF  @Status_JIG_OUT <> 'On Machine' BEGIN
				...
				,N'Socket ('+ (smallcode ) + N') ไม่ได้อยู่ในเครื่องจักร !!' AS Error_Message_THA ,'' AS Handling
				FROM APCSProDB.trans.jigs WHERE barcode = @QRCodeOut
			...
			WHILE @idx <= 17 BEGIN
				IF NOT EXISTS (SELECT 1 FROM APCSProDB.trans.machine_jigs WHERE machine_id = @MC_ID AND idx = @idx) BEGIN
					...
					SELECT 'TRUE' AS Is_Pass,'' AS Error_Message_ENG
						...
						, '' AS Handling,@QRCodeIn AS QRCode
		...
		IF NOT EXISTS(SELECT 1 FROM APCSProDB.trans.machine_jigs WHERE jig_id = @JIG_ID_IN) BEGIN
			SELECT 'FALSE' AS Is_Pass,'Update Failed. Can not update Socket to machine !!' AS Error_Message_ENG
				,N'อัพเดทผิดพลาด Socket ยังไม่ถูกนำเข้าในเครื่องจักร !!' AS Error_Message_THA ,'' AS Handling
			RETURN
		END
		...
		BEGIN CATCH
			...
			,N'การติดตั้ง Socket ผิดพลาด !!' AS Error_Message_THA ,'' AS Handling
		END CATCH
