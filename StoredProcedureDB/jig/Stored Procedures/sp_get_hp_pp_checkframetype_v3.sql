...
-- =============================================
CREATE PROCEDURE [jig].[sp_get_hp_pp_checkframetype_v3]
...
BEGIN
	...
	SET @HP_ID = (SELECT [jig_id] FROM [APCSProDB].[trans].[machine_jigs]
	...
	SET @PP_ID = (SELECT [jig_id] FROM [APCSProDB].[trans].[machine_jigs]
								INNER JOIN APCSProDB.mc.machines on APCSProDB.trans.machine_jigs.machine_id = APCSProDB.mc.machines.id
								where name = @MCNo AND idx = 2) 

	--Check MCNo
	IF NOT EXISTS(SELECT 1 FROM APCSProDB.mc.machines WHERE name = @MCNo)
	BEGIN
	...
	IF (SELECT COUNT(TB.idx) From (SELECT idx FROM [APCSProDB].[trans].[machine_jigs] 
		...
		WHERE APCSProDB.mc.machines.name = @MCNo AND APCSProDB.jig.categories.name in ('HP','PP') AND idx <= 2 ) AS TB)  = 2 
	BEGIN
		--Find HP/PP OnMachine
		IF NOT EXISTS (SELECT jigs.barcode FROM APCSProDB.trans.jigs 
		  INNER JOIN APCSProDB.jig.productions on APCSProDB.jig.productions.id = APCSProDB.trans.jigs.jig_production_id
		  INNER JOIN APCSProDB.jig.categories on APCSProDB.jig.categories.id = APCSProDB.jig.productions.category_id
		  WHERE APCSProDB.trans.jigs.id = @HP_ID
		  --(SELECT [jig_id]
		...
		IF NOT EXISTS (SELECT jigs.barcode FROM APCSProDB.trans.jigs 
		  INNER JOIN APCSProDB.jig.productions on APCSProDB.jig.productions.id = APCSProDB.trans.jigs.jig_production_id
		  INNER JOIN APCSProDB.jig.categories on APCSProDB.jig.categories.id = APCSProDB.jig.productions.category_id
		  WHERE APCSProDB.trans.jigs.id = @PP_ID
		  --(SELECT [jig_id]
				--							FROM [APCSProDB].[trans].[machine_jigs]
				--							INNER JOIN APCSProDB.mc.machines on APCSProDB.trans.machine_jigs.machine_id = APCSProDB.mc.machines.id
				--							where name = @MCNo AND idx = 2) 
		...
		SET @FrameLot = (SELECT TRIM(FRAME_NAME) AS FRAME_NAME  FROM [APCSDB].[dbo].[LCQW_UNION_WORK_DENPYO_PRINT] where LOT_NO_1 =  @LotNo)
		SET @FrameHP = (SELECT DISTINCT APCSProDB.jig.productions.name FROM APCSProDB.trans.jigs 
		  ...
		  --(SELECT [jig_id]
				...
				--							INNER JOIN APCSProDB.mc.machines on APCSProDB.trans.machine_jigs.machine_id = APCSProDB.mc.machines.id
				--							where name = @MCNo AND idx = 1) 
		AND APCSProDB.jig.categories.name = 'HP')
		...
		SET @FramePP = (SELECT DISTINCT APCSProDB.jig.productions.name FROM APCSProDB.trans.jigs 
		  ...
		  --(SELECT [jig_id]
				...
				--							INNER JOIN APCSProDB.mc.machines on APCSProDB.trans.machine_jigs.machine_id = APCSProDB.mc.machines.id
				--							where name = @MCNo AND idx = 2) 
		AND APCSProDB.jig.categories.name = 'PP')
		...
		--			  INNER JOIN APCSProDB.jig.productions on APCSProDB.jig.productions.id = APCSProDB.trans.jigs.jig_production_id  
		--			  INNER JOIN APCSProDB.jig.categories on APCSProDB.jig.categories.id = APCSProDB.jig.productions.category_id 
		--			  INNER JOIN (SELECT jr.jig_id,id,record_class,mc_no FROM
		--						  [APCSProDB].[trans].[jig_records] jr
		--						  INNER JOIN (SELECT MAX(id) as max_id,jig_id FROM APCSProDB.trans.jig_records GROUP BY jig_id) AS mjr 
		--						  on mjr.max_id = jr.id AND jr.jig_id = mjr.jig_id
		--						  ) AS jrTS on jrTS.jig_id = jigs.id AND jrTS.record_class = jigs.jig_state 
