CREATE PROCEDURE [jig].[sp_get_kanagata_checkpackage]
	@kanagataNo as VARCHAR(100) = NULL, --'FI-009','T-178'
...
BEGIN
	...
	FROM      APCSProDB.trans.jigs INNER JOIN
							...
							where APCSProDB.method.jig_sets.name = @Package and APCSProDB.trans.jigs.qrcodebyuser = @kanagataNo and (jig_sets.is_disable is null or jig_sets.is_disable =0)) 
	BEGIN
	...
	ELSE BEGIN
		SELECT 'FALSE' AS Is_Pass,'This package ('+ @Package +') cannot be used with a Kanagata type ('+ name +'). !!' AS Error_Message_ENG
					,N'Package ('+ @Package +N') นี้ไม่สามารถใช่้กับ Kanageta Type ('+ name +N') นี้ได้ !!' AS Error_Message_THA
