...
-- =============================================
CREATE PROCEDURE  [jig].[sp_get_kanagata_predictiveshot_v2]
			...
			where jigs.id <> @root_id and root_jig_id = @root_id) as table1 where table1.Cul_ShotPerFrame = 'Expire')
				begin
					Select 'FALSE' as Is_Pass,'Kanagata Part Life Time expire. !!' AS Error_Message_ENG
						...
						,MAX(FORMAT((CONVERT (decimal(18 , 2),APCSProDB.trans.jig_conditions.value) / CONVERT (decimal(18 , 2),APCSProDB.jig.productions.expiration_value)) * 100, 'N2')) AS LifeTime_Percen 
