...
-- =============================================
CREATE PROCEDURE [jig].[sp_set_tsukiage_setup_v2]
...
BEGIN
	...
	SET @LifeTime = (SELECT  TOP 1 (APCSProDB.trans.jig_conditions.value / 1000) 
						FROM APCSProDB.trans.jigs INNER JOIN
						APCSProDB.trans.jig_conditions ON APCSProDB.trans.jigs.id = APCSProDB.trans.jig_conditions.id INNER JOIN
	...
	IF @Status  = 'To Machine' BEGIN
			...
			BEGIN
				...
				BEGIN
						...
						--//////////UPDATE JIG OLD
							UPDATE APCSProDB.trans.jigs set status = 'To Machine',[jig_state] = 11,updated_at = GETDATE(),updated_by = @OPID where id = @JIGIdOld
							...
							(SELECT jig_production_id FROM APCSProDB.trans.jigs where id = @JIGIdOld), GETDATE(),@OPID,@OPNo,'To Machine')
		...
		SELECT DISTINCT 'TRUE' AS Is_Pass,''AS Error_Message_ENG,'' AS Error_Message_THA,''AS Handling,
			smallcode AS SmallCode, productions.name AS Type_Name, tsukiage_recipes.tsukiage_no AS TsukiageNo  
		FROM APCSProDB.trans.jigs 
			INNER JOIN APCSProDB.jig.productions ON jigs.jig_production_id = productions.id 
			INNER JOIN APCSProDB.jig.tsukiage_recipes ON productions.id = tsukiage_recipes.production_id
