...
-- =============================================
CREATE PROCEDURE [jig].[sp_get_kanagata_total_record_endlot_v2]
...
BEGIN
	...
	,ISNull((SELECT BigSpringMax = Node.Data.value('(BigSpringMax)[1]', 'INT') FROM extend_data.nodes('EndLots/EndLot') Node(Data)),0)AS BigSpringMax
	,ISNull((SELECT SmallSpringMax = Node.Data.value('(SmallSpringMax)[1]', 'INT') FROM extend_data.nodes('EndLots/EndLot') Node(Data)),0)AS SmallSpringMax
	,ISNull((SELECT FinGateGuidePunchMax = Node.Data.value('(FinGateGuidePunchMax)[1]', 'INT') FROM extend_data.nodes('EndLots/EndLot') Node(Data)),0)AS FinGateGuidePunchMax
	,ISNull((SELECT StripperBoltMax = Node.Data.value('(StripperBoltMax)[1]', 'INT') FROM extend_data.nodes('EndLots/EndLot') Node(Data)),0)AS StripperBoltMax
	,LotPM.Comment
	,LotPM.PMNo
	,LotPM.ResetTime 
