...
CREATE  PROCEDURE [jig].[sp_set_socket_repair]
(
		  @Process_id		INT				= NULL
...
BEGIN
	SET NOCOUNT ON;
				...
				SELECT	  @jig_id				=  jigs.id
						...
						, @accumulate_lifetime	=  IIF(jig_conditions.accumulate_lifetime IS NULL,jig_conditions.value,accumulate_lifetime + jig_conditions.[value]  )
...
BEGIN  	
	...
	IF (@Img1 IS NULL )
	BEGIN 
				IF (@jig_state NOT IN (4,6) )--12	On Machine , 2 Stock , 14 Scrap 
				BEGIN 
	...
	BEGIN 
		...
		SET  [value]				= 0
			, accumulate_lifetime	=  @accumulate_lifetime
		WHERE id =  @jig_id
		...
		SELECT TOP(1)  'TRUE'										AS Is_Pass 
					 ...
					 , ISNULL(jig_records.operated_by, 1)			AS MyUserID
