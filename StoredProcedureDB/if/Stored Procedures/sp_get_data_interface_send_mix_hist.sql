...
-- =============================================
CREATE PROCEDURE [if].[sp_get_data_interface_send_mix_hist]
...
BEGIN
	...
	SET NOCOUNT ON;
	----Test day
	--SET @FromDate = '2025-05-12 00:00:00';
	--SET @ToDate = '2025-05-12 23:59:59';
	----Test month
	...
	FROM (
		SELECT  [A].*
			...
			, CASE 
				...
				ELSE 0 
			END AS [condition] 
		FROM #TableTempMixHist AS [A]
		LEFT JOIN (
			SELECT [AB].[HASUU_LotNo]
				, SUM(ISNULL([AB].[QTY], 0)) AS [QTY] 
			FROM #TableTempMixHist AS [AB]
