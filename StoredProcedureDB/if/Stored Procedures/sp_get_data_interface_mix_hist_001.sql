...
-- =============================================
CREATE PROCEDURE [if].[sp_get_data_interface_mix_hist_001]
...
BEGIN
	...
	--	, [TimeStamp_time]
	--FROM (
	--	SELECT CAST('' AS CHAR(10)) AS [M_O_No]
	--		, CAST('' AS CHAR(2)) AS [FREQ]
	--		, CAST([lot_mas].[lot_no] AS CHAR(10)) AS [HASUU_LotNo]
	--		, CAST([lot_mem].[lot_no] AS CHAR(10)) AS [LotNo]
	--		, CAST('' AS CHAR(20)) AS [P_O_No] 
	--		, CAST([sur_lot_master].[stock_class] AS CHAR(2)) AS [Stock_Class]
	--		, CAST([packages].[name] AS CHAR(10)) AS [Type_Name]
	--		, CAST([device_names].[name] AS CHAR(20)) AS [ROHM_Model_Name]
	--		, CAST([surpluses].[pdcd] AS CHAR(5)) AS [PDCD]
	--		, CAST([device_names].[assy_name] AS CHAR(20))  AS [ASSY_Model_Name]
	--		, CAST(ISNULL([Fukuoka].[R_Fukuoka_Model_Name], '') AS CHAR(20)) AS [R_Fukuoka_Model_Name]
	--		, CAST(ISNULL([device_names].[rank], '') AS CHAR(5)) AS [TIRank]
	--		, CAST(ISNULL([device_names].[rank], '') AS CHAR(3)) AS [Rank]
	--		, CAST(ISNULL([device_names].[tp_rank], '') AS CHAR(3)) AS [TPRank]
	--		, CAST('' AS CHAR(3)) AS [SUBRank]
	--		, CAST('' AS CHAR(2)) AS [Mask]
	--		, CAST('' AS CHAR(3)) AS [KNo]	
	--		, CAST([surpluses].[mark_no] AS CHAR(10)) AS [MNo]
	--		, CAST('' AS CHAR(4)) AS [Tomson1]
	--		, CAST('' AS CHAR(4)) AS [Tomson2]
	--		, CAST(ISNULL([surpluses].[qc_instruction], '') AS CHAR(4)) AS [Tomson3]
	--		, FORMAT(ISNULL([allocat].[allocation_Date], [lot_combine].[created_at]), 'yyyy-MM-dd HH:mm:ss') AS [allocation_Date]
	--		, ( CASE 
	--			WHEN SUBSTRING([lot_mem].[lot_no], 5, 1) = 'D' THEN CAST('NO' AS CHAR(12))
	--			ELSE CAST(ISNULL([allocat].[ORNo], '') AS CHAR(12))
	--		END ) AS [ORNo]
	--		, CAST(ISNULL([allocat].[WFLotNo], '') AS CHAR(20)) AS [WFLotNo]
	--		, CAST('' AS CHAR(4)) AS [User_Code]
	--		, CAST('' AS CHAR(1)) AS [LotNo_Class]
	--		, CAST(ISNULL([surpluses].[label_class], '') AS CHAR(1)) AS [Label_Class]
	--		, CAST('' AS CHAR(1)) AS [Multi_Class]
	--		, CAST(ISNULL([surpluses].[product_control_class], '') AS CHAR(3)) AS [Product_Control_Clas]
	--		, [device_names].[pcs_per_pack] AS [Packing_Standerd_QTY]
	--		, CAST('' AS CHAR(3)) AS [Date_Code]
	--		, CAST('' AS CHAR(1)) AS [HASUU_Out_Flag]
	--		, ( CASE 
	--				-- D Lot (LotMaster)
	...
	--				---ELSE [surpluses].[pcs] 
	--		END ) AS [QTY]
	--		, CAST('' AS CHAR(1)) AS [Transfer_Flag]
	--		, IIF([lot_mem].[pc_instruction_code] = 13, [surpluses].[pcs], 0) AS [Transfer]
	--		, CAST([lot_combine].[created_by] AS CHAR(5)) AS [OPNo]
	--		, CAST('' AS CHAR(1)) AS [Theoretical]
	--		, CAST('B' AS CHAR(1)) AS [OUT_OUT_FLAG]
	...
	--		, IIF(([lot_mas].[qty_pass] + [lot_mas].[qty_combined]) < [dn_mas].[pcs_per_pack], IIF([lot_mas].[pc_instruction_code] IN (1, 11, 13), 0, 1), 0) AS [condition]
	...
	--	) AS [lot_combine]
	--	INNER JOIN [APCSProDB].[trans].[lots] AS [lot_mas] ON [lot_combine].[lot_id] = [lot_mas].[id]
	--		AND [lot_mas].[wip_state] != 200
	--	INNER JOIN [APCSProDB].[method].[device_names] AS [dn_mas] ON [lot_mas].[act_device_name_id] = [dn_mas].[id]
	--	INNER JOIN [APCSProDB].[trans].[lots] AS [lot_mem] ON [lot_combine].[member_lot_id] = [lot_mem].[id]
	--	LEFT JOIN [APCSProDB].[method].[allocat_temp] AS [allocat] ON [lot_mem].[lot_no] = [allocat].[LotNo]
	...
	--	INNER JOIN [APCSProDB].[trans].[surpluses] ON [surpluses].[lot_id] = [lot_combine].[member_lot_id]
	--	INNER JOIN [APCSProDB].[trans].[surpluses] AS [sur_lot_master] ON [sur_lot_master].[lot_id] = [lot_combine].[lot_id]
	--	OUTER APPLY (
	--		SELECT TOP 1 [lot_record].*
	...
	--		WHERE TRIM([allocat_temp].[ROHM_Model_Name]) = TRIM([device_names].[name])
	--			AND TRIM([allocat_temp].[ASSY_Model_Name]) = TRIM([device_names].[assy_name])
	--	) AS [Fukuoka]
	--) AS [A]
	--WHERE [condition] = 0
	--	AND [production_category] NOT IN (21, 22)
	--	AND ( [pc_instruction_code] != 11 
	--		OR ([pc_instruction_code] = 11 AND [lot_type] = 'D')
	--	) 
	--ORDER BY [MIXD_DATE], [HASUU_LotNo];

	DECLARE @table_Fukuoka TABLE 
	(
		[ROHM_Model_Name] VARCHAR(20) ,
		[ASSY_Model_Name] VARCHAR(20) ,
		[R_Fukuoka_Model_Name] VARCHAR(20)
	);

	INSERT INTO @table_Fukuoka
	...
	FROM (
		...
		FROM (
			...
			OUTER APPLY (
				SELECT TOP 1 [R_Fukuoka_Model_Name]
				FROM @table_Fukuoka AS[allocat_temp]
				WHERE TRIM([allocat_temp].[ROHM_Model_Name]) = TRIM([device_names].[name])
					AND TRIM([allocat_temp].[ASSY_Model_Name]) = TRIM([device_names].[assy_name])
			) AS [Fukuoka]
		) AS [A]
		LEFT JOIN (
			SELECT [A].[HASUU_LotNo]
				, SUM(ISNULL([A].[QTY], 0)) AS [QTY]
			FROM (
				SELECT CAST([lot_mas].[lot_no] AS CHAR(10)) AS [HASUU_LotNo]
					, ( CASE 
							-- D Lot (LotMaster)
							WHEN [lot_combine].[lot_id] = [lot_combine].[member_lot_id] AND SUBSTRING([lot_mas].[lot_no], 5, 1) = 'D' 
								THEN IIF([lot_mas].[pc_instruction_code] IN (1,11,13), [lot_mas].[qty_out], [lot_mem].[qty_in]) 
	...
	--FULL OUTER JOIN @table1 AS [MIX_1] ON [A].[HASUU_LotNo] = [MIX_1].[HASUU_LotNo]
	--	AND [A].[LotNo] = [MIX_1].[LotNo]
	WHERE [condition] = 0
		--AND [A].[QTY] != [MIX_1].[QTY]
		--AND [A].[HASUU_LotNo] = '2421A6331V'
