...
-- =============================================
CREATE PROCEDURE [if].[sp_set_data_stock_text_ver_002]
...
BEGIN
	...
	SELECT DISTINCT [sur].[stock_class] AS [stock_class]
		...
		, ( CASE
			WHEN [lots].[wip_state] IN (100, 70) THEN 0
			ELSE [lots].[qty_out]
		END ) AS [wip_qty]
		, ( CASE
	...
	WHERE [sur].[in_stock] IN (2, 4)
		AND [sur].[pcs] >= 0
		AND (
			(SUBSTRING([lots].[lot_no], 5, 1) <> 'D' AND [lots].[wip_state] IN (70, 100))
			OR (SUBSTRING([lots].[lot_no], 5, 1) = 'D')
		);
END