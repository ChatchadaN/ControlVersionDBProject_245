...
CREATE PROCEDURE [req].[sp_get_request_order] 
...
BEGIN
	...
	BEGIN
		...
		WHERE ([orders].[order_no] = @order_no and @order_no is not null) --add order_no is not null by far 2025/02/13 14.30
	...
	BEGIN
		...
		BEGIN
			...
			WHERE 
			[orders].[state] LIKE 0
			and [inchanges].[inchange_by] = @emp_id
		...
		BEGIN
			...
			WHERE 
			[orders].[state] in (1 , 2, 3, 4)
			and [orders].[requested_by] = @emp_id
			ORDER BY records.last_record DESC
		END
	END
	ELSE IF (@is_function = 3) --Get Data show on report #create at 2025/05/06 time : 14.21 by Aomsin
	BEGIN
		SELECT [A2].*
