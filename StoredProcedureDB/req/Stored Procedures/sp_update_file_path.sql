...
-- =============================================
CREATE PROCEDURE [req].[sp_update_file_path]
	-- Add the parameters for the stored procedure here
	 @order_no VARCHAR(11) = ''
	,@file_path NVARCHAR(255) = ''
	,@ImgTable image_paths READONLY --test 2025-06-02 far
AS
BEGIN
	...
	--			--+ ''', @ImgTable = ''' + (select COUNT(*) as CountRow from @ImgTable)
	--			+ ''''
	...
	BEGIN
		...
		SET @order_id = (SELECT [id] from [AppDB_app_244].[req].[orders] where [order_no] = @order_no);
		SET @requested_by = (SELECT [requested_by] FROM [AppDB_app_244].[req].[orders] where [order_no] = @order_no);
