...
CREATE PROCEDURE [req].[sp_update_request_order_ver_002] 
...
BEGIN
	...
	, @Id_reqestor int = null
	, @status_current TINYINT = NULL

	SELECT @empId = CONVERT(int,@empId_save)
	SELECT @get_request_id = [id] 
		,@Id_reqestor = requested_by
		,@status_current = state
	FROM [AppDB_app_244].[req].[orders] WHERE [order_no] = @order_no
	...
	BEGIN TRANSACTION;
	BEGIN TRY
		IF (@empId_save != @Id_reqestor and @status_current = '6' and @is_status = '3')
		begin
			COMMIT;
			SELECT 'FALSE' AS [Is_Pass] 
				, 'User information does not match !!' AS [Error_Message_ENG]
				, N'ข้อมูลผู้ใช้ไม่ตรงกัน !!' AS [Error_Message_THA] 
				, N'' AS [Handling];
			RETURN;
		end
		else if(@status_current != '2' and @status_current != '6' and @is_status = '6')
		begin
			COMMIT;
			SELECT 'FALSE' AS [Is_Pass] 
				, 'Data is incorrect !!' AS [Error_Message_ENG]
				, N'Wait for Reqrestor Status จะต้องเลือกหลังจาก Do Status เท่านั้น!!' AS [Error_Message_THA] 
				, N'' AS [Handling];
			RETURN;
		end
		...
		BEGIN
			...
			EXEC [StoredProcedureDB].[req].[sp_set_send_mail_notification] @request_id = @get_request_id,  @send_mail = @send_to_mail, @set_page = @set_page --add mail 2025-06-04
