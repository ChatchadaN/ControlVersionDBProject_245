...
CREATE PROCEDURE [req].[sp_get_data_graph_ver_002] 
		...
		SELECT [t_main].[state]
			...
			left join (
				SELECT [item_labels].[val] AS [state]
					,[item_labels].[label_eng] AS [state_name]
					,[item_labels].[color_code]
					,isnull(COUNT([orders].[state]),0) AS [count]
				FROM [APCSProDWR].[req].[item_labels] 
				LEFT JOIN [APCSProDWR].[req].[orders] ON [orders].[state] = [item_labels].[val] 
					--AND (CONVERT(DATE, [orders].[requested_at]) BETWEEN convert(date,@start_date) AND convert(date,@end_date)) 
					AND ((CONVERT(DATE, [orders].[requested_at]) BETWEEN convert(date,@start_date) AND convert(date,@end_date)) or (@start_date is null and @end_date is null))
					--AND ((CONVERT(DATE, [orders].[requested_at]) BETWEEN convert(date,@start_date) AND convert(date,@end_date)) or (@start_date ='' and @end_date ='')or (@start_date is null and @end_date is null))
