...
-- =============================================
CREATE PROCEDURE [cac].[sp_get_wip_monitor_delay_lot_condition_table]
...
BEGIN
	...
	BEGIN
		...
		, [item_labels7].[label_eng] as [quality_status]
		, case when [HoldLot].[StartTime] is not null then DATEDIFF(day, [HoldLot].[StartTime],GETDATE()) else 0 end as [hold_date]
		, [wip_monitor_delay_lot_condition_detail].[contact_pic_id] as [contact_pic]
		, [wip_monitor_delay_lot_condition_detail].[response_plant_id] as [response_plant]
		...
		left join [DBx].[QAS].[HoldLotAbnormal] as [HoldLot] with (NOLOCK) on [HoldLot].[System] = 'HOLDLOT' and [HoldLot].[LotNo] = [lots].[lot_no]
		left join [APCSProDB].[trans].[item_labels] as [item_labels7] with (NOLOCK) on [item_labels7].[name] = 'lots.quality_state' and [item_labels7].[val] = [lots].[quality_state]
  ...
  --		from (
		...
		--	, [item_labels7].[label_eng] as [quality_status]
		--	, case when [HoldLot].[StartTime] is not null then DATEDIFF(day, [HoldLot].[StartTime],GETDATE()) else '0' end as [hold_date]
		--	, case when [lots].[updated_at] < [days1].[date_value] then [days1].[date_value] else [lots].[updated_at] end as [update_where]
		--	, [wip_monitor_no_movement_lot_detail].[contact_pic_id] as [contact_pic]
		--	, [wip_monitor_no_movement_lot_detail].[response_plant_id] as [response_plant]
		--	from [APCSProDB].[trans].[lots] with (NOLOCK) 
		--	inner join [APCSProDB].[method].[device_slips] with (NOLOCK) on [device_slips].[device_slip_id] = [lots].[device_slip_id]
		--	inner join [APCSProDB].[method].[device_versions] with (NOLOCK) on [device_versions].[device_id] = [device_slips].[device_id]
		--	inner join [APCSProDB].[method].[device_names] with (NOLOCK) on [device_names].[id] = [device_versions].[device_name_id]
		...
		--	--order by [lots].[lot_no]
		--) as [data]
		--where DATEDIFF(day,[data].[update_where],GETDATE()) >= @day_condition
		--order by [data].[lot]
		...
		FROM (
			...
			--INNER JOIN [APCSProDB].[method].[device_slips] WITH (NOLOCK) ON [device_slips].[device_slip_id] = [lots].[device_slip_id]
			--INNER JOIN [APCSProDB].[method].[device_versions] WITH (NOLOCK) ON [device_versions].[device_id] = [device_slips].[device_id]
			INNER JOIN [APCSProDB].[method].[device_names] WITH (NOLOCK) ON [device_names].[id] = [lots].[act_device_name_id] --[device_versions].[device_name_id]
			INNER JOIN [APCSProDB].[method].[packages] WITH (NOLOCK) ON [packages].[id] = [device_names].[package_id]
			INNER JOIN [APCSProDB].[method].[package_groups] WITH (NOLOCK) ON [package_groups].[id] = [packages].[package_group_id]
			...
			LEFT JOIN [APCSProDB].[method].[processes] AS [processes2] WITH (NOLOCK) ON [processes2].[id] = [job2].[process_id]
			LEFT JOIN [APCSProDWH].[cac].[wip_monitor_no_movement_lot_detail] WITH (NOLOCK) ON [wip_monitor_no_movement_lot_detail].[lot_no] = [lots].[lot_no]
			LEFT JOIN [DBx].[QAS].[HoldLotAbnormal] AS [HoldLot] WITH (NOLOCK) ON [HoldLot].[System] = 'HOLDLOT' 
				AND [HoldLot].[LotNo] = [lots].[lot_no]
			LEFT JOIN [APCSProDB].[trans].[item_labels] AS [item_labels7] WITH (NOLOCK) ON [item_labels7].[name] = 'lots.quality_state' 
				AND [item_labels7].[val] = [lots].[quality_state]
			WHERE [lots].[wip_state] IN (20,10,0)
				AND [days].[date_value] <= CONVERT(DATE, GETDATE())
				AND NOT(DATEDIFF(DAY,[days2].[date_value],@date_value) >= @day_delay_condition)
				AND DATEDIFF(day,[lots].[updated_at],GETDATE()) >= @day_condition
