...
-- =============================================
CREATE PROCEDURE [rcs].[sp_set_maching_rack_device]
...
BEGIN
	...
	BEGIN TRY
		...
		IF EXISTS(SELECT 1 FROM APCSProDB.rcs.rack_devices rack_devices
		...
		BEGIN
			...
			--check rack นี้เคยมีการ regis มาก่อนมั้ย ถ้ามี Max(prio) + 1 ไม่มี set device = 1
			IF EXISTS(SELECT 1 FROM APCSProDB.rcs.rack_devices WHERE rack_control_id = @rack_control_id)
			BEGIN
				print 'new device : same rack'
				SET @priority_old = (SELECT MAX([priority]) FROM APCSProDB.rcs.rack_devices WHERE rack_control_id = @rack_control_id)
				SET @priority_new = @priority_old + 1
			...
			ELSE
			BEGIN
				print 'new device : not same rack'
				SET @priority_new = 1
