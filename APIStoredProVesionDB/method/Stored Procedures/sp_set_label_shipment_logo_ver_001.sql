...
-- =============================================
CREATE PROCEDURE [method].[sp_set_label_shipment_logo_ver_001] 
...
BEGIN
	...
	BEGIN TRY
        ...
        BEGIN
            -- ถ้ามีอยู่แล้ว ให้ส่งข้อความว่าไม่สามารถบันทึกได้
				...
				SET @SQL = N'
					...
					SET 
						...
						file_extension = '''+ISNULL(@file_extension,'')+''',
						picture_data = CONVERT(VARBINARY(MAX), ''' + @picture_data + '''),
						updated_at = GETDATE(),
			...
			--,@parameter,@description=@description,@file_extension = @file_extension,@picture_data=@picture_data,@emp_id=@emp_id,@value=@value;
		...
		-- insert data into [APCSProDBFile].[method].[label_shipment_logo] 
			...
			SET @SQL = N'
				INSERT INTO [APCSProDBFile].[method].[label_shipment_logo]
					([value], [description], [file_extension], [picture_data], [created_at], [created_by])
				VALUES (
					' + CAST(@value AS VARCHAR) + ',
					''' + @description + ''',
					''' + @file_extension + ''',
					CONVERT(VARBINARY(MAX), ''' + @picture_data + '''),
					GETDATE(),
