...
-- =============================================
CREATE PROCEDURE [man].[sp_get_employee_list_001]
...
BEGIN
	...
	,shift_calendars.name AS shift_name
	...
	FROM [DWH].[man].[employees]
	 ...
	 INNER JOIN [DWH].man.shift_calendars ON [DWH].man.employees.shift_id = [DWH].man.shift_calendars.id
	 ...
	 LEFT JOIN [DWH].man.positions 
	 ON [DWH].man.employees.position_id = [DWH].man.positions.id
     WHERE ( [employees].[emp_code] = @emp_code  OR ISNULL(@emp_code,'') = '') AND (headquarters.id = @hq_id or ISNULL(@hq_id,0) = 0 )
END
