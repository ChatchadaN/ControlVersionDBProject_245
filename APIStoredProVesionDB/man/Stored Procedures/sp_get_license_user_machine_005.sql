...
-- =============================================
CREATE PROCEDURE [man].[sp_get_license_user_machine_005]
...
BEGIN
	...
	SELECT GETDATE()
		...
		, HOST_NAME()
		, APP_NAME()
		, 'EXEC [mc].[sp_get_license_user_machine_005] @emp_code = ''' + @emp_code 
			+ ''', @machine_model = ''' + @machine_model 
			+ ''', @machine_name = ''' + @machine_name + ''''
	
	IF(@is_automotive =  0)
	BEGIN 
		IF NOT EXISTS ( SELECT 'xx' FROM [10.29.1.116].[TEC_Skill_TestDemo].[dbo].[vw_LastMStaffLicense_model_All]
			WHERE Staffcode =  @emp_code 
			AND model_name  = @machine_model
			AND [License]  != 'AUTOMOTIVE')
		BEGIN 
			SELECT    '3002'			AS code
		...
		BEGIN			
			SELECT      
				CASE WHEN  DATEADD(MONTH, 6, CertificationDate) <= GETDATE()  THEN 128  ELSE 200 END AS code
	...
	BEGIN
		...
		IF EXISTS(SELECT 'xxx'  FROM [10.29.1.116].[TEC_Skill_TestDemo].[dbo].[vw_LastMStaffLicense_model_All]
			WHERE [License] = 'AUTOMOTIVE' 
