...
-- =============================================
CREATE PROCEDURE  [jig].[sp_get_jig_check_004]
(
		...
		END
			...
			BEGIN
			 ...
			 --			AND  jigs.id					= @JIG_ID 
				...
				SET @STDLifeTime = (SELECT TOP 1 CASE WHEN  productions.expiration_base  = 0 THEN  production_counters.alarm_value  ELSE  productions.expiration_base  END 
											FROM APCSProDB.trans.jigs
											INNER JOIN APCSProDB.trans.jig_conditions 
		 			...
		 			SET @LifeTime		=	(SELECT TOP 1   CASE WHEN @process_id =  4     --MP check life time =value+(warn_value/ f-press / qty(kanagata count)
											THEN CONVERT(INT,(CONVERT(INT,max_lifetime.LifeTime)  + (ISNULL(warn_value,1)  / @INPUT_QTY / @QTY)))
											ELSE
											([LifeTime] + ISNULL(warn_value, 1))
											END  AS  [Lifetime]
										FROM  
						...
						BEGIN
								...
								FROM  
									...
									ORDER BY max_lifetime.LifeTime_Percen DESC , Life_Time DESC
								...
								--		, ISNULL(jig_set_list.use_qty,0)					AS QTY
								--		, CONVERT(int,@LifeTime)							AS Life_Time
								--		, @STDLifeTime  AS STD_Life_Time			--CONVERT(int,production_counters.alarm_value)		AS STD_Life_Time
								--		, jigs.id											AS jig_id
								--		, FORMAT(COALESCE((CAST(jig_member.value  AS decimal(18 , 2)) * CAST(100  AS decimal(18 , 2)))/ NULLIF(jig_member.expiration_value,0) ,0), 'N2') AS LifeTime_Percen
								--		, @periodcheck_value								AS  periodcheck_value
								--		, @period_value										AS  period_value
								--FROM 
								--	(SELECT  CASE WHEN lots.is_special_flow =  1 THEN    [lot_special_flows].jig_set_id 
								...
								--INNER JOIN APCSProDB.method.jig_sets					
								--ON jig_sets.process_id					=	sp_jig.act_process_id
								--OR sp_jig.jig_set_id				=   jig_sets.id
								--INNER JOIN APCSProDB.method.jig_set_list			
								--ON jig_sets.id							=	jig_set_list.jig_set_id
								--INNER JOIN APCSProDB.jig.productions					
								--ON productions.id						=	jig_set_list.jig_group_id
