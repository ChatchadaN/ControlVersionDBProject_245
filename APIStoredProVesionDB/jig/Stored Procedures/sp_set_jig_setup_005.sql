...
-- =============================================
CREATE PROCEDURE [jig].[sp_set_jig_setup_005]
...
BEGIN
    ...
    -- Insert statements for procedure here
		...
		SELECT    GETDATE()
				...
				, HOST_NAME()
				, APP_NAME()
				, 'EXEC [jig].[sp_set_jig_setup] @JIG_ID = ''' + ISNULL(CAST(@JIG_ID AS nvarchar(MAX)),'') 
				  + ''', @QRCode = ''' + ISNULL(CAST(@QRCode AS nvarchar(MAX)),'') +''', @Recipe = ''' 
				  + ISNULL(CAST(@Recipe AS nvarchar(MAX)),'')+  ''',@OpNO = ''' 
...
--lot no ต้องเช็คด้วยว่าส่งหรือไม่ส่ง เพราะเป็นจังหวะ set ถ้าไม่มี lot ก็ต้อง setup ได้
		...
		BEGIN 
			--Check PP To machine and BM request machine
				IF @MCNo <> (SELECT TOP 1 mc_no FROM APCSProDB.trans.jig_records WHERE jig_records.jig_id = @JIG_ID 
				...
				BEGIN
						...
						SELECT	  'FALSE' AS Is_Pass
								...
								, N'กรุณาตรวจสอบ MCNo ของ ' + @Shot_name +'  ที่เว็บ JIG System' AS Handling
						FROM APCSProDB.trans.jigs 
						WHERE jigs.id = @JIG_ID
				...
				BEGIN
						...
						SET @JIG_OLD = (SELECT jig_id FROM APCSProDB.trans.machine_jigs WHERE machine_id = @mcid AND idx = 2)
		...
		BEGIN 
				...
				END
				ELSE IF  EXISTS (SELECT 1 FROM APCSProDB.trans.machine_jigs WHERE machine_id = @MCId AND idx = 1)
				BEGIN 
							SELECT  TOP 1  'FALSE'														AS Is_Pass
									, N'This JIG ('+ jigs.barcode + N') Is use on Machine ('+ machines.name + N')!!' AS Error_Message_ENG
									, N'JIG นี้ ('+ jigs.barcode + N') ถูกใช้งานอยู่ที่ Machine เครื่องนี้ ('+ machines.name + N') !!'	AS Error_Message_THA
				...
				END
		 END
		IF (@Shot_name = 'Tsukiage')
		BEGIN  
				 
				IF EXISTS(SELECT TOP(1) machine_id 
							FROM APCSProDB.trans.jigs
							...
							WHERE machine_id = @MCId  AND categories.name = @Shot_name) 
				BEGIN
						SET @JIG_OLD = (SELECT TOP(1) jig_id from APCSProDB.trans.jigs
										LEFT JOIN APCSProDB.trans.machine_jigs on machine_jigs.jig_id = jigs.id
										LEFT JOIN APCSProDB.mc.machines on machines.id = machine_jigs.machine_id
										INNER JOIN APCSProDB.jig.productions on productions.id = jigs.jig_production_id 
										INNER JOIN APCSProDB.jig.categories on categories.id = productions.category_id  
							...
							(
									...
									, GETDATE()
									, @JIG_OLD
									, (SELECT jig_production_id FROM APCSProDB.trans.jigs where id = @JIG_OLD)
									, GETDATE()
									, @OPID
									, @OPNo
									, 'To Machine'
				...
				ELSE BEGIN
					...
					update APCSProDB.trans.jigs set location_id = NULL,status = 'On Machine',[jig_state] = 12,updated_at = GETDATE(),updated_by = @OPID where id = @JIG_OLD
					...
					BEGIN 
							...
							VALUES (@MCId,1,1,@JIG_OLD,GETDATE(),@OPID)

					END
					ELSE BEGIN
							INSERT INTO APCSProDB.trans.machine_jigs (machine_id,idx,jig_group_id,jig_id,created_at,created_by) 
							VALUES (@MCId,2,1,@JIG_OLD,GETDATE(),@OPID)
	...
	BEGIN  IF NOT EXISTS (  SELECT TOP 1 jig_sets.id 
						   FROM 
							...
							INNER JOIN APCSProDB.trans.jigs
							ON productions.id				= jigs.jig_production_id
							WHERE (jigs.id   = @JIG_ID  )  AND ISNULL(jig_sets.is_disable, 0) =  0
	...
	BEGIN  
			IF  EXISTS (	SELECT TOP 1 jig_sets.id 
							...
							ON  jig_sets.process_id = sp_jig.act_process_id
							AND jig_sets.id = sp_jig.jig_set_id
							AND jig_sets.[name] =  package_name
							INNER JOIN APCSProDB.method.jig_set_list
							ON jig_sets.id =  jig_set_list.jig_set_id
							INNER JOIN APCSProDB.jig.productions
							ON productions.id = jig_set_list.jig_group_id
						...
						BEGIN
							...
							SELECT  TOP 1    'TRUE'											AS Is_Pass		
								...
								FROM  
									(	SELECT		  warn_value AS warn_value
													, APCSProDB.trans.jig_conditions.value   AS [LifeTime]
													, FORMAT(COALESCE((CAST(jig_conditions.value AS decimal(18 , 2)) * CAST(100  AS decimal(18 , 2)))/ NULLIF(productions.expiration_value,0) ,0), 'N2') AS LifeTime_Percen
													, jigs.smallcode
													, productions.name		AS [Type]
													, jigs.id				AS jig_id 
							...
							BEGIN
									...
									SET @JIG_OLD =	(SELECT jig_id   FROM APCSProDB.trans.machine_jigs where machine_id = @MCId and idx = 1 )
	...
	BEGIN  
		...
		END
		BEGIN TRY 
		IF (@Shot_name = 'Capillary')
		BEGIN
			 IF EXISTS (SELECT 1 FROM APCSProDB.trans.machine_jigs WHERE machine_id = @mcid AND idx = 3)
				BEGIN
					SET @JIG_OLD = (SELECT jig_id FROM APCSProDB.trans.machine_jigs WHERE machine_id = @mcid AND idx = 3)
					...
					ON  jig_sets.process_id = sp_jig.act_process_id
					AND jig_sets.id = sp_jig.jig_set_id
					AND jig_sets.code = sp_jig.recipe
					INNER JOIN APCSProDB.method.jig_set_list
					ON jig_sets.id =  jig_set_list.jig_set_id
					INNER JOIN APCSProDB.jig.productions
					ON productions.id = jig_set_list.jig_group_id
		...
		BEGIN
			...
			END
		END 
		ELSE IF(@Shot_name = 'Wedge') --idx : 31,32
		BEGIN 
		SET @idx  = 31
			...
			END
		END
		ELSE IF(@Shot_name = 'Wire')		--idx : 33,34
		BEGIN 
		...
		SET @idx  = 33
			...
			END
		END
		ELSE IF(@Shot_name = 'Cutter') --idx : 35,36
		BEGIN 
		SET @idx  = 35
