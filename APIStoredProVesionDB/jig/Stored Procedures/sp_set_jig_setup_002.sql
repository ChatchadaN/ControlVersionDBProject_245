...
-- =============================================
CREATE PROCEDURE [jig].[sp_set_jig_setup_002]
...
BEGIN
	...
	SELECT	  @JIG_ID				= jigs.id 
			...
			, @STDLifeTime			= productions.expiration_value
...
--lot no ต้องเช็คด้วยว่าส่งหรือไม่ส่ง เพราะเป็นจังหวะ set ถ้าไม่มี lot ก็ต้อง setup ได้
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN 
					SET @JIG_OLD =	(SELECT jig_id as Detail FROM APCSProDB.trans.machine_jigs WHERE machine_id = @MCId and idx = 1 )
					IF @JIG_ID <> @JIG_OLD 
					BEGIN
						 
							--/////////Check JIG On Machine Old //OR// JIG NEW
							IF NOT EXISTS( SELECT 1 FROM APCSProDB.trans.machine_jigs WHERE jig_id = @JIG_ID AND machine_id = @MCId) 
							BEGIN
							...
							SELECT   'TRUE'												AS Is_Pass
										...
										, 0													AS QTY
										, CAST(jig_conditions.value AS INT)					AS Life_Time
										, CAST(production_counters.alarm_value AS INT)		AS STD_Life_Time
										, jigs.id											AS jig_id
										, FORMAT(COALESCE((CAST(jig_conditions.value  AS decimal(18 , 2)) * CAST(100  AS decimal(18 , 2)))/ NULLIF(production_counters.alarm_value,0) ,0), 'N2') AS LifeTime_Percen
										, @periodcheck_value								AS  periodcheck_value
										, @period_value										AS  period_value	
	...
	END
	ELSE 
	BEGIN  IF NOT EXISTS ( SELECT jig_sets.id 
	...
	BEGIN 
		...
		SET @idx  = 31
			...
			BEGIN 
				...
				END 
				IF   EXISTS (SELECT 1 FROM APCSProDB.trans.machine_jigs  WHERE machine_id = @mcid AND idx = @idx AND jig_id =  @JIG_ID)
				BEGIN
						...
						WHERE machine_id = @mcid AND idx = @idx AND jig_id =  @JIG_ID

						RETURN

				END 

				IF   (SELECT COUNT(machine_jigs.machine_id) FROM APCSProDB.trans.machine_jigs  WHERE machine_id = @mcid AND idx BETWEEN 31 AND 32) = 2
				BEGIN
				 
						SELECT    'FALSE'										AS Is_Pass
								, N'PLEASE CLEAR JIG ON MACHINE !!'		AS Error_Message_ENG
								, N'กรุณา CLEAR JIG บน MACHINE ก่อน !!'			AS Error_Message_THA
								, N'กรุณาตรวจสอบข้อมูลที่เว็บ JIG หรือติดต่อ System'		AS Handling
						FROM APCSProDB.trans.machine_jigs  
						INNER JOIN APCSProDB.trans.jigs  
						ON jigs.id = machine_jigs.jig_id
		...
		SET @idx  = 33
			...
			BEGIN
				...
				IF   (SELECT COUNT(machine_jigs.machine_id) FROM APCSProDB.trans.machine_jigs  WHERE machine_id = @mcid AND idx BETWEEN 33 AND 34) = 2
				BEGIN
		...
		SET @idx  = 35
			...
			BEGIN
				...
				IF   (SELECT COUNT(machine_jigs.machine_id) FROM APCSProDB.trans.machine_jigs  WHERE machine_id = @mcid AND idx BETWEEN 35 AND 36) = 2
				BEGIN
