...
-- =============================================
CREATE PROCEDURE [jig].[sp_get_jig_setup_002]
...
BEGIN
	...
	WHERE (barcode = @QRCode OR qrcodebyuser = @QRCode)
		...
		BEGIN 
							...
							BEGIN
								...
								BEGIN
									SELECT TOP 1   'FALSE'								AS Is_Pass
											, 'Kanagata Part Life Time expire ('+ @LifeTime +'/'+@STDLifeTime+') '	AS Error_Message_ENG
	 										, N'Kanakata Part หมดอายุการใช้งาน ('+ @LifeTime +'/'+@STDLifeTime+') '		AS Error_Message_THA
				...
				END
					ELSE  IF (@Shot_name IN ('Cutter', 'Wedge',  'Wire'))
					BEGIN
