...
-- =============================================
CREATE PROCEDURE [jig].[sp_set_carrier_cleaned_001]
...
BEGIN
	...
	SET NOCOUNT ON;
		...
		SELECT    GETDATE()
				...
				, HOST_NAME()
				, APP_NAME()
				, 'EXEC [jig].[sp_set_carrier_cleaned] @JIG_ID = ''' + ISNULL(CAST(@JIG_ID AS nvarchar(MAX)),'') + ''', @QRCode = ''' + ISNULL(CAST(@QRCode AS nvarchar(MAX)),'') + ''', State Now = ''' + ISNULL(CAST(@Status AS nvarchar(MAX)),'') + ''',@OpNO = ''' 
		...
		BEGIN 
				...
				IF (@jig_state  IN ( 16 , 8 , 18)) --16 = Appearance   , 18 = Clean Ng , 8 = To Clean
				BEGIN
						...
						BEGIN 	 
							...
							SELECT    'TRUE'										AS Is_Pass
									, N'This Carrier ('+ @QRCode + ') To Clean !!'	AS Error_Message_ENG
									, N'Carrier นี้ ('+ @QRCode + N') กำลัง Clean !!'   AS Error_Message_THA
									, ''											AS Handling
							RETURN
						END 
						IF (@Status = '0') 
						BEGIN 	 
							...
							SELECT    'TRUE'										AS Is_Pass
									, N'This Carrier ('+ @QRCode + ') Clean NG !!'	AS Error_Message_ENG
									, N'Carrier นี้ ('+ @QRCode + N')  Clean NG !!'	AS Error_Message_THA
									, ''											AS Handling
							RETURN
						...
						BEGIN 
							...
							SELECT    'TRUE'														AS Is_Pass
									, N'This Socket ('+ @QRCode + ') in Clean!!'					AS Error_Message_ENG
									, N'Socket นี้ ('+ @QRCode + N') ถูกนำเข้าสถานะ Clean เรียบร้อย !!!!'	AS Error_Message_THA
									, ''															AS Handling
							RETURN
						...
						BEGIN
							...
							SELECT    'FALSE'												AS Is_Pass 
									...
									, N'Carrier ('+ @QRCode +N') clean ไม่สำเร็จ !!'			AS Error_Message_THA 
				...
				BEGIN
					...
					SELECT    'FALSE'												AS Is_Pass 
							, 'Please take the carrier ('+ @QRCode +') to appearance !!'	AS Error_Message_ENG
							, N'กรุณานำ carrier ไป ('+ @QRCode +N') Appearance ก่อน !!'			AS Error_Message_THA 
							, N'กรุณาลงทะเบียนที่เว็บ JIG Controlsystem'					AS Handling
							, ''													AS Warning
