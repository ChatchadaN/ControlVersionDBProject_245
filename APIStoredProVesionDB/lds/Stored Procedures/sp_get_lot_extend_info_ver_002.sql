...
-- =============================================
CREATE PROCEDURE [lds].[sp_get_lot_extend_info_ver_002]
	@template_id INT = 0,
...
BEGIN
	...
	WHERE (RT.id = @template_id AND MAP.is_display = 1)
	OR (ME.is_common = 1 AND ME.is_common_display = 1);
...
    SET @params = N'@template_id INT, @flow NVARCHAR(30), @mc_no NVARCHAR(30), @lot_no NVARCHAR(20), @package CHAR(20), @device CHAR(20), @opno_setup VARCHAR(8), @lot_start_time DATETIME, @lot_end_time DATETIME';

	IF (ISNULL(@template_id,0) <> 0)
        SET @sql += N' AND mst.lot_record_templates_id = @template_id';
...
    EXEC sp_executesql @sql, @params, @template_id = @template_id, @flow = @flow, @mc_no = @mc_no, @lot_no = @lot_no, @package = @package, @device = @device, @opno_setup = @opno_setup
