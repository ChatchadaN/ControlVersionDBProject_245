...
-- =============================================
CREATE PROCEDURE [api].[sp_read_ocr_record_ver_002]
...
BEGIN
	...
	SELECT TOP(1000) [lot_marking_verify_records].[id]
	, [lots].[lot_no]
	, [packages].[name] AS [package]
	, [device_names].[name] AS [device]
	, ISNULL([lot_marking_verify_records].[value],'-')
	, CAST([lot_marking_verify_records].[is_pass] AS INT) AS [is_pass]
	, ISNULL([lot_marking_verify_records].[marking_picture_id],0)
