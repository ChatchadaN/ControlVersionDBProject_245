...
-- =============================================
CREATE PROCEDURE [api].[sp_ocr_read_compare_ver_001]
...
BEGIN
	...
	DECLARE @lot_manual_d TABLE(lot_id BIGINT, lot_no CHAR(20))
	...
	BEGIN
		...
		BEGIN
			...
			IF EXISTS(SELECT [master_data].[mark]
				FROM
				(
					...
					, CAST([MANUAL_TITLE_2] AS INT) AS [logo_mark]
					, CAST('' as xml).value('xs:base64Binary(sql:column("picture_data"))', 'varchar(max)') AS [logo_image]
					FROM @lot_master
					INNER JOIN [APCSProDB].[dbo].[LCQW_UNION_WORK_DENPYO_PRINT] ON [LCQW_UNION_WORK_DENPYO_PRINT].[LOT_NO_2] = [@lot_master].[lot_no]
					INNER JOIN [APCSProDBFile].[ocr].[marking_logo_picture] ON [marking_logo_picture].[id] = CAST([LCQW_UNION_WORK_DENPYO_PRINT].[MANUAL_TITLE_2] AS INT)
					UNION ALL
					SELECT CAST(1 AS BIT) AS [status]
					, [lot_masks].[mno] [mark]
					, @is_special AS [is_special]
					, CAST(0 AS BIT) AS [is_request]
					, @lot_no AS [lot_no]
					, CAST(0 AS INT) AS [logo_mark]
					, '' AS [logo_image]
					FROM [APIStoredProDB].[dbo].[lot_masks]
					WHERE [lot_masks].[lot_no] = @lot_no
					UNION ALL
					SELECT CAST(1 AS BIT) AS [status]
					, [lot_masks].[mno] [mark]
					, @is_special AS [is_special]
					...
					, '' AS [logo_image]
					FROM [APIStoredProDB].[dbo].[lot_masks]
					INNER JOIN @lot_manual_d AS [manual_lots] ON [manual_lots].[lot_no] = [lot_masks].[lot_no]
				) AS [master_data]
				WHERE REPLACE(@mark,' ','') LIKE '%' + REPLACE([master_data].[mark],' ','') + '%'
			)
			BEGIN
				SELECT CAST(1 AS BIT) AS [status]
			END
