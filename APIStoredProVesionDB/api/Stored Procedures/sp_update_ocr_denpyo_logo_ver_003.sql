...
-- =============================================
CREATE PROCEDURE [api].[sp_update_ocr_denpyo_logo_ver_003]
...
BEGIN
	...
	INNER JOIN [APCSProDB].[trans].[lots] ON [LCQW_UNION_WORK_DENPYO_PRINT].[LOT_NO_2] = [lots].[lot_no]
	INNER JOIN [APCSProDB].[trans].[days] ON [lots].[in_plan_date_id] = [days].[id]
	...
	WHERE LEN(MANUAL_TITLE_2) = 0
		AND [days].[date_value] = FORMAT(GETDATE(), 'yyyy-MM-dd'); 
END
