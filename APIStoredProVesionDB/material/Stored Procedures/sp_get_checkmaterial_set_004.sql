...
-- =============================================
CREATE PROCEDURE [material].[sp_get_checkmaterial_set_004]
...
SET @CheckCount2 = ( SELECT COUNT(*)  FROM STRING_SPLIT(@QRCode,';') )
	...
	IF (@package_id IN  (80,81,88,89,82, 87, 91,232,233, 246 , 242))	
		BEGIN
			...
			BEGIN
				...
				-- CHECK MATERIAL IS MATCHING WITH LOT
								IF  EXISTS (SELECT Prod_name FROM  #TEMP_RIST INNER JOIN ( SELECT productions.name 
																					  FROM APCSProDB.trans.lots
																					  INNER JOIN APCSProDB.method.device_flows
																					  ON lots.device_slip_id = device_flows.device_slip_id
																					  AND  device_flows.step_no = @step_no_now
			...
			BEGIN
				...
				SELECT   'TRUE'		AS Is_Pass
					...
					SELECT GETDATE()
						...
						,HOST_NAME()
						,APP_NAME()
						,'TRUE RETURN ALL QR CODE'  + ' ' + CAST(@QRCode AS nvarchar(MAX))  + ' , PACKAGE :' +  CAST(@package_id AS nvarchar(MAX)) 
						, @LotNo
