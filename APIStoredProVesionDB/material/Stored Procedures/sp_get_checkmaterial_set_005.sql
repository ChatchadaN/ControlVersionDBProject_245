...
-- =============================================
CREATE PROCEDURE [material].[sp_get_checkmaterial_set_005]
...
SET @CheckCount2 = ( SELECT COUNT(*)  FROM STRING_SPLIT(@QRCode,';') )
	...
	END	
	IF (@package_id IN  (80,81,88,89,82, 87, 91,232,233, 246 ,242, 121, 505, 103, 53,54  ,218 , 219))	
		BEGIN
								...
								BEGIN 
									...
									--SET @Expire_Date  = (SELECT Expire_Date FROM #TEMP) 
									 ...
									 --BEGIN
										...
										IF NOT  EXISTS ( SELECT  CASE WHEN lots.is_special_flow =  1 THEN    [lot_special_flows].material_set_id 
																ELSE device_flows.material_set_id   END 
																FROM APCSProDB.trans.lots  
																INNER JOIN APCSProDB.method.device_flows
																ON lots.device_slip_id = device_flows.device_slip_id
																...
																ON [special_flows].id = [lot_special_flows].special_flow_id
																AND [lot_special_flows].step_no =  [special_flows].step_no
																WHERE  lots.id  =   @lot_id AND device_flows.material_set_id IS NOT  NULL
														)
										BEGIN 
												SELECT  'FALSE' AS Is_Pass
														,'Device slips this material has not been registered yet !!' AS Error_Message_ENG
