...
-- =============================================
CREATE PROCEDURE [material].[sp_get_checkmaterial_set_009]
	...
	END
		...
		BEGIN
			...
			-- CHECK EMBOSS HAVE IN DB
					IF NOT EXISTS (SELECT id FROM    APCSProDB.material.productions WHERE  REPLACE(UPPER(name),' ','') = (SELECT REPLACE(UPPER( Prod_name),' ','') FROM  @TEMP_RIST)
			...
			-- CHECK COVER TAPE HAVE IN DB 
 					IF NOT EXISTS (SELECT id FROM    APCSProDB.material.productions WHERE REPLACE(REPLACE(UPPER(productions.name),' ',''),'MM','') = (SELECT  REPLACE(REPLACE(UPPER(Prod_name),' ',''),'MM','') FROM  @TEMP_RIST)
																							AND productions.details =  @Material_type	)
						BEGIN 
			...
			-- CHECK COVER TAPE,EMBOSS TAPE AFTER PIVOT
					...
					IF  EXISTS (SELECT id FROM  APCSProDB.material.productions WHERE REPLACE(UPPER(productions.name),' ','') = (SELECT REPLACE(UPPER(Prod_name),' ','') FROM  @TEMP_RIST))
					BEGIN 
			-- CHECK TYPE IN DB
						IF  EXISTS (SELECT id FROM    APCSProDB.material.productions WHERE REPLACE(UPPER(productions.name),' ','') = (SELECT  REPLACE(UPPER(Prod_name),' ','') FROM  @TEMP_RIST)
																							AND productions.details =  @Material_type	)
			...
			-- CHECK MATERIAL IS MATCHING WITH LOT
							IF  EXISTS (SELECT Prod_name FROM  @TEMP_RIST INNER JOIN (SELECT productions.name  FROM 
														...
														ON REPLACE(REPLACE(UPPER(Prod_name),' ',''),'MM','')   = REPLACE(REPLACE(UPPER(production_name.name),' ',''),'MM','') 
													)
									BEGIN 
		...
		END 
			...
			BEGIN 
				...
				-- CHECK COVER HAVE IN DB
						IF  EXISTS (SELECT id FROM    APCSProDB.material.productions WHERE  REPLACE(UPPER(name),' ','') = (SELECT REPLACE(UPPER(Prod_name),' ','') FROM  @TEMP)
 				...
 				-- PIVOT COVER TAPE ( Z7400,13.5,500,2DS0971A15,Dec.2024 )
 						IF NOT EXISTS (SELECT id FROM    APCSProDB.material.productions WHERE REPLACE(REPLACE(UPPER(productions.name),' ',''),'MM','') = (SELECT  REPLACE(REPLACE(UPPER(Prod_name),' ',''),'MM','') FROM  @TEMP)
																									AND productions.details =  @Material_type	)
							BEGIN 
				...
				-- PIVOT COVER TAPE ( 00000,9.5MM,2023/03/31,QQQQ,2024/03/31 )
						...
						IF NOT EXISTS (SELECT id FROM    APCSProDB.material.productions WHERE  REPLACE(UPPER(name),' ','') = (SELECT REPLACE(UPPER(Prod_name),' ','') FROM  @TEMP)
																								AND productions.details =  @Material_type	)
							BEGIN 
				-- PIVOT COVER TAPE ( 9.5MM,LOTNO,2023/03/31,2024/03/31,00000 )
									DELETE FROM  @TEMP
				...
				-- CHECK COVER TAPE,EMBOSS TAPE AFTER PIVOT
						IF  EXISTS (SELECT id FROM  APCSProDB.material.productions WHERE REPLACE(UPPER(productions.name),' ','') = (SELECT REPLACE(UPPER(Prod_name),' ','') FROM  @TEMP))
						BEGIN 
				-- CHECK TYPE IN DB
							IF  EXISTS (SELECT id FROM    APCSProDB.material.productions WHERE REPLACE(UPPER(productions.name),' ','') = (SELECT  REPLACE(UPPER( Prod_name),' ','') FROM  @TEMP)
																								AND productions.details =  @Material_type	)
		...
		END
		ELSE IF (@CheckCount2 = 2)  --M30617413604-05;A0254
		BEGIN 
							DECLARE  @TEMP1 TABLE
							(
										Material_LotNo	NVARCHAR(100) 
							...
							IF  EXISTS (SELECT id FROM    APCSProDB.material.productions WHERE  REPLACE(UPPER(name),' ','') = (SELECT REPLACE(UPPER(Prod_name),' ','') FROM  @TEMP1))
					...
					-- CHECK MATERIAL SET ID IN DB
									...
									ELSE 
									BEGIN
											IF  EXISTS (SELECT Prod_name FROM  @TEMP1 
																		INNER JOIN ( SELECT productions.name  FROM 
																					(SELECT  CASE WHEN lots.is_special_flow =  1 THEN    [lot_special_flows].material_set_id 
																					ELSE device_flows.material_set_id   END  AS material_set_id  
																					,  CASE WHEN lots.is_special_flow =  1 THEN    [lot_special_flows].act_process_id 
																					ELSE device_flows.act_process_id   END  AS act_process_id 
																...
																ON REPLACE(UPPER( Prod_name),' ','')  = REPLACE(UPPER(production_name.name),' ','') 
															)
											BEGIN 
		...
		BEGIN 
							...
							IF  EXISTS (SELECT id FROM    APCSProDB.material.productions WHERE  REPLACE(UPPER(name),' ','') = (SELECT REPLACE(UPPER( Prod_name),' ','') FROM  @TEMP2))
					...
					-- CHECK MATERIAL SET ID IN DB
									...
									ELSE 
									BEGIN
											IF  EXISTS (SELECT Prod_name FROM  @TEMP2 
																		 INNER JOIN ( SELECT productions.name  FROM 
																					(SELECT  CASE WHEN lots.is_special_flow =  1 THEN    [lot_special_flows].material_set_id 
																					ELSE device_flows.material_set_id   END  AS material_set_id  
																					,  CASE WHEN lots.is_special_flow =  1 THEN    [lot_special_flows].act_process_id 
