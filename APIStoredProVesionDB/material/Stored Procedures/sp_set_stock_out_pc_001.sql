...
-- =============================================
CREATE PROCEDURE [material].[sp_set_stock_out_pc_001]
...
BEGIN
	...
	BEGIN TRY
		...
		INSERT INTO @TargetIDs (material_id)
		SELECT  [value] FROM STRING_SPLIT(@material_id,',')
		...
		BEGIN
			...
			SET	  materials.location_id = ( SELECT TOP 1 id   FROM  APCSProDB.material.locations WHERE wh_code  = 'TR000')  
				, updated_at =  GETDATE() 
				, updated_by =  @emp_id 
			WHERE  id  =  @CurrentID
