...
-- Description:	get lot in process
-- =============================================
CREATE PROCEDURE [atom].[sp_get_lot_in_process_001]	-- Add the parameters for the stored procedure here	
	@lot_no varchar(10) = '%'
	, @lot_type varchar(1) = '%'
...
BEGIN
 ...
 --       )
	...
	select [id]
		...
		, CONVERT(VARCHAR,[update_time],120)  AS update_time
		, [total]
		, [good]
		, [ng]
		, [operator]
		...
		, [andon] 
		, [app_name]
		, CONVERT(VARCHAR,[plan_shipdate],23)  AS [plan_shipdate]
		, [e_slip_id]
	from (
		select [lots].[id]
			, [lots].[lot_no]
	...
	where   [lots].[lot_no]   =  @lot_no				OR [lots].[lot_no]   IS NULL
		and [lots].[lot_type] = @lot_type				OR [lots].[lot_type] IS NULL
		and [lots].[package_group] = @package_group		OR [lots].[package_group]  IS NULL
		and [lots].[package] = @package					OR [lots].[package]  IS NULL
		and [lots].[device] = @device					OR [lots].[device]  IS NULL
		and [lots].[process] = @process					OR [lots].[process]  IS NULL
		and [lots].[operation] = @job					OR [lots].[operation] IS NULL
		and [lots].[process_state] = @process_state		OR [lots].[process_state]  IS NULL
		and [lots].[quality_state] = @quality_state		OR [lots].[quality_state]  IS NULL
		and upper([lots].[delay_status]) = REPLACE(@status, ' ', '')
...
--				, N'ไม่พบข้อมูลLot!!'		AS Error_Message_THA
--				, ''						AS Handling
