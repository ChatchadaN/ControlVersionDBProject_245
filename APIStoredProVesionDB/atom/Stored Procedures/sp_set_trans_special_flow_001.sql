...
-- =============================================
CREATE PROCEDURE [atom].[sp_set_trans_special_flow_001]
...
BEGIN
	...
	select GETDATE()
		...
		, HOST_NAME()
		, APP_NAME()
		, 'exec [atom].[sp_set_trans_special_flow_001] @lot_id = ''' + ISNULL(CAST(@lot_id AS varchar),'') 
	...
	begin catch
		...
		select GETDATE()
			...
			, HOST_NAME()
			, APP_NAME()
			, 'rollback exec [atom].[sp_set_trans_special_flow_001] @lot_id = ''' + ISNULL(CAST(@lot_id AS varchar),'') 
				+ ''', @is_special_flow = ''' + ISNULL(CAST(@is_special_flow AS varchar),'')
				+ ''', @step_no = ''' + ISNULL(CAST(@step_no AS varchar),'') 
				+ ''', @flow_pattern_id = '''+ ISNULL(CAST(@flow_pattern_id AS varchar),'') 
				+ ''', @machine_id = ''' + ISNULL(CAST(@machine_id AS varchar),'') 
