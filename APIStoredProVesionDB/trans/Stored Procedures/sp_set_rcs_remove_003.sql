...
-- =============================================
CREATE PROCEDURE [trans].[sp_set_rcs_remove_003]
...
BEGIN
	...
	DECLARE @nowDateTime datetime = (SELECT GETDATE()), 
			@OldRackId int, @LotId int = (SELECT id FROM APCSProDB.trans.lots WHERE lot_no = @Item),
  	...
  	--GET Categories Rack
	DECLARE @WHCode INT
	SELECT @WHCode = wh_code FROM APCSProDB.trans.locations
	WHERE id = @Address_id
	...
	BEGIN TRY
		...
		BEGIN
			SELECT TOP(1) @OldRackId = curr.location_id
		...
		BEGIN
			SELECT @Is_Pass = 'FALSE',
				   @Error_Message_ENG = 'This rack is Disabled Location.',
				   @Error_Message_THA = N'Rack นี้ไม่เปิดใช้งาน กรุณาตรวจสอบข้อมูล',
				   @Handling = N'Please check your data.'
	...
	BEGIN CATCH
		SELECT @Is_Pass = 'FALSE',
			   @Error_Message_ENG = ERROR_MESSAGE(),
			   @Error_Message_THA = ERROR_MESSAGE(),
			   @Handling = N'กรุณาตรวจสอบข้อมูลที่ Rack Control System (APCSPro)'
	...
	Finally:
		...
		SELECT GETDATE()
			...
			, 'EXEC [trans].[sp_set_rcs_remove_003] @LotNo  = ''' + ISNULL(CAST(@Item AS nvarchar(MAX)),'') 
				...
				+ ''',@Rack = ''' + ISNULL(CAST(@Rack AS nvarchar(MAX)),'') + 
				+ ''',@WHCode = ''' + ISNULL(CAST(@WHCode AS nvarchar(MAX)),'') + 
