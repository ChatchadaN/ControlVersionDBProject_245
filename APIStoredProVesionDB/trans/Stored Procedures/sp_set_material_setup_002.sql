...
-- =============================================
 CREATE PROCEDURE [trans].[sp_set_material_setup_002] 
...
BEGIN
	...
	IF EXISTS( SELECT 1 FROM  DBx.MAT.MixAGPaste  WHERE DBx.MAT.MixAGPaste.QRCode =  @barcode  AND @barcode like 'MAT%')
		BEGIN 
				...
				ON  MixAGPaste.AGPasteType =  DBx.MAT.Material.Model 
...
--//////////////////////// Update Machine Materials   2022/02/10
				...
				--////////////////// UPDATE Material Out
					...
					IF @type = 'SOLDER TAPE' OR @type = 'SOLDER BALL' OR @type = 'PASTE'  BEGIN
						SELECT @barcode_out = materials.barcode FROM APCSProDB.trans.machine_materials 
							INNER JOIN APCSProDB.mc.machines ON machines.id = machine_materials.machine_id
							INNER JOIN APCSProDB.trans.materials ON materials.id = machine_materials.material_id
						WHERE machines.name = @mcno AND machine_materials.idx = 1
					...
					--		INNER JOIN APCSProDB.mc.machines ON machines.id = machine_materials.machine_id
					--		INNER JOIN APCSProDB.trans.materials ON materials.id = machine_materials.material_id
					--	WHERE machines.name = @mcno AND machine_materials.idx = 2
					--END
					...
					BEGIN
						...
						END
					END
					ELSE IF @type = 'SOLDER TAPE' OR @type = 'SOLDER BALL'  OR @type = 'PASTE' 
					BEGIN
						UPDATE APCSProDB.trans.machine_materials 
						SET material_id = @material_id
							,[updated_at] = GETDATE()
				...
				FROM DBx.MAT.MixAGPaste 
				INNER JOIN DBx.MAT.Material 
				ON  (SELECT SUBSTRING(REPLACE(MixAGPaste.AGPasteType, '-', '') ,0,CASE WHEN CHARINDEX('/', MixAGPaste.AGPasteType) = 0 THEN LEN(REPLACE(MixAGPaste.AGPasteType, '-', ''))+1 ELSE CHARINDEX('/',REPLACE(MixAGPaste.AGPasteType, '-', ''))  END ))  =
					(SELECT SUBSTRING(REPLACE(DBx.MAT.Material.Model , '-', '') ,0,CASE WHEN CHARINDEX('/', DBx.MAT.Material.Model ) = 0 THEN LEN(REPLACE(DBx.MAT.Material.Model , '-', ''))+1 ELSE CHARINDEX('/',REPLACE(DBx.MAT.Material.Model , '-', ''))  END )) 
				WHERE (MixAGPaste.QRCode = @barcode )   
				ORDER BY  MixAGPaste.FinishTimeMix ASC
