...
-- =============================================
CREATE PROCEDURE  [trans].[sp_get_material_type_007]
	...
	BEGIN
		...
		BEGIN 
				...
				BEGIN
					...
					--5 use time
					IF @date_now < @wait_limit_date  
					BEGIN
						SELECT 'FALSE' AS Is_Pass
						, 'Resin type is not usable. ('+CONVERT(VARCHAR,@wait_limit_date,121) +')' AS Error_Message_ENG
						, N'Resin Type นี้นำมาใช้งานก่อนกำหนด ('+CONVERT(VARCHAR,@wait_limit_date,121) +') ' AS Error_Message_THA
						, N'กรุณาตรวจสอบข้อมูลที่เว็บ material' AS Handling
						RETURN
					END
					 
					--6 expire time
					IF @date_now > @open_limit_date1
					BEGIN 

						SELECT 'FALSE' AS Is_Pass
						, 'Resin type has expired. ('+CONVERT(VARCHAR,@open_limit_date1,121) +')' AS Error_Message_ENG
						, N'Resin Type หมดอายุ.('+CONVERT(VARCHAR,@open_limit_date1,121) +')' AS Error_Message_THA
						, N'กรุณาตรวจสอบข้อมูลที่เว็บ material' AS Handling
						RETURN
					END
