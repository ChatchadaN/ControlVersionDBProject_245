...
-- Author:		KITTITAT
-- =============================================
CREATE PROCEDURE [trans].[sp_set_carrier_002]
	@lot_no nvarchar(20),
...
BEGIN
	...
	SELECT GETDATE()
		...
		, HOST_NAME()
		, APP_NAME()
		, 'EXEC [APIStoredProVersionDB].[trans].[sp_set_carrier_002] @lot_no = ''' + ISNULL(CAST(@lot_no AS varchar),'') 
		...
		ELSE BEGIN
			...
			ELSE BEGIN
				...
				SELECT 'TRUE' as Is_Pass
					...
					, N'' AS Error_Message_THA
					, N'' AS Handling
					, @next_carrier_no as next_carrier_no
					, @lot_no as lot_no
			END
		END
		RETURN;
