...
-- =============================================
CREATE PROCEDURE [trans].[sp_get_rcs_rack_pro_001]
...
BEGIN
	...
	SELECT 
		...
		@JobId = CASE 
		   ...
		   END
		--@JobId = CASE WHEN @isCurrentStepNo = 1 THEN currentDevFlow.job_id ELSE nextDevFlow.job_id END
		...
		BEGIN
			...
			BEGIN
				SELECT 'FALSE'					AS Is_Pass
					...
					, N'ไม่พบข้อมูล Rack กรุณาตรวจสอบข้อมูล'				AS Error_Message_THA
					, 'Please Contact SYSTEM'		AS Handling
	 				, ISNULL(@newRackName, '')		AS new_name 
					, ISNULL(@newRackName, '')		AS new_address 
			...
			INNER JOIN APCSProDB.rcs.rack_addresses as rack_addresses ON rack_controls.id = rack_addresses.rack_control_id
			...
			--Step 1 GET Rack_id จาก pk, device, job และ priority AND Check Rack Space
			SET @get_rack_id = (SELECT TOP 1 rack_id FROM @RackAuto_TB ORDER BY priority_rack , rack_name ASC)
		
			IF @get_rack_id IS NULL
			BEGIN
				Print 'Rack Setting not set'
			...
			ELSE
			BEGIN
				--Step 2  Retrieve New Rack Address Auto
				DECLARE @RackName VARCHAR(50)
				SET @RackName =(SELECT rack_controls.name FROM APCSProDB.rcs.rack_controls WHERE id = @get_rack_id)
				
				SELECT TOP 1 
					@newAddressID	= rack_addresses.id
					, @newRackName	= rack_controls.[name]
					, @newAddress	= rack_addresses.[address]
				FROM APCSProDB.rcs.rack_addresses
				INNER JOIN APCSProDB.rcs.rack_controls ON rack_addresses.rack_control_id = rack_controls.id
				WHERE rack_control_id = @get_rack_id AND rack_addresses.item IS NULL
				...
				BEGIN
					...
					SELECT 'FALSE'						AS Is_Pass
						...
						, CONCAT(N'Rack : ', @RackName ,N'เต็ม กรุณาตรวจสอบข้อมูล')		AS Error_Message_THA
						, 'Please Contact SYSTEM'		AS Handling
	 					, ISNULL(@newRackName, '')		AS new_name 
						, ISNULL(@newRackName, '')		AS new_address 
						, @newAddressID					AS new_address_id
