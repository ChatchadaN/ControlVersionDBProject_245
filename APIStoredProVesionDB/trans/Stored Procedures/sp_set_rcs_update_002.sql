...
-- =============================================
CREATE PROCEDURE [trans].[sp_set_rcs_update_002]
...
BEGIN
	...
	DECLARE @nowDateTime datetime = (SELECT GETDATE()) 
			, @WHCode		INT
			, @OldRackId	INT
			, @LotId		INT = (SELECT id FROM APCSProDB.trans.lots WHERE lot_no = @Item)
			, @Is_Pass		BIT
			, @Error_Message_ENG VARCHAR(100)
			, @Error_Message_THA NVARCHAR(100)
	...
	--CHECK IF item in RCS PRO || CHECK ไม่มี item นี้'วาง'อยู่ที่เก่า
	IF NOT EXISTS(SELECT 1 FROM DBx.dbo.rcs_current_locations WHERE location_id = @Address_id and lot_id = @LotId AND rcs_current_locations.[status] = 1)
	AND EXISTS (SELECT 1 FROM APCSProDB.rcs.rack_addresses WHERE item = @Item AND id = @Address_id AND [status] = 1 )
	BEGIN
		...
		SELECT GETDATE()
			...
			,HOST_NAME()
			,APP_NAME()
			, 'EXEC [trans].[sp_set_rcs_input_rack_004_multi]  @Item  = ''' + ISNULL(CAST(@Item AS nvarchar(MAX)),'') 
		...
		EXEC [APIStoredProVersionDB].[trans].[sp_set_rcs_input_rack_004_multi] 
		 @emp_id			= 	@emp_id			
		 ,  @App_Name		=   @App_Name		
		 ,  @Item			=   @Item	
