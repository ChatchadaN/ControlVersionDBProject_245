...
-- Author:		KITTITAT
-- =============================================
CREATE PROCEDURE [trans].[sp_get_carrier_003]
...
BEGIN
	...
	SELECT GETDATE()
		...
		, HOST_NAME()
		, APP_NAME()
		, 'EXEC [APIStoredProVersionDB].[trans].[sp_get_carrier_003] @lot_no = ''' + ISNULL(CAST(@lot_no AS varchar),'') 
	...
	BEGIN
		IF (
			SELECT COUNT([jigs].[qrcodebyuser])
			...
			WHERE [categories].[short_name] = 'Carrier'
			  AND [jigs].[qrcodebyuser] = @carrier_no
		) = 0
		BEGIN
			SELECT 'FALSE' as Is_Pass
	...
	BEGIN
		IF (
			...
			INNER JOIN APCSProDB.trans.jigs 
				ON [productions].[id] = [jigs].[jig_production_id]
			WHERE [categories].[short_name] = 'Carrier'
			  AND [jigs].[qrcodebyuser] = @next_carrier_no
		) = 0
		BEGIN
			SELECT 'FALSE' as Is_Pass
