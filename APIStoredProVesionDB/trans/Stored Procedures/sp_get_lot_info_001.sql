...
-- =============================================
CREATE PROCEDURE [trans].[sp_get_lot_info_001]
...
BEGIN
	...
	SELECT GETDATE()
		...
		, HOST_NAME()
		, APP_NAME()
		, 'EXEC [APIStoredProVersionDB].[trans].[sp_get_lot_info_001] @e_slip_id = ''' + ISNULL(CAST(@e_slip_id AS varchar),'') 
	...
	BEGIN
		...
		BEGIN 
			...
			---------------------------------------------------------
			IF EXISTS (SELECT 1 FROM APCSDB.dbo.LCQW_UNION_WORK_DENPYO_PRINT WHERE LOT_NO_2 = @lot_no)
			BEGIN
