...
-- =============================================
 CREATE PROCEDURE [trans].[sp_set_material_setup_007] 
...
BEGIN
	...
	(
		mat_type	NVARCHAR(100)
		, idx		INT 
		, to_idx	INT 
		, count_idx	INT 
	)
	INSERT INTO @condition 
	VALUES ('BONDING WIRE' , 1 , 10, 10)
	, ('FRAME' , 11 , 20 , 10)	
	, ('SOLDER TAPE' , 1 , 5 , 3)	
	, ('SOLDER BALL' , 1 , 5 , 3)
	, ('PASTE' , 1 , 5 , 3)	
 ...
 --IF EXISTS( SELECT 1 FROM  DBx.MAT.MixAGPaste  WHERE DBx.MAT.MixAGPaste.QRCode =  @barcode  AND @barcode like 'MAT%')
	...
	--						,  ISNULL(CONVERT(NVARCHAR,@wait_limit_date,20 ),'')					AS wait_limit_date
	--						,  ISNULL(CONVERT(NVARCHAR,@open_limit_date1,20 ),'')					AS open_limit_date
	...
	IF @Location_id <> 9   
	BEGIN  --AND @Mat_state NOT IN (1,2)
		SELECT 'FALSE' AS Is_Pass ,
		...
		RETURN
	END
	ELSE IF (SELECT  COUNT(machine_materials.idx) AS idx_number   
			 FROM APCSProDB.trans.machine_materials 
			 ...
			 INNER JOIN @condition  AS con
			 ON  machine_materials.idx  BETWEEN con.idx AND con.to_idx
			 WHERE machines.id   = @MC_ID 
			 AND   con.mat_type = @type	 
			 AND material_id <>  @material_id
			 ) >= (SELECT  count_idx FROM    @condition WHERE mat_type = @type	 )
	 BEGIN 
			SELECT 'FALSE' AS Is_Pass 
		...
		BEGIN	
			...
			BEGIN TRY
				...
				SET [material_state] = 12
					...
					, is_production_usage = IIF(@mat_type = 'WAFER Surpluses',ISNULL(is_production_usage,0) + 1  , is_production_usage)
				WHERE barcode = @barcode


--//////////////////////// Update Machine Materials   2022/02/10      this machine have material set up on machine
			...
			BEGIN
				--////////////////// UPDATE MATERIAL OUT
					...
					IF EXISTS (SELECT 1 FROM APCSProDB.trans.machine_materials WHERE machine_id = @MC_ID AND material_id = @material_id)
					BEGIN
					...
					BEGIN
						...
						IF NOT EXISTS (SELECT 1 FROM APCSProDB.trans.machine_materials  
						WHERE machine_id = (SELECT id FROM [APCSProDB].[mc].[machines] where name = @mcno) AND idx = @idx)
						BEGIN 
			...
			BEGIN
					IF @type = 'BONDING WIRE' 
					BEGIN
							IF	(SELECT COUNT(machine_materials.idx) AS idx_number   
								 FROM APCSProDB.trans.machine_materials 
								 INNER JOIN APCSProDB.mc.machines ON machines.id = machine_materials.machine_id
								 INNER JOIN APCSProDB.trans.materials ON materials.id = machine_materials.material_id
								 WHERE machine_materials.machine_id = @MC_ID  AND machine_materials.idx BETWEEN 1 AND 10 
								 ) >= 10
							BEGIN 
										SELECT 'FALSE' AS Is_Pass 
												,N'PLEASE CLEAR MATERIAL ON MACHINE !!' AS Error_Message_ENG
