...
-- =============================================
CREATE PROCEDURE [trans].[sp_set_material_outoffmachine_002] 
...
BEGIN
	...
	IF EXISTS( SELECT 1 FROM  DBx.MAT.MixAGPaste  WHERE DBx.MAT.MixAGPaste.QRCode =  @barcode)
		BEGIN 
	...
	SELECT @material_id = m.id, @Location_id_out  = location_id, @Mat_state_out  = material_state 
	,@mat_type_out  = p.name, @limitdate_out  = ISNULL(m.extended_limit_date,m.limit_date)
	, @qty_out = m.quantity
		...
		BEGIN TRANSACTION
			BEGIN TRY
				...
				INSERT INTO [APCSProDB].[trans].[material_records]
				   ...
				   ,[to_location_id])
					...
					SELECT @mat_record_id_out AS [id]
						...
						,[is_production_usage]
						,IIF(@is_outoffstock = 1,0,IIF([materials].[quantity] = 0,0,2)) AS [material_state] --0 Out of stock,2 Used
						,[process_state]
						,[qc_state]
						,[first_ins_state]
						,[final_ins_state]
				...
				--update material out
				   ...
				   SET 
					  [material_state] = IIF(@is_outoffstock = 1,0,IIF([materials].[quantity] = 0,0,2)) --1
					  --,[location_id] = 9
					  ,[updated_at] = GETDATE()
