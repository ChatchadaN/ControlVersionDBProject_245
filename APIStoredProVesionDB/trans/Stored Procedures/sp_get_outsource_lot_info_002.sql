...
-- =============================================
CREATE PROCEDURE [trans].[sp_get_outsource_lot_info_002] 
...
BEGIN
    ...
    -- Insert statements for procedure here	
		...
		IF EXISTS (SELECT 1 FROM OPENDATASOURCE ('SQLNCLI', 'Data Source = 10.28.1.144; User ID=ship;Password=ship;' ).[Half_Product].[dbo].[Half_Product_Order_List] hp
					INNER JOIN APCSProDB.dbo.LCQW_UNION_WORK_DENPYO_PRINT dp ON dp.LOT_NO_2 = TRIM(hp.LotNo)
					INNER JOIN APCSProDB.trans.lots ON lots.lot_no = TRIM(hp.LotNo)
					WHERE  OutSourceLotNo   = @lot_outsource  AND lots.e_slip_id IS NULL AND lots.carrier_no IS NULL)
		BEGIN 
