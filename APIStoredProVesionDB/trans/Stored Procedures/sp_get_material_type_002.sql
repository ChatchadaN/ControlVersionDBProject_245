...
-- =============================================
CREATE PROCEDURE  [trans].[sp_get_material_type_002]
...
--@barcode = ''(ข้อมูลที่ตัดได้)
	...
	SET @barcode = SUBSTRING(@Barcode, LEN(@Barcode) - 11, 12);
	...
	BEGIN
		...
		BEGIN 
				...
				BEGIN
						...
						SET @MANU_COND_PRIFORM =  (SELECT TRIM(MANU_COND_PRIFORM)   FROM  APCSDB.dbo.LCQW_UNION_WORK_DENPYO_PRINT
		...
		END 
		ELSE  IF EXISTS( SELECT 1 FROM  DBx.MAT.MixAGPaste  WHERE DBx.MAT.MixAGPaste.QRCode =  @barcode)
		BEGIN 
				SELECT    @barcode				=  MixAGPaste.QRCode
						...
						, @STDLifeTimeUser		= __Material.STDLifeTimeUser
						, @PreformExp			= MixAGPaste.StartTimeMix + __Material.STDLifeTimeUser 
				FROM DBx.MAT.MixAGPaste 
				INNER JOIN DBx.MAT.__Material 
				ON  MixAGPaste.AGPasteType =  DBx.MAT.__Material.Model 
				...
				END 
			 	...
			 	SELECT    @barcode				=  MixAGPaste.QRCode
						...
						, @limitdate			= MixAGPaste.StartTimeMix + __Material.STDLifeTimeUser 
				FROM DBx.MAT.MixAGPaste 
				INNER JOIN DBx.MAT.__Material 
				ON  MixAGPaste.AGPasteType =  DBx.MAT.__Material.Model 
				WHERE (MixAGPaste.QRCode = @barcode )   
