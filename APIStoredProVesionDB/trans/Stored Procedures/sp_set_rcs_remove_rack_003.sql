...
-- =============================================
CREATE PROCEDURE [trans].[sp_set_rcs_remove_rack_003] 
...
BEGIN
	...
	FROM [APCSProDB_229].[rcs].[rack_controls]
	...
	INNER JOIN [APCSProDB_229].[rcs].[rack_addresses] ON [rack_controls].[id] = [rack_addresses].[rack_control_id]
	...
	BEGIN TRY
		...
		BEGIN
			...
			UPDATE [APCSProDB_229].[rcs].[rack_addresses]
			SET [item] = NULL,
				[status] =  @Status,
				[updated_at] = GETDATE(),
				[updated_by] = @emp_id
			...
			INSERT INTO [APCSProDB_229].[rcs].[rack_address_records]
			SELECT 
				GETDATE()
				,'2'
				,[id]
				...
				,[updated_at]
				,[updated_by]
			FROM [APCSProDB_229].rcs.rack_addresses
			WHERE [id] = @Address_id 
				AND [rack_control_id] = @rack_crt_id
			...
			BEGIN
				UPDATE [APCSProDB_229].[trans].[surpluses]
				SET [location_id] = NULL,
					[updated_at] = GETDATE(),
					[updated_by] = @emp_id
			...
			BEGIN
				UPDATE [APCSProDB_229].[trans].[jigs]
				SET [location_id] = NULL,
					[updated_at] = GETDATE(),
					[updated_by] = @emp_id
				WHERE [barcode] = @Item;
